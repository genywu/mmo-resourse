...
#pragma comment(lib, "../appworld/anticheat/ijl15l.lib")
...
//	SAFEDELETEARRAY(pFileData);
	...
	m_pszImageFileName = (char*)M_ALLOC(st + 1);
...
bool CImage::ReadBitmapFromMemory(const BYTE * pData, unsigned int uiDataLen)
{
	...
	BYTE * pFileData = (BYTE*)M_ALLOC(uiDataLen + 1);			//临时文件数据
...
bool CImage::ExportJpegToMemory(BYTE *& pData, DWORD & dwSize, int iQuality)
{
	...
	BYTE * byBuffer = (BYTE*)M_ALLOC(m_dwWidth * m_dwHeight * bpp);
...
void CImage::Rotate(float fRotAngle)
{
	...
	DWORD * pBitmapBuffer = (DWORD*)M_ALLOC(m_dwBitmapSize*sizeof(DWORD));
...
void CImage::Zoom(int iScale)
{
	...
	if (iScale > 1)
	{
		...
		apNew		   = (DWORD*)M_ALLOC(m_dwBitmapSize*sizeof(DWORD));
		apBuffer	   = apNew;
	...
	m_pBitmapOri	= (DWORD*)M_ALLOC(m_dwBitmapSize*sizeof(DWORD));
...
bool CImage::CreateBitmap(DWORD dwWidth, DWORD dwHeight, DWORD dwColor)
{
	...
	m_pBitmapOri    = (DWORD*)M_ALLOC(m_dwBitmapSize*sizeof(DWORD));
	m_pBitmapBuffer = (DWORD*)M_ALLOC(m_dwBitmapSize*sizeof(DWORD));
...
{
	DWORD * pLineTemp  = (DWORD*)M_ALLOC(m_dwWidth*sizeof(DWORD));
	if (pLineTemp == NULL)
		return;
