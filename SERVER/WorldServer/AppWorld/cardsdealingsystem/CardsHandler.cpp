...
DWORD CCardsHandler::QueryChargePriceByTimeType( CCard::eCardTime cardtime )
{
	...
	{
	case CCard::CT_6:
		return CGlobeSetup::GetSetup()->m_dwCard6HoursChargePrice;
	case CCard::CT_12:
		return CGlobeSetup::GetSetup()->m_dwCard12HoursChargePrice;
	case CCard::CT_24:
		return CGlobeSetup::GetSetup()->m_dwCard24HoursChargePrice;
	default:
		return 0;
	}
}
...
DWORD CCardsHandler::QueryTimeByTimeType( CCard::eCardTime cardtime )
{
	...
	{
	case CCard::CT_6:
		return CGlobeSetup::GetSetup()->m_dwCard6Hours;
	case CCard::CT_12:
		return CGlobeSetup::GetSetup()->m_dwCard12Hours;
	case CCard::CT_24:
		return CGlobeSetup::GetSetup()->m_dwCard24Hours;
	default:
		return 0;
	}
}
...
void CCardsHandler::AddDenys(CCard *card)
{
	...
	{
		// init di
		DenyInfo *di = OBJ_CREATE(DenyInfo);
...
int CCardsHandler::Check(CCard *card)
{
	...
	if( itC != _DenyCards.end() ) // insert
	{
		if(itC->second->times >= CGlobeSetup::GetSetup()->m_dwCardRetryNum)
			return 501; // 
	}
	...
	if( itA != _DenyAccounts.end() ) // insert
	{
		if(itA->second->times >= CGlobeSetup::GetSetup()->m_dwCardRetryNum)
			return 502; // 
	}
...
void CCardsHandler::CancelAllCards()
{
