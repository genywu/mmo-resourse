...
BOOL ValidateOperator::Init(LONG lASID)
{
	...
	c_pSecurityPool = (CnPool*)M_ALLOC(sizeof(CnPool));
	if (NULL == c_pSecurityPool)
		return FALSE;
	new(c_pSecurityPool)CnPool(32);
	c_pSecurityPool->Init(GetGame()->GetSecurityConnectionString());

	c_pFcmCnPool = (CnPool*)M_ALLOC(sizeof(CnPool));
	if (NULL == c_pFcmCnPool)
		return FALSE;
	new(c_pFcmCnPool)CnPool(32);
...
VOID ValidateOperator::Release(VOID)
{
	...
	if(NULL != c_pSecurityPool)
	{
		c_pSecurityPool->~CnPool();
		M_FREE(c_pSecurityPool, sizeof(CnPool));
	}
	if(NULL != c_pFcmCnPool)
	{
		c_pFcmCnPool->~CnPool();
		M_FREE(c_pFcmCnPool, sizeof(CnPool));
	}
}
...
{
	ValidateOperator* pRe = (ValidateOperator*)M_ALLOC(sizeof(ValidateOperator));
	if(NULL != pRe)
...
{
	if(NULL != ppRVO)
		M_FREE(*ppRVO, sizeof(ValidateOperator));
}
