...
OtherOperator* OtherOperator::CreateRejectOperator(const char* pCdKey, LONG lEndTime)
{
	...
	OtherOperator* pRe = (OtherOperator*)M_ALLOC(sizeof(OtherOperator));
	if(NULL != pRe)
	{
		pRe->m_dwDataType = eDT_RejectData;
		pRe->m_pData = M_ALLOC(sizeof(tagRejectData));
		if(NULL == pRe->m_pData)
		{
			M_FREE(pRe, sizeof(OtherOperator));
			return NULL;
		}
...
VOID OtherOperator::ReleaseOperator(OtherOperator **ppOperator)
{
	...
	case eDT_RejectData:
		{
			M_FREE((*ppOperator)->m_pData, sizeof(tagRejectData));
		}
		break;
	case eDT_LoginAndlogout:
		{
			M_FREE((*ppOperator)->m_pData, sizeof(tagLoginAndlogout));
		}
	    break;
	default:
	    return;
	...
	M_FREE(*ppOperator, sizeof(OtherOperator));
}
...
BOOL OtherOperator::UserSecurityOperate(const char* pCdKey, DWORD dwIp, LONG &lWorkFlag, char arrSecurityCard[])
{
	...
	catch(_com_error &e)
	{
		...
		{
			AddErrorLogText("UserSecurityOperator Repair CnPtr failed, end!");
			ValidateOperator::GetSecurityCnPool()->ReturnCnPtr(cnPtr);
			return FALSE;
		}
	}
		...
		if(0 == lDbRe)
		{
			...
			if(2 == lWorkFlag || 3 == lWorkFlag)
			{
				...
				if (varOutCard.vt == (VT_ARRAY | VT_UI1))
				{
					...
					{
						AddErrorLogText("user[%s]'s card data is bad!!!", pCdKey);
						bRe = FALSE;
					}
				}
				else
...
LPVOID OtherOperator::RejectAccountFromDB(LPVOID pThis)
{
	...
	{
		AddErrorLogText("Get CnPtr failed, end!");
...
OtherOperator* OtherOperator::CreateLoginAndLogout(const char* pCdKey, LONG lLSID, eLoginAndLogoutType eLogin)
{
	...
	if(NULL != pRe)
	{
		...
		pRe->m_pData = M_ALLOC(sizeof(tagLoginAndlogout));
		if(NULL == pRe->m_pData)
		{
			M_FREE(pRe, sizeof(OtherOperator));
			return NULL;
...
LONG OtherOperator::UserLoginAndLogout(const char* pCdKey, LONG lLSID, DWORD dwLogin, set<DWORD> &setLSID)
{
	...
	try
	{
		...
		paraCurrTime = CmdPtr->CreateParameter(L"curr_time", adInteger, adParamInput, 32, dwCurrTime);
	...
	catch(_com_error &e)
	{
		...
		{
			AddErrorLogText("UserLoginAndLogout Repair CnPtr failed, end!");
			ValidateOperator::GetFcmCnPool()->ReturnCnPtr(cnPtr);
			return eULALR_RunLose;
		}
	}
