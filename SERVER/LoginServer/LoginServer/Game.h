...
//! 用于输出调试信息
#define OUT_VALIDATE_FCM_INFO_20090720
#define SHOW_LOGIN_MSG_20090817

#include "../public/UseMemManageLS.h"
#include "..\AppLogin\AccountClient.h"
#include "..\AppLogin\CSdoaValidate.h"
#include "..\AppLogin\CSdoaFcm.h"
#include "..\AppLogin\LoaclAccountValidate.h"
#include "..\nets\netlogin\message.h"
#include "..\nets\netlogin\AinMessage.h"
#include "..\AppLogin\LoginManager.h"
#include "..\AppLogin\COnCmdConnect.h"
#include "../public/greta/regexpr2.h"
...
public:
	...
	void						ReLoadNoQueue(VOID){m_bLoadNoQueueCDKeyList = true;}
...
public:
	...
	VOID		SetWsReadyState(LONG lWorldNumber, BOOL bIsReady)
	{
		...
		if(bIsReady)
			AddLogText("WorldServer[%d] is ready!!!", lWorldNumber);
		else
		{
			AddLogText("Wait WorldServer[%d] reset!!!", lWorldNumber);
...
public:
	...
	void OnNetClientClose(const char* szCdkey);
...
public:
	...
	VOID			ReLoadLoginSetup(VOID);
	VOID			ReLoadIpLimitSetup(VOID);
...
public:
	void	OpenCloseInfo(void){bOutInfo = !bOutInfo;AddLogText("登陆消息输出【%s】", ((bOutInfo) ? "开启" : "关闭"));}
