...
: CBaseDBEntity()
{
	...
	memset(m_szGoodsID, 0, sizeof(m_szGoodsID));
...
CDBGoods::CDBGoods(const CGUID& id)
...
// virtual interfaces
void CDBGoods::ProcessMsg(DB_OPERATION_TYPE op, long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	switch(op)
	{
	case DB_OPER_SAVE:
...
// 从Player消息中读取数据
void CDBGoods::ReadDataFromMsgForPlayer(BYTE* msgBuf, long& msgBufPos)
{

}
void CDBGoods::Save(long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	...
	for(int i=0; i<(int)attrNum; i++)
	{
		...
		else
		{
			...
			std::map<string, CDataEntityManager*>::iterator itr = GetGame()->GetDBEntityManager()->GetObjAttrDef().find(string("goods"));
...
void CDBGoods::AddEntityDataToMsg(DB_OPERATION_TYPE opType, vector<BYTE>& pBA)
{
...
// 检查更新属性里是否有指定表名的属性名
bool CDBGoods::FindTableUpdateFlag(const string& tableName)
{
