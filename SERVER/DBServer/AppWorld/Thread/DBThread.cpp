	...
	if(tempSaveThreadData)
	{
		...
		if(pBaseEntity)
		{
			...
			if(GetGame()->GetSetup()->dwViewDbOperDeltaTime <= lviewTime)
			{
				char szGuid[128];
				pBaseEntity->GetGUID().tostring(szGuid);
				AddLogText("线程ID[%d]:耗时:%d(ms),操作类型[%d],对象类型[%s],对象ID[%s].", 
		...
		{
			AddLogText("ProcData entity is Null!");
			SAFE_DELETE(tempSaveThreadData);
			return true;
		}
	...
	case DB_OPER_LOAD_NOCACHE:
		{
			...
			CMessage::AsyWriteFile(GetGame()->GetStatckFileName(),szOutPutStr);
#endif
			hRet = CRsEntity::LoadAllEntityGroup(pRoot, GetAdoBasePtr()->GetConnPtr(), pRoot->GetCurDbOperType());
#ifdef _DEBUG_SM_INFO_
	...
	case DB_OPER_LOAD_NOCACHE:
		{
			...
			CMessage::AsyWriteFile(GetGame()->GetStatckFileName(),szOutPutStr);
#endif
			hRet = CRsEntity::LoadEntitys(szSql, strComFlag, entityMap, GetAdoBasePtr()->GetConnPtr(), dbOperType);
#ifdef _DEBUG_SM_INFO_
