...
#include "..\AppServer/Mail/MailManager.h"
...
g_MailManager(NULL)
{
	g_RgnManager		= MP_NEW CRgnManager;//new CRgnManager;
	g_MailManager		= MP_NEW CMailManager;
...
bool   GameManager::Initialize(unsigned long nMaxFreeGuidEventNum)
{
	...
	int i = 0;
	for(;i < nMaxFreeGuidEventNum;i++)
	{
		CGUIDEvent* pGuidEvent = MP_NEW CGUIDEvent(this);
...
void   GameManager::Release()
{
	...
	MP_DELETE(g_RgnManager);
	g_RgnManager = NULL;
	MP_DELETE(g_MailManager);
	g_MailManager = NULL;
...
CBaseObject	*GameManager::FindObject(OBJECT_TYPE objecttype,const CGUID& guid)
{
	...
	case TYPE_PET:			// 宠物
		{
			pObj = (CBaseObject	*)GetInst(CPetCtrl).GetPet(guid);
		}
		break;
