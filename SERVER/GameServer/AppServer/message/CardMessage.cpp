	...
	case MSG_C2S_CARD_SELL_REQUEST:		// 客户端请求寄售点卡
		{
			...
			//// 仓库金额超过20乙，不能寄售
			//if( pPlayer->GetDepotMoney() >= CGlobeSetup::GetSetup()->dwGoldCoinLimit )
			//{
			...
			{
			case 0: // 6 hours
				if( pPlayer->GetMoney() < CGlobeSetup::GetSetup()->m_dwCard6HoursChargePrice ) {
					CMessage msg(MSG_S2C_CARD_SELL_RESPONSE);
			...
			case 1: // 12 hours
				if( pPlayer->GetMoney() < CGlobeSetup::GetSetup()->m_dwCard12HoursChargePrice ) {
					CMessage msg(MSG_S2C_CARD_SELL_RESPONSE);
			...
			default: // 24 hours and other
				if( pPlayer->GetMoney() < CGlobeSetup::GetSetup()->m_dwCard24HoursChargePrice ) {
					CMessage msg(MSG_S2C_CARD_SELL_RESPONSE);
			...
			// 手续费够，检查寄售金额是否在合法区间内
			if( CardSellPrice < CGlobeSetup::GetSetup()->m_dwCardLowerLimitPrice ||
				CardSellPrice > CGlobeSetup::GetSetup()->m_dwCardUpperLimitPrice )
			{
				CMessage msg(MSG_S2C_CARD_SELL_RESPONSE);
	...
	case MSG_C2S_CARD_SALECOSTS_REQUEST: // 请求费用。
		{
			...
			msg.Add(CGlobeSetup::GetSetup()->m_dwCard6HoursChargePrice);
			msg.Add(CGlobeSetup::GetSetup()->m_dwCard12HoursChargePrice);
			msg.Add(CGlobeSetup::GetSetup()->m_dwCard24HoursChargePrice);
			msg.SendToPlayer(pPlayer -> GetExID());
		}
		break;
