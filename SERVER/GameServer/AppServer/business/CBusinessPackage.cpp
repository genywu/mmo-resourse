...
namespace Business
{
	...
	bool CPackage::IsValid() const
	{
		...
		return m_pContainer->GetVolume() > 0 && m_pGoods != NULL &&
			m_pGoods->GetAddonPropertyValues( GAP_GOODS_MAXIMUM_DURABILITY, 2 ) > 0;
	}
	...
	LONG CPackage::GetCurDurability()
	{
		...
		return m_pGoods->GetAddonPropertyValues( GAP_GOODS_MAXIMUM_DURABILITY, 2 );
	}
	...
	DWORD CPackage::UpdateEndure( DWORD dwProfit )
	{
		if( m_pGoods == NULL )
		{
			BUSINESS_LOG_ERROR( "Busiess package GOODS does not exist!" );
			return 0;
		}

		DWORD dwSubEndure = dwProfit * GetInst( CBusinessManager ).GetSetup().fEndureQuo;
		DWORD dwCurEndure = GetCurDurability();
		DWORD dwRet = dwCurEndure >= dwSubEndure ? dwCurEndure - dwSubEndure : 1;
		m_pGoods->ChangeAttribute( GAP_GOODS_MAXIMUM_DURABILITY, dwRet, 2, true );
	...
	{
		if(m_pGoods && m_pContainer)		
			return m_pContainer->GetGoodsMap()->size();
	...
	BOOL CPackage::CanSwap( CGoods *pPackGoods )
	{
		...
		if( m_pGoods != NULL )
		{
			...
			if( lAmountLimit < GetCurGoodsAmount() )
			{
				return FALSE;
			}
		}
	...
	void CPackage::DecodeFromDB( DBReadSet &db )
	{
		...
		if( valid == 1 )
		{
			...
			if( pGoods == NULL )
			{
				BUSINESS_LOG_WARNING( "Decode pakcage goods failed." );
			}
