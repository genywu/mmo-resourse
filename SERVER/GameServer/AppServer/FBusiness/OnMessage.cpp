...
namespace FBusiness
{
	...
	{
		char strNotify[1024];
		_snprintf( strNotify, 1024, CStringReading::LoadString( NPC_SHOP_STRING, ENT_FATAL_ERROR ), err ); 
#ifdef _ENABLE_NOTIFY
		pPlayer->SendNotifyMessage( strNotify );
#else
		PutoutLog( LOG_FILE, LT_ERROR, "Fatal error [%d] in Player [%s] 's trading.", err, 
			pPlayer->GetName() );
#endif
	}
	...
	{
		pPlayer->SendNotifyMessage( CStringReading::LoadString( NPC_SHOP_STRING, type ) );
	}
	...
	{
		GoodsHelper::GoodsListType BuyGoodsList;
		...
		{
			// invalid operation
			PutoutLog( LOG_FILE, LT_WARNING, "Player [%s] no goods to buy, ignore this operation.",
				pPlayer->GetName() );
			return ;
		}

		// 购买数量超出最大限制
		if( lCount > GetInst( CConfiger ).GetGlobalCfg().lPlayerBuyCount )
		{
			NotifyNormalError( pPlayer, ENT_LIMIT_COUNT );
			return ;
		} 
		...
		for( long i = 0; i < lCount; ++ i )
		{
			...
			{
				// 非法操作
				PutoutLog( LOG_FILE, LT_WARNING, "Player [%s] buy goods [%d] is not in NPC sell list.",
					pPlayer->GetName(), goods.lIndex );
				return ;
			}
		}
		
		// 限制拥有数量检查
		if( !GoodsHelper::CanPlayerBuy( pPlayer, BuyGoodsList ) )
		{
			NotifyNormalError( pPlayer, ENT_LIMIT_COUNT );
			return ;
		}
		...
		{
			const GoodsHelper::GoodsInfo &info = BuyGoodsList[i];
		...
		if( bRet )
		{
			...
			for( size_t i = 0; i < addResult.size(); ++ i )
			{
				...
				{
					//记录日志
					GetGameLogInterface()->logT030_goods_log_npc( pPlayer, dwGoodsIndex,
	...
	void CShop::DoPlayerSell( CPlayer *pPlayer, const CCachedSellData::SellGoodsListT &sell_list )
	{
		...
		for( size_t i = 0; i < delResult.size(); ++ i )
		{
			...
			{
				// 删除物品日志
				GetGameLogInterface()->logT030_goods_log_npc( pPlayer, pGoods->GetBasePropertiesIndex(), 
