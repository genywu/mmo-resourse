...
class CRgnManager : public BaseMemObj
{
public:
	// 个人房屋副本物品类
	struct PHGoods
	...
	struct tagDupRgnData : public BaseMemObj
	{
	public:
		tagDupRgnData(void);
		~tagDupRgnData(void);
	...
	long CreateRgnMsg(CScript* pScript/*挂起的脚本ID*/, long templateRgnID/*模板Region的ID*/, eRgnType rgnType, const CGUID& ownerID=NULL_GUID/*NORMAL类型则ID为空*/, const char* szRetVarName=NULL/*异步返回值变量名*/, const char* szRetRgnIDName=NULL/*异步返回值变量名*/);
	...
	//!					数据块个数
	long				GetDupRgnDataNum(void) { return m_mapDupRgnData.size(); }
