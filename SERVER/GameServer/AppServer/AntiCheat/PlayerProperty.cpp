...
namespace AntiCheat
{
	...
	bool CPlayerProperty::OnEnter()
	{
		...
		if( m_pOwner == NULL )
		{
			PutoutLog( LOG_FILE, LT_ERROR, "Invalid player [%s], startup failed.", 
				GuidToString( m_OwnerID ) );
			return true;
		}
	...
	bool CPlayerProperty::StartCheck()
	{
		...
		if( m_pOwner == NULL )
		{
			PutoutLog( LOG_FILE, LT_ERROR, "Invalid player [%s].", GuidToString( m_OwnerID ) );
			return 0;
		}
		// 启动定时器
		FILTER_GM( m_pOwner, true );
	...
	bool CPlayerProperty::CheckSendQuestion()
	{
		...
		if (m_cachedProper.saved.nActFactor >= GetInst(CConfiger).GetSystem().ActivityMax)
		{
			...
			if( pRegion == NULL )
			{
				PutoutLog( LOG_FILE, LT_WARNING, "The region for [%s] does not exist.", GuidToString( m_OwnerID ) );
				return false;
			}
			if( pRegion->GetACRgnType() == CRegion::ACRT_FIELD )
			{
				// 是否超过答题间隔,现在时间减去上次答题时间
				if ((time(NULL) - m_cachedProper.LastAnswerTime ) > GetInst(CConfiger).GetSystem().AnswerInterval)
				{
					// 是否在Pk状态
					if( m_cachedProper.nAntiCheatPK > 0 )
					{
						if ((time(NULL) - m_cachedProper.LastAnswerTime ) > GetInst(CConfiger).GetSystem().PkTimeInterval)
						{
							return true;
						}
					}
	...
	void CPlayerProperty::OnAntiCheatAnswer(int sel )
	{
		...
		if( m_pOwner == NULL )
		{
			PutoutLog( LOG_FILE, LT_ERROR, "Invalid player when received message from client [%s].", 
				GuidToString( m_OwnerID ) );
			return ;
		}
		FILTER_GM( m_pOwner, NULL_RET );
		...
		if (!m_CheckFinish)
		{
			PutoutLog( LOG_FILE, LT_WARNING, "Multiple messages responsed from client [%s], ignore it.", 
				m_pOwner->GetName() );
			return ;
		}
		m_CheckFinish = false;
		...
		// 修改上次答题
		m_cachedProper.LastAnswerTime = time(NULL);
	...
	bool CPlayerProperty::ReginsterAntiCheatTimer( bool bFirst)
	{
		...
		if (!bFirst)
		{
			...
			tagTimerVar* pTimvar =  CBaseObject::CreateTimerVar(41);
		...
		else
		{
			// 增加,切服或者第一登录
			int lTime = 0; //60 * 1000;
			tagTimerVar* pTimvar =  CBaseObject::CreateTimerVar(42);
			pTimvar->TimerType = (eChildObjType)ANTICHEAT;
			m_lCheckTimer= GameManager::GetInstance()->Schedule(this,(void*)pTimvar,timeGetTime() + lTime,0);
			return true;
		}
	...
	bool CPlayerProperty::RegisterPkTimer()
	{
		...
		tagTimerVar* pTimvar =  CBaseObject::CreateTimerVar(43);
	...
	bool CPlayerProperty::RegisterAtkActTimer()
	{
		...
		tagTimerVar *pTimvar =  CBaseObject::CreateTimerVar(44);
	...
	bool CPlayerProperty::RegisterAnswerTimeOutTimer()
	{
		...
		tagTimerVar* pTimvar =  CBaseObject::CreateTimerVar(45);
	...
	long CPlayerProperty::OnTimeOut(DWORD timerid, DWORD curtime, const void *var )
	{
		...
		{
			CancelTimers();
			PutoutLog( LOG_FILE, LT_ERROR, "The player [%s] is invalid.", GuidToString( m_OwnerID ) );
			return 0;
		}
		...
		default:
			PutoutLog( LOG_FILE, LT_WARNING, "Unknown timer [%d].", pTimervar->TimerType );
			break;	
		}	
		return 0;
	}
	...
	bool CPlayerProperty::IsKickPlayer()
	{
		...
		else
		{
			...
			GetGameLogInterface()->logT900_anticheat_player_log(m_pOwner, LOG_LOGIN_TYPE,
	...
	void CPlayerProperty::KickPlayer()
	{
		...
		FILTER_GM( m_pOwner, NULL_RET );
		FILTER_LEVEL( m_pOwner->GetLevel() , NULL_RET );
		GetGameLogInterface()->logT900_anticheat_player_log(m_pOwner,LOG_KICK_TYPE, 
			m_cachedProper.saved.nAccWrongTimes, m_cachedProper.saved.nCurWrongTimes, m_cachedProper.saved.nBonusTimes );
	...
	{
		GetGameLogInterface()->logT901_anticheat_answer_log( m_pOwner, 
