...
, m_pCMsPacketFactory   (pCMsPacketFactory)
{
    ...
    AssertEx(AL_NORMAL, m_pCMsPacketFactory, _T("封包工厂不能为空"));
    if (szIP)
    {
#ifdef UNICODE
        ::WideCharToMultiByte(
    ...
    {
        AssertEx(AL_NORMAL, szIP, _T("IP地址不能为空"));
    }
    ...
    {
        AssertEx(AL_CATAST, m_pMsServerSocket, _T("内存不足"));
    }
    ...
    if (m_pCMsPoolManager && m_ppMsClientSocket)
    {
        FOR_x_TO(i, dwMaxClientCount)
        {
            ...
            {
                AssertEx(AL_CATAST, m_ppMsClientSocket[i], _T("内存不足"));
            }
        }
    }
    else
    {
        AssertEx(AL_CATAST, m_pCMsPoolManager, _T("内存不足"));
        AssertEx(AL_CATAST, m_ppMsClientSocket, _T("内存不足"));
    }
...
VOID CMsServerNetManager::LogServerNet(TCHAR* szLog)
{
    if (szLog)
    {
        m_MsServerNetLog << L_LOCK_A << L_TIME;
        m_MsServerNetLog.Logf(_T("[服务器网络管理器]\t%s"), szLog);
        m_MsServerNetLog << L_NEWLINE << L_UNLOCK_A;
    }
}
...
BOOL CMsServerNetManager::Accept(VOID)
{
    ...
    if (AcceptSocket != INVALID_SOCKET)
    {
        ...
        AssertEx(AL_NORMAL, bRet, _T("套接字绑定完成端口失败"));

        this->LogServerNet(_T("通知有新的连接进入!"));
        m_ppMsClientSocket[dwNextIndex]->OnEnter();
...
// 空闲处理
VOID CMsServerNetManager::OnThreadFreeWork(VOID)
{
    // 无事通知
...
// 停止之前
BOOL CMsServerNetManager::ForeStop(VOID)
{
    return m_lpCMsIocpManager->Stop(TRUE);
}

// 消息处理
VOID CMsServerNetManager::OnThreadMsgExecute(LPMSG lpMsg)
{
    // 消息通知
    ...
    default:
        {
            ...
            AssertEx(AL_NORMAL, FALSE, _T("未知IOCP消息"));
        }break;
    }
}
    ...
    default:
        {
            ...
            AssertEx(AL_NORMAL, FALSE, _T("严重错误逻辑"));
        }
    }
}