...
CONST TCHAR *WINDOW_CLASS_NAME = _T("HGE__WNDCLASS");
...
BOOL CALL HGE_Impl::System_Initiate()
{
    ...
    System_Log(_T("HGE Started..\n"));

    System_Log(_T("HGE version: %X.%X"), HGE_VERSION>>8, HGE_VERSION & 0xFF);
    GetLocalTime(&tm);
    System_Log(_T("Date: %02d.%02d.%d, %02d:%02d:%02d\n"), tm.wDay, tm.wMonth, tm.wYear, tm.wHour, tm.wMinute, tm.wSecond);

    System_Log(_T("Application: %s"),szWinTitle);
    os_ver.dwOSVersionInfoSize=sizeof(os_ver);
    GetVersionEx(&os_ver);
    System_Log(_T("OS: Windows %ld.%ld.%ld"),os_ver.dwMajorVersion,os_ver.dwMinorVersion,os_ver.dwBuildNumber);

    GlobalMemoryStatus(&mem_st);
    System_Log(_T("Memory: %ldK total, %ldK free\n"),mem_st.dwTotalPhys/1024L,mem_st.dwAvailPhys/1024L);
    ...
    if (!RegisterClass(&winclass)) {
        _PostError(_T("Can't register window class"));
        return FALSE;
    }
    ...
    if (!hwnd)
    {
        _PostError(_T("Can't create window"));
        return FALSE;
    }
...
BOOL CALL HGE_Impl::System_Start()
{
    ...
    if(!hwnd)
    {
        _PostError(_T("System_Start: System_Initiate wasn't called"));
        return FALSE;
    }

    if(!procFrameFunc) {
        _PostError(_T("System_Start: No frame function defined"));
        return FALSE;
    }
...
VOID CALL HGE_Impl::System_SetStateString(hgeStringState state, CONST TCHAR *value)
{
    FILE *hf;
    ...
    switch(state)
    {
        case HGE_ICON:            szIcon=value;
                                ...
                                break;
        case HGE_TITLE:            _tcscpy(szWinTitle,value);
                                if(pHGE->hwnd) SetWindowText(pHGE->hwnd, szWinTitle);
                                break;
        case HGE_INIFILE:        if(value) _tcscpy(szIniFile,Resource_MakePath(value));
                                else szIniFile[0]=0;
                                break;
        case HGE_LOGFILE:        if(value)
                                {
                                    _tcscpy(szLogFile,Resource_MakePath(value));
                                    hf=_tfopen(szLogFile, _T("w"));
                                    if(!hf) szLogFile[0]=0;
                                    else fclose(hf);
                                }
                                else szLogFile[0]=0;
...
CONST TCHAR* CALL HGE_Impl::System_GetStateString(hgeStringState state) {
    switch(state) {
...
TCHAR* CALL HGE_Impl::System_GetErrorMessage()
{
    return szError;
}

VOID CALL HGE_Impl::System_Log(CONST TCHAR *szFormat, ...)
{
    FILE *hf = NULL;
    va_list ap;
    ...
    hf = _tfopen(szLogFile, _T("a"));
    if(!hf) return;

    va_start(ap, szFormat);
    _vftprintf(hf, szFormat, ap);
    va_end(ap);
...
BOOL CALL HGE_Impl::System_Launch(CONST TCHAR *url)
{
    if((ULONG_PTR)ShellExecute(pHGE->hwnd, NULL, url, NULL, NULL, SW_SHOWMAXIMIZED)>32) return TRUE;
    else return FALSE;
}

VOID CALL HGE_Impl::System_Snapshot(CONST TCHAR *filename)
{
    LPDIRECT3DSURFACE8 pSurf;
    TCHAR *shotname, tempname[_MAX_PATH];
    INT i;

    if(!filename)
    {
        i=0;
        shotname=Resource_EnumFiles(_T("shot???.bmp"));
        while(shotname)
        {
            i++;
            shotname=Resource_EnumFiles();
        }
        _stprintf(tempname, _T("shot%03d.bmp"), i);
...
HGE_Impl::HGE_Impl()
{
    ...
    //INT i;
    //for(i=_tcslen(szAppPath)-1; i>0; i--) if(szAppPath[i]=='\\') break;
    //szAppPath[i+1]=0;
}

VOID HGE_Impl::_PostError(TCHAR *error)
{
    System_Log(error);
    _tcscpy(szError,error);
}
