...
enum GAMEDATA_INDEX
{
	...
	//nParam = (char*) 指向一个缓冲区的指针，其空间不少于1024字节。
	...
	GDI_TEAM_REFUSE_INVITE_STATUS,//获取拒绝邀请的状态
	//Return = (int)(bool)bEnableRefuse 为真值表示拒绝状态生效，否则表示不拒绝。

	GDI_TEAM_TOJOIN_LIST,		//获取主角所在队伍加入列表
	...
	GDI_NPC_RES,				//NPC资源
	//In GetGameData: uParam = (RoleBaseInfo*)
	//In FreeGameData: uParam = (KNpcRes *)
	...
	GDI_SCENE_TIME_INFO,					//当前主角所处的地域时间环境
...
enum GAMEDATA_CHANGED_NOTIFY_INDEX
{
	...
	//uParam = (const char* )pContent 存储任务提示信息的缓冲区，（字符串为控制符已经编码的字符串）
	//nParam = pContent内有效内容的长度(单位：字节)，（不包括字符串结尾符号）长度最大一定不超过255字节
...
enum GAMEOPERATION_INDEX
{
	...
	//===========组队相关==================
	GOI_TEAM_COLLECT_NEARBY_LIST,//获取周围队伍的列表

	GOI_TRAM_REFUSE_INVITE,		//拒绝别人邀请自己加入队伍
	//nParam = (int)(bool)bEnableRefuse 为真值表示拒绝状态生效，否则表示不拒绝。

	GOI_TEAM_APPLY,				//申请加入她人队伍
	//uParam = (KUiTeamItem*)	要申请加入的队伍的信息

	GOI_TEAM_NEW,				//新组队伍
	//uParam = (const char*)pTeamName -> 新组队伍的名称

	GOI_TEAM_APPOINT,			//任命队长，只有队长调用才有效果
	//uParam = (KUiPlayerItem*)pPlayer -> 新队长的信息
	//			KUiPlayerItem::nData = 0

	GOI_TEAM_INVITE,			//邀请别人加入队伍，只有队长调用才有效果
	//uParam = (KUiPlayerItem*)pPlayer -> 要邀请的人的信息
	//			KUiPlayerItem::nData = 0

	GOI_TEAM_KICK,				//踢除队里的一个队员，只有队长调用才有效果
	//uParam = (KUiPlayerItem*)pPlayer -> 要踢除的队员的信息
	//			KUiPlayerItem::nData = 0

	GOI_TEAM_APPROVE,			//批准他人加入队伍，只有队长调用才有效果
	...
	GOI_TEAM_CLOSE,				//关闭组队，只有队长调用才有效果
	//nParam = (int)(bool)bClose 为真值表示处于关闭状态，否则表示不处于关闭状态

	GOI_TEAM_INVITE_RESPONSE,	//对组队邀请的回复
...
//	GOI_DROP_ITEM,				//放置物品到游戏窗口
	...
	GOI_IS_SCENE_MAP_SHOWING,	//设置场景的小地图是否显示的状态
	...
	GOI_PAINT_SCENE_MAP,		//绘制场景的小地图
	//nParam = (KUiRegion*) h 与 v 表示绘制起始点在屏幕上横纵坐标坐标（单位：像素点）,Width与Height值无意义
	//nParam = (KUiRegion*) pMapRegion 绘制地图的哪个范围，只在浏览小地图绘制时有效
	//Return = (int)(bool)bHaveMap 返回值表示当前场景是否有小地图
...
struct IClientCallback
{
	...
	virtual void ChannelMessageArrival(DWORD nChannelID, char* szSendName, const char* pMsgBuff, unsigned short nMsgLength) = 0;
	virtual void MSNMessageArrival(char* szSourceName, char* szSendName, const char* pMsgBuff, unsigned short nMsgLength) = 0;
	virtual void NotifyChannelID(char* ChannelName, DWORD channelid) = 0;
};
...
struct _declspec (novtable) iCoreShell
{
	...
	//找x, y所指的Player
	virtual int FindSelectPlayer(int x, int y, void* pReturn) = 0;
	//找Name指定的Player
	virtual int FindSpecialPlayer(char* Name, void* pReturn) = 0;
	...
	virtual int  FreeGameData(unsigned int uDataId, unsigned int uParam, int nParam) = 0;
	//绘制游戏对象
