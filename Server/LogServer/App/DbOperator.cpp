...
MultiObjPool<DbOperator, LockOfWin32> DbOperator::c_MultiObjPool;
#ifdef APEX_TEST
...
DbOperator* DbOperator::CreateDbOperator(LONG lDbID)
{
	...
	if(c_mapExecuteData.end() == ite)
		return NULL;
	DbOperator *pDbOperator = c_MultiObjPool.Multi_Alloc(_FILE_AND_LINE_);
...
{
	if(NULL == ppDbOperator || NULL == *ppDbOperator)
		return;
	(*ppDbOperator)->~DbOperator();
	c_MultiObjPool.Multi_Free(*ppDbOperator);
	*ppDbOperator = NULL;
}
...
LPVOID DbOperator::ExecuteThread(LPVOID pThis)
{
	...
	catch(_com_error &e)
	{
		...
		{
			AddErrLog("Repair CnPtr failed, end!");
			pDbOperator->m_pExecuteData->pCnPool->ReturnCnPtr(cnPtr);
			return pThis;
		}	
	}
