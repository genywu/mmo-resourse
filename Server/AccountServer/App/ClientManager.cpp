...
BOOL ClientManager::OnClientLogin(DWORD dwIP, DWORD dwNo, DWORD dwSocket, DWORD &dwRetrunInfo)
{
	...
	if(m_mapClientInfo.end() == iteInfo)
	{
        ...
        Log4c::Warn(ROOT_MODULE,FormatText("AS_CLIMGR_2",dwNo));//非法客户端登陆，ID[%d]没有在配置文件中找到！
	...
	if(iteInfo->second._dwIP != dwIP)
	{
        ...
        Log4c::Warn(ROOT_MODULE,FormatText("AS_CLIMGR_0",dwNo));//客户端[%d]的IP跟配置有出入，拒绝登陆！
		return FALSE;
	}
	if(iteInfo->second._dwLoginState == eLS_OnLine)
	{
        ...
        Log4c::Warn(ROOT_MODULE,FormatText("AS_CLIMGR_1",dwNo));//客户端[%d]已经登陆了，拒绝多次登陆！
		return FALSE;
	}
...
BOOL ClientManager::OnClientLogout(DWORD dwSocket)
{
	...
	if(m_mapClientInfo.end() == iteInfo)
	{
		Log4c::Error(ROOT_MODULE,"未配置的客户端出现在登陆列表中[IP]！！！！");
...
BOOL ClientManager::LoadClientSetup(const char *pFilename)
{
	...
	if( !stream.is_open() )
	{
		Log4c::Error(ROOT_MODULE,"%s文件打开失败！", pFilename);
		return FALSE;
	}
	...
	do 
	{
		...
		{
			Log4c::Error(ROOT_MODULE,"错误的客户端类型设置“%s”！", strType.c_str());
			bRe = FALSE;
			continue;
		}
		...
		if(m_mapClientInfo.end() != iteInfo)
		{
			Log4c::Error(ROOT_MODULE,"客户端编号重复“%d”，覆盖之前信息！", tmpClientInfo._dwNo);
