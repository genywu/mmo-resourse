...
{
	CGUID	leafGuid;           // 叶子GUID
    CGUID	rootGuid;           // 根GUID
...
private:
	...
	CRITICAL_SECTION	m_LoadCriticalSection;      // SQL队列Load临界对象	
	CRITICAL_SECTION	m_SaveCriticalSection;      // SQL队列Save临界对象	
	CRITICAL_SECTION	m_MailCriticalSection;      // 邮件队列临界对象	
	CRITICAL_SECTION	m_RetCriticalSection;       // 线程返回队列临界对象	
	CRITICAL_SECTION	m_RetMailCriticalSection;   // 邮件线程返回队列临界对象
	...
	list<CGUID>		m_listDirtyEntityGuid;      // 记录有变化的对象
	list<string>	m_listDirtyEntityName;

	
	list<CGUID>		m_listLoadBeginEntityGuid;  // 记录正在被线程操作的对象
	list<CGUID>		m_listSaveBeginEntityGuid;

	
	list<ThreadData*> m_listLoadThreadOperData;     // 投递到线程的操作数据
	list<ThreadData*> m_listSaveThreadOperData;	
	list<ThreadData*> m_listThreadRetData;          // 读取或存储过程返回数据

	
	static HANDLE		m_hLoadThreadEvent;     // Load事件句柄	
	static HANDLE		m_hSaveThreadEvent;     // Save事件句柄	
	static HANDLE		m_hMailThreadEvent;     // 邮件线程事件句柄
	...
	list<ThreadData*> m_listMailThreadData;     // 邮件操作完全独立	
	list<ThreadData*> m_listMailThreadRetData;  // 邮件操作返回数据
