...
void CDBMail::ProcessMsg(DB_OPERATION_TYPE op, long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	...
	case DB_OPER_SAVE:
		{
			SaveThreadData* saveData = new SaveThreadData(GetExID(), DB_MAIL);
			// 需要记录一个时间戳,在存盘线程做
			Save(msgid, msgBuf, msgBufPos, bufSize);
			saveData->saveObj->objGUID = GetExID();
			saveData->saveObj->objType = DB_MAIL;
			saveData->saveObj->objNum  = 1;
			*(CDBMail*)(saveData->dbObj) = *this;
	...
	case DB_OPER_LOAD:
		{
			if(b_IsFirstCreated)
			{
				LoadThreadData* loadData = new LoadThreadData(GetExID(), DB_MAIL);
				loadData->loadObj->objGUID = GetExID();
				loadData->loadObj->objType = DB_MAIL;
				loadData->loadObj->objNum  = 1;
				*(CDBMail*)(loadData->dbObj) = *this;
	...
	case DB_OPER_DELETE:
		{
			...
			saveData->saveObj->objGUID = GetExID();
			saveData->saveObj->objType = DB_MAIL;
			saveData->saveObj->objNum  = -1; // 如果个数为负数则负了多少就表示删除多少个对象
			*(CDBMail*)(saveData->dbObj) = *this;
			GetGame()->GetDBEntityManager()->PushSaveThreadData(saveData);
		}
		break;
...
void CDBMail::Save(long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	// 解析消息
	...
	if(goodsGroupFlag == 1)
	{
		m_pGoodsGroupPtr->Save(0, msgBuf, msgBufPos, bufSize);
	}
}
