...
void CArea::AddObject(CBaseObject* pObject)
{
	if( pObject )
	{
		...
		case TYPE_PLAYER:
			{
				long lOldPlayerSize = (long)m_setPlayers.size();
		...
		default:
			{
				Log4c::Warn(ROOT_MODULE, GetText("GS_AREA_1"));
			}
			break;
		}
	}
...
eGoodsPickValue CArea::CanPickUpGoods( CPlayer* pPlayer, CGoods* pGoods )
{
	...
	if( pPlayer && pGoods )
	{
		...
		if( it != m_mGoodsProtection.end() && timeGetTime()< it->second.dwTimeStamp+ GlobalSetup::GetSetup() -> dwGoodsProtectedTimer )
		{
...
void CArea::AddGoodsDelTime(CGoods *pGoods,tagGoodsProtection& gp)
{
	...
	DWORD dwGoodsDisappearTime	= GlobalSetup::GetSetup() -> dwGoodsDisappearTimer;
	if( ( pGoods -> GetAddonPropertyValues(GAP_PARTICULAR_ATTRIBUTE, 1) & 
		PA_DELETE_WHEN_DROP_DOWN ) == PA_DELETE_WHEN_DROP_DOWN )
	{
	...
	stDestObj* pDestObj = new stDestObj;//new stDestObj;
...
void CArea::OnGoodsDelTimeOut(stDestObj* pDestObj)
{
	...
	//CServerRegion * pRegion= dynamic_cast<CServerRegion*>(pGoods->GetFather());
	if (pServerRgn!=NULL)
	{
	...
	//GameManager::GetInstance()->FreeObject(pGoods);
}
...
void CArea::OnRefreshMonster(long lRefeashIndex)
{
	...
	//					if( bRet == false )
	//					{
	//						Log4c::Warn(ROOT_MODULE,"无法为Monster “%s” 找到合适的位置！", pMonster->GetName());
	//						return;
	//					}
...
void CArea::RemoveObject(CBaseObject* pObject)
{
	if( pObject )
	{
		...
		case TYPE_PLAYER:
			{
				long lOldPlayerNum = (long)m_setPlayers.size();				
...
BOOL CArea::FindShapes( LONG lType, vector<CShape*>& vOut )
{
	...
	if( pRegion )
	{
		...
		default:
			{
				Log4c::Warn(ROOT_MODULE, GetText("GS_AREA_0"));//the lType param error!
			}
			break;
		}
		bResult = TRUE;
...
{		
	...
	const tagTimerVar* pTimvar=	(const tagTimerVar*)var;
...
void CArea::WakeUp()
{
	...
	for( hsGUIDIt it = m_setActivedMonsters.begin(); it != m_setActivedMonsters.end(); it++ )
	{
		...
		{
			AI_EVENT_SENDER( pShape->GetAI() ).Wakeup();
			//pShape->GetAI()->WakeUp();
		}
	}
}