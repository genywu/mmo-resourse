...
void OnOrgasysMessage(CMessage* pMsg)
{
	...
	case MSG_W2S_ORGSYS_InitPlayerFacSys:
		{
			//GetOrganiCtrl()->SendInitFacDataToPlayer(pMsg);
		}
		break;
	...
	case MSG_C2S_ORGSYS_DisbandFaction_Request:
		...
		break;
	//case MSG_C2S_ORGSYS_UpgradeFaction_Reques: //! 升级行会请求
	//	{
	//	}
	//	break;
	...
	case MSG_C2S_Faction_SetChairman_Request: //! 传位请求
		...
		break;
    case MSG_C2S_Faction_FindFactionList:
        {
            CPlayer *pPlayer = pMsg->GetPlayer();
            if( !pPlayer ) break;
	...
	case MSG_C2S_QUEST_Complete:
		{
			...
			if(pPlayer /*&& pPlayer->GetQuestState(wQuestID) == eQS_NotComplete*/)
			{
				...
				Script::RunEnv env( pPlayer, NULL, pMsg->GetRegion() );
	...
	case MSG_C2S_QUEST_Accept:
		{
			...
			else
			{
				...
				if (bReAccept)
				{
					...
					GetInst( ScriptSys ).RunScript( env, strReAccept );
					break;
				}
				//接受个人列表中的任务
	...
	case MSG_C2S_QUEST_Disband:
		{
			...
			long wQuestID   = pMsg->GetLong();//>GetWord();
			CPlayer* pPlayer = pMsg->GetPlayer();
			if(pPlayer /*&& pPlayer->GetQuestState(wQuestID) == eQS_NotComplete*/)
			{
				const CQuest* pQuest = CQuestSystem::GetQuest((LONG)wQuestID);
				...
				if(strScript == NULL)	break;
                /// 增加脚本变量
                VariableList* m_VarList = pPlayer->GetVariableList();
                if( !m_VarList->HasVar( "$loc_QuestId" ) )
                {
                    m_VarList->AddVar( "$loc_QuestId", (double)wQuestID );
                }
                else
                {
                    m_VarList->SetVarValue( "$loc_QuestId", (double)wQuestID );
                }
				Script::RunEnv env( pPlayer, NULL, pMsg->GetRegion() );
				GetInst( ScriptSys ).RunScript( env, strScript );
			}
	...
	case MSG_C2S_MERCQUEST_Update:
		{
			if(pMsg->GetPlayer())
			{
				...
				//const char* strScript = CQuestSystem::GetMercUpdateScript();
				//if(strScript == NULL)	break;
				//stRunScript sScript;
				//sScript.pszFileName = (char*)strScript;
				//sScript.desShape = NULL;
				//sScript.pRegion =  pMsg->GetRegion();
				//sScript.srcShape	=pMsg->GetPlayer() ;
				//RunScript( &sScript, const_cast<CHAR*>( GetGame() -> GetScriptFileData(strScript) ));
				//重新获得上次列表的ID的任务

				//int FreshType= pPlayer->CanFreshMercQuest(ListID);//CanFresh
				//pMsg->GetPlayer()->AcquireMercQuestList(ListID);
			}
		}
		break;
	case MSG_W2S_SHARE_QUESTLIST_UPDATE:
		{
			CGUID teamGuid=NULL_GUID,PGuid;
			...
			if (pTeam)
			{
                ...
                pMsg->GetGUID( PGuid );
                pTeam->UpdateShareQuestList(&quest,true,PGuid);
			}
		}
		break;
				...
				if (0 == btype)
 				{
                    ...
                    if ( pPlayer->GetQuestState(quest.lQuestID) ==eQS_No || 
                         pPlayer->GetAttributeQuestIsShare( quest.lQuestID )==0)
                    {
                        /// 同时增加共享规则
                        break;
                    }
...
/*				CVariableList* stackVarList = CScript::GetStackVarListInst();
	...
	case MSG_C2S_Faction_ChangeJoinBattle:
        ...
        // 请求排行榜
    case MSG_C2S_QUEST_PLAYERRANKS:
        {
            CPlayer *pPlayer = pMsg->GetPlayer();
            if(pPlayer == NULL)
                return;
              
            pMsg->SetType(MSG_S2W_QUEST_PLAYERRANKS);
            pMsg->Send();
        }
        break;

        // 返回排行数据
    case MSG_W2S_QUEST_PLAYERRANKS:
        {
            CGUID guidPlayer;
            pMsg->GetGUID(guidPlayer);
            pMsg->SetType(MSG_S2C_ANSWER_PLAYERRANKS);
            pMsg->SendToPlayer(guidPlayer);
        }
        break;
	}
}