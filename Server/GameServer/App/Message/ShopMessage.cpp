...
bool CheckUnique(CPlayer* pPlayer,CGoodsBaseProperties* pProperty,DWORD dwTotalNum)
{
	...
	{
		SENDMSGBOX(pPlayer,AppFrame::GetText("27"));
		return false;
	}
	else
	{
		...
		if(dwTotalNum>1)
		{
			SENDMSGBOX(pPlayer,AppFrame::GetText("30"));
			return false;
		}
	}
	return true;
...
void OnShopMessage(CMessage* pMsg)
{
	...
	case MSG_C2S_SHOP_BUY:	//玩家请求在商店中购买一个物品		
		{					
			...
			if (pNpc)
			{
				...
				_snprintf(pszLog1,1024,"[MSG_C2S_SHOP_BUY][NPC NAME:%s][物品ID:%d][物品名:%s][物品数量:%d][ConID:%d][ConPos:%d]",
					pNpc->GetName(),dwGoodsId,GoodsSetup::QueryGoodsName(dwGoodsId),dwGoodsNum,dwContainerId,dwContainerPos);
		...
		//	DWORD dwNpcAreaId=pServerRegion->GetAreaId();
			...
			//物品检查
 			TradeList::tagTrade *pTrade = TradeList::GetTradeList(pNpc->GetOriginName());			
			if(!pTrade)
			{				
				CancelTrading( pPlayer );
				return;
			...
			bool bFound=false;
			TradeList::tagTradeData* pTradeData=NULL;
			...
			if (pTradeData->dwCreditId!=0 && pTrade->_creditType!=TradeList::CREDIT_NOT)
			{
				//区域声望商店
				if (pTrade->_creditType==TradeList::CREDIT_AREA)
				{
					if (pTradeData->dwCreditVal>pPlayer->GetCredit(pTradeData->dwCreditId))
					{
						SENDMSGBOX(pPlayer,AppFrame::GetText("29"));
						return;
					}
				}
				//组织声望商店
				else if (pTrade->_creditType==TradeList::CREDIT_ORGAN)
				{
					if( (long)pTradeData->dwCreditVal>pPlayer->GetOrganCredit(pTradeData->dwCreditId))
					{
						SENDMSGBOX(pPlayer,AppFrame::GetText("29"));
						return;
					}
			...
			if (dwContainerId!=0)
			{
				...
				//货币商店
				if(pTrade->tradeType==TradeList::ETT_MONEY || pTrade->tradeType==TradeList::ETT_SILVER)
				{
					...
					if(dwTotalPrice  > pPlayer->GetMoney())
					{
						...
						SENDMSGBOX(pPlayer,AppFrame::GetText("2"));
						return;
					}
					...
					if(dwTotalSilver  > pPlayer->GetSilver())
					{
						...
						SENDMSGBOX(pPlayer,AppFrame::GetText("14"));
						return;
					}
					...
					if(pPlayer->AddGoodsWithProtect(pGoods,FALSE))
					{
						...
						//玩家NPC商店买物品的日志记录												
						GetLogicLogInterface()->logT030_goods_log_npc(pPlayer,dwGoodsId,GoodsGuid,strGoodsName.c_str(),0,dwLogPrice,dwGoodsNum);	
					}
					else
					{
						SENDMSGBOX(pPlayer,AppFrame::GetText("4"));
						bResult=true;
					}
				}
				//以物换物商店
				else if(pTrade->tradeType==TradeList::ETT_GOODS)
				{
					vector<tagTestArg_ID_Num>vecDel;
					vector<TradeList::tagTradeGoods>&vecTrade=pTradeData->_vecTreadGoods;
					tagTestArg_ID_Num tgdelArg;
					//得到要删除物品的列表
					for (int i=0;i<(int)vecTrade.size();i++)
					{
					...
					if (!bDelSucc)
					{
						SENDMSGBOX(pPlayer,AppFrame::GetText("28"));
						bResult=true;
					}
					CGUID GoodsGuid = pGoods->GetExID();
					//测试成功
					...
					{		
						...
						{	
							...
							//玩家NPC商店买物品的日志记录												
							GetLogicLogInterface()->logT030_goods_log_npc(pPlayer,dwGoodsId,GoodsGuid,strGoodsName.c_str(),0,0,dwGoodsNum);
						}
						else
						{
							SENDMSGBOX(pPlayer,AppFrame::GetText("4"));
					...
					if (!bResult)
					{
						...
						{
							if (vecDel[i]._dwGoodsID==GlobalSetup::GetSetup()->dwWeimianGoodsId)
							{
						...
						if(dwSum>0)
						{
							GetLogicLogInterface()->logT031_goods_wm_npc(pPlayer,dwGoodsId,GoodsGuid,dwTotalNum,dwSum);
						}	
					}
				}
			...
			else
			{
				...
				{
					for (int i=0;i<(int)dwMaxGoods;i++)
					{
				...
				if(pTrade->tradeType==TradeList::ETT_MONEY || pTrade->tradeType==TradeList::ETT_SILVER)
				{
					...
					{
						for(int i=0;i<(int)vecAddResult.size();i++)
						{
							...
							{
								GetLogicLogInterface()->logT030_goods_log_npc(pPlayer,dwGoodsId,goodsguid,
									GoodsSetup::QueryGoodsName(dwGoodsId),0,dwLogPrice,dwTotalNum);                                
							}
						}
					}
					else
				...
				else if(pTrade->tradeType==TradeList::ETT_GOODS)
				{
					...
					for (int i=0;i<(int)vecTrade.size();i++)
					{
						...
						if (tgdelArg._dwGoodsID==GlobalSetup::GetSetup()->dwWeimianGoodsId)
						{
						...
						if (!bResult)
						{
							for(int i=0;i<(int)vecAddResult.size();i++)
							{
								...
								else
								{
									...
									GetLogicLogInterface()->logT030_goods_log_npc(pPlayer,dwGoodsId,goodsguid,
									GoodsSetup::QueryGoodsName(dwGoodsId),0,0,dwTotalNum);                                  

									//添加成功,记录位面精华消耗日志,一组物品要消耗的位面数量
									if (dwWmNum>0)
									{
										GetLogicLogInterface()->logT031_goods_wm_npc(pPlayer,dwGoodsId,goodsguid,
	...
	case MSG_C2S_SHOP_BUYBACK:
		{
			...
			*/
			...
			if(dwTotalSilver  > pPlayer->GetSilver())
			{								
				pPlayer -> SendNotifyMessage(AppFrame::GetText("14"));
				CMessage msg(MSG_S2C_SHOP_BUYBACK);
				msg.Add((BYTE)0);
				msg.Add(goodsID);
				msg.SendToPlayer(pPlayer->GetExID());
			...
			if( pExistedGoods == NULL )
			{
				...
				if( pContainer->Add(dwContainerPos,pGoods, NULL, &comMsg) )
				{
					...
					_snprintf(pszGoodsLog,1024,"[操作:SHOP_BUYBACK][玩家名:%s][容器ID:%d][容器位置:%d][物品ID:%d][物品名:%s][物品GUID:%s][物品数量:%d]",
						pPlayer->GetName(),dwContainerId,dwContainerPos,pGoods->GetBasePropertiesIndex(),GoodsSetup::QueryGoodsName(pGoods->GetBasePropertiesIndex()),srcGoodsGUID,pGoods->GetAmount());
					CMessage::AsyWriteFile(GetGame()->GetGoodsLogFile(),pszGoodsLog);							
#endif	
					...
					//玩家NPC商店回购物品的日志记录												
					GetLogicLogInterface()->logT030_goods_log_npc(pPlayer,dwGoodsId,goodsGuid,strGoodsName,3,dwTotalSilver,dwGoodsNum);	                    
	...
	case MSG_C2S_BUSINESS_BUY:
		{
			...
			for( WORD n = 0; n < wAmount; n++ )
			{					
				dwMaxStack = GoodsSetup::QueryGoodsMaxStackNumber( vecBuyGoodsId[n] );
				CGoodsBaseProperties *pProperties = GoodsSetup::QueryGoodsBaseProperties( vecBuyGoodsId[n] );
				if( !pProperties )
				{
					continue;
				}
				...
				else
				{
					...
					for( int i=0; i < (int)vecGoodsNum[n]; i++ )
					{
				...
				for( size_t i = 0; i < buyGoodsVec.size(); i++ )
				{
					...
					if( buyGoodsVec[i].pGoods )
					{
						...
						_snprintf( pszGoodsLog, 1024, "[物品回收:BUSINESS_BUY 金币不够][物品ID:%d][物品名:%s][物品GUID:%s][物品数量:%d]",
							buyGoodsVec[i].pGoods->GetBasePropertiesIndex(), GoodsSetup::QueryGoodsName(buyGoodsVec[i].pGoods->GetBasePropertiesIndex()),srcGoodsGUID,buyGoodsVec[i].pGoods->GetAmount());
						CMessage::AsyWriteFile( GetGame()->GetGoodsLogFile(), pszGoodsLog );							
					}
#endif
					CGoodsFactory::GarbageCollect( &( buyGoodsVec[i].pGoods ), 47 );						
				}
				pPlayer -> SendNotifyMessage( AppFrame::GetText("GS_NPCSHOP_2") );
				return;
			}
			...
			for( size_t i = 0; i < buyGoodsVec.size(); i++ )
			{
				...
				// 记录操作日志
				GetLogicLogInterface()->logT700_business_npc( pPlayer, buyGoodsVec[i].pGoods, 
	...
	case MSG_C2S_SHOP_SELL:	//玩家请求卖一个物品到商店
		{			
			...
			if(dwSilverPrice==0 )
			{
				pPlayer -> SendNotifyMessage( AppFrame::GetText("6"));
				CMessage msg(MSG_S2C_SHOP_SELL);
				msg.Add((BYTE)0);
				msg.Add(pGoods->GetExID());
				msg.SendToPlayer(pPlayer->GetExID());
			...
			{				
				...
				{
				pPlayer -> SendNotifyMessage( AppFrame::GetText("7") );
				CMessage msg(MSG_S2C_SHOP_SELL);
				msg.Add((BYTE)0);
				msg.Add(pGoods->GetExID());
				msg.SendToPlayer(pPlayer->GetExID());
				...
				{
					pPlayer -> SendNotifyMessage( AppFrame::GetText("15") );
					CMessage msg(MSG_S2C_SHOP_SELL);
					msg.Add((BYTE)0);
					msg.Add(pGoods->GetExID());
					msg.SendToPlayer(pPlayer->GetExID());
				...
				{	
					...
					if(pRemovedGoods)
					{
						...
						_snprintf(pszGoodsLog,1024,"[操作:SHOP_SELL][玩家名:%s][容器ID:%d][容器位置:%d][物品ID:%d][物品名:%s][物品GUID:%s][物品数量:%d]",
							pPlayer->GetName(),dwContianerId,dwPos,pGoods->GetBasePropertiesIndex(),GoodsSetup::QueryGoodsName(pGoods->GetBasePropertiesIndex()),srcGoodsGUID,pGoods->GetAmount());
						CMessage::AsyWriteFile(GetGame()->GetGoodsLogFile(),pszGoodsLog);							
#endif	
						...
						const char* strGoodsName=GoodsSetup::QueryGoodsName(dwGoodsId);
						DWORD dwGoodsNum=pGoods->GetAmount();
						GetLogicLogInterface()->logT030_goods_log_npc(pPlayer,dwGoodsId,goodsGuid,strGoodsName,1,dwSilverPrice,dwGoodsNum);	                        
					}																									
				}
			}//else end				
		}
	...
	case MSG_C2S_ESSENCE_SHOP_OPEN:
		{
			...
			TradeList::tagTrade *pTrade = TradeList::GetTradeList(szNpcName);
			if (pTrade)
			{
				...
				long lNum= (long)pTrade->vecGoods.size();			
				msg.Add(lNum);//物品数量
				if(pTrade->tradeType==TradeList::ETT_MONEY || pTrade->tradeType==TradeList::ETT_SILVER)
				{
					for(int i=0;i< (int)pTrade->vecGoods.size();i++)
					{					
	...
	case MSG_C2S_BUSINESS_SELL:	//玩家请求卖一个物品到商店
		{	
			...
			for( long i = 0; i < lSellAmount; ++ i )
			{
				...
				if( (long)pGoods->GetAmount() < goods_info.count )
				{
				...
				if( pBusinessGoods == NULL )
				{
                    Log4c::Error(ROOT_MODULE,"%-15s Not found business goods from id [%d]",__FUNCTION__,pGoods->GetBasePropertiesIndex());
			...
			for( SellGoodsList::iterator it = sell_goods.begin(); it != sell_goods.end(); ++ it )
			{
				...
				// 记录操作日志
				GetLogicLogInterface()->logT700_business_npc( pPlayer, goods_info.pGoods, 
				...
				if( goods_info.pGoods->GetAmount() == goods_info.count )
				{
					...
					_snprintf( pszGoodsLog, 1024, "[物品回收:BUSINESS_SELL][物品ID:%d][物品名:%s][物品GUID:%s][物品数量:%d]",
						goods_info.pGoods->GetBasePropertiesIndex(), GoodsSetup::QueryGoodsName( goods_info.pGoods->GetBasePropertiesIndex() ), srcGoodsGUID, goods_info.pGoods->GetAmount() );
					CMessage::AsyWriteFile( GetGame()->GetGoodsLogFile(), pszGoodsLog );							
#endif
	...
	case MSG_C2S_BUSINESS_INVEST:
		{
			...
			//{
			//	pPlayer->SendNotifyMessage(AppFrame::GetText("4"));
			//	return;
			//}
	...
	case MSG_C2S_SHOP_REPAIR: //玩家请求在商店中修理一个物品
		{
			...
			if(pGoods && pGoods -> CanReparied())
			{
				...
				if( pPlayer ->GetSilver() >= dwPrice )
				{
					...
					{
						GetLogicLogInterface()->logT023_goods_log_repair(pPlayer,pGoods->GetBasePropertiesIndex(),pGoods->GetExID(),1);
					...
					{
						pPlayer -> SendNotifyMessage(AppFrame::GetText("8"));
					}
				}
				else
				{
					pPlayer -> SendNotifyMessage(AppFrame::GetText("9"));
				}
			}
		}
		break;
	...
	case MSG_C2S_SHOP_REPAIRALL:	//修理全部物品
		{
			...
			{
				pPlayer -> SendNotifyMessage( AppFrame::GetText("10") );
				break;
			}
			...
			if(pPlayer->GetSilver()>=lPrice.m_dwPrice)
			{
				...
				GetLogicLogInterface()->logT023_goods_log_repair(pPlayer,0,CGUID::GUID_INVALID,2);
			}
			else
			{
				pPlayer -> SendNotifyMessage(AppFrame::GetText("9"));
