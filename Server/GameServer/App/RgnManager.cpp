...
CRgnManager::~CRgnManager()
{
	...
	{
		OBJ_RELEASE(CServerRegion, rgnItr->second);
	}
...
CServerRegion* CRgnManager::AddNormalRgn(const CGUID& rgnID, long templateRgnID)
{
	...
	{
		ret = OBJ_CREATE_PVOID(CServerRegion, (void*)&rgnID);
		ret->SetExID(rgnID);
...
long CRgnManager::CreateRgnMsg(Script* pScript/*挂起的脚本*/, long templateRgnID, eRgnType rgnType, const CGUID& ownerID, const char* szRetVarName/*异步返回值变量名*/, const char* szRetRgnIDName/*异步返回值变量名*/)
{
	...
	case RGN_PERSONAL:
		{
			...
			if(player->GetCurPRgnNum() >= GlobalSetup::GetSetup()->lCreatePDupRgnMaxNum)
			{
				char szGUID[128];
				player->GetExID().tostring(szGUID);
				Log4c::Trace(ROOT_MODULE,FormatText("GS_RGNMGR_0", GlobalSetup::GetSetup()->lCreatePDupRgnMaxNum, szGUID));
				return -4;//-4.0f;
			}
		}
		break;
	case RGN_TEAM:
		{
			CPlayer* player = GetGame()->FindPlayer(ownerID);
			if(player == NULL) return 0;
			if(player->GetCurTRgnNum() >= GlobalSetup::GetSetup()->lCreateTDupRgnMaxNum)
			{
				//设置返回值
				...
				char szGUID[128];
				player->GetExID().tostring(szGUID);
				Log4c::Trace(ROOT_MODULE,FormatText("GS_RGNMGR_1", GlobalSetup::GetSetup()->lCreateTDupRgnMaxNum, szGUID));
				return -4;//-4.0f;
			}
		}
		break;
...
long CRgnManager::OnCreateRgnMsg(CMessage* pMsg)
{
	...
	case RGN_PERSONAL:
		{	
			if(pPlayer && pPlayer->GetCurPRgnNum() >= GlobalSetup::GetSetup()->lCreatePDupRgnMaxNum)
			{	
				CMessage msg(MSG_S2W_RGN_CREATE_QUEST);
				msg.Add(srcGSID); // srcGSID
				msg.Add(ownerID); // owner ID
			...
			if(pPlayer)
			{
				if(pPlayer->GetCurPRgnNum() == 0) // 是初次创建
					pPlayer->RegisterCreatePRgnTimerID(GlobalSetup::GetSetup()->lCreateDupRgnDeltaTime);
	...
	case RGN_TEAM:
		{	
			if(pPlayer && pPlayer->GetCurTRgnNum() >= GlobalSetup::GetSetup()->lCreateTDupRgnMaxNum)
			{	
				CMessage msg(MSG_S2W_RGN_CREATE_QUEST);
				msg.Add(srcGSID); // srcGSID
				msg.Add(ownerID); // owner ID
			...
			if(pPlayer)
			{
				...
				if(pPlayer->GetCurTRgnNum() == 0) // 是初次创建
					pPlayer->RegisterCreateTRgnTimerID(GlobalSetup::GetSetup()->lCreateDupRgnDeltaTime);
	...
	if(rgn)
	{
		rgn->RegisterDeleteRgnTimerID(GlobalSetup::GetSetup()->lPDupRgnDelTime);
		long pos = 0;
	...
	if(rgnItr != m_mapPersonalHouseDupRgn.end())
	{
		...
		if( pRegion )
		{
			...
			if(vecGoodsItr == tVecGoods.end()) // 未找到
			{
				...
				tGoods.posX = (float)posX;
				tGoods.posY = (float)posY;
...
void CRgnManager::DeletePHGoods(const CGUID& rgnID, const char* goodsName)
{
	...
	if(rgnItr != m_mapPersonalHouseDupRgn.end())
	{
		...
		if( pShape )
		{
			...
			Log4c::Trace(ROOT_MODULE,FormatText("GS_RGNMGR_13", szRgnID, szGoodsID, tGoods.name));
...
bool CRgnManager::AddDupRgnData(long lTemplateID, const BYTE* pData, long lBufSize)
{
	if(pData && lBufSize > 0)
	{
		if(m_mapDupRgnData.size() < 128)//GlobalSetup::GetSetup()->lGsDupRgnDataMaxNum)
		{
			map<long, tagDupRgnData*>::iterator itr = m_mapDupRgnData.find(lTemplateID);
			if(itr != m_mapDupRgnData.end() && itr->second)
			{
				OBJ_RELEASE(tagDupRgnData, itr->second);
				m_mapDupRgnData.erase(itr);
			}
			tagDupRgnData* tagData = OBJ_CREATE(tagDupRgnData);
