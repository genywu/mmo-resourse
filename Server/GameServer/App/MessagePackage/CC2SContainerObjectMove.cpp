...
BOOL CC2SContainerObjectMove::Receive( CMessage* pMessage )
{
	...
	if( pMessage )
	{
		...
		if( ( m_lSourceContainerExtendID == CS2CContainerObjectMove::PEI_EQUIPMENT ||
            m_lSourceContainerExtendID == CS2CContainerObjectMove::PEI_EQUIP_GOD ||
            m_lSourceContainerExtendID == CS2CContainerObjectMove::PEI_EQUIP_LAND ||
            m_lSourceContainerExtendID == CS2CContainerObjectMove::PEI_EQUIP_HUMAN )
            && m_lDestinationContainerType==TYPE_SESSION)
			return FALSE;
		...
		if(m_lSourceContainerExtendID==CS2CContainerObjectMove::PEI_EQUIPMENT || m_lDestinationContainerExtendID==CS2CContainerObjectMove::PEI_EQUIPMENT)
		{
			...
			if(pPlayerRegioin)
			{
				...
				{
					//不允许穿上，取下，交换装备
					const char* pStrMsg=AppFrame::GetText("51");
					if(pStrMsg)
						pPlayer->SendNotifyMessage(pStrMsg);
					return FALSE;
				}
			}
		}
		...
		{
			if(m_lSourceContainerExtendID != CS2CContainerObjectMove::PEI_EQUIPMENT || 
				!(EC_MEDAL1 == m_dwSourceContainerPosition || EC_MEDAL2 == m_dwSourceContainerPosition || EC_MEDAL3 == m_dwSourceContainerPosition)
				)
			{
				return FALSE;
			}
		...
		if(m_lSourceContainerExtendID == CS2CContainerObjectMove::PEI_MEDAL_CONTAINER)
		{
			if(m_lDestinationContainerExtendID != CS2CContainerObjectMove::PEI_EQUIPMENT || 
				!(EC_MEDAL1 == m_dwDestinationContainerPosition || EC_MEDAL2 == m_dwDestinationContainerPosition || EC_MEDAL3 == m_dwDestinationContainerPosition)
				)
			{
				return FALSE;
			}
		...
		if(m_lSourceContainerType == TYPE_PLAYER && m_lDestinationContainerType == TYPE_PLAYER)
		{
			...
			//m_DestinationContainerID			= pPlayer->GetExID();
			//既然是玩家到玩家  那就必须是同一个玩家
		...
		{
			if(timeGetTime() - pPlayer->GetLastPickGoodsTime() <= GlobalSetup::GetSetup()->dwPickGoodsInterval)
			{
				return FALSE;
			}
			else
			...
			{				
  				//GetGame()->SendTopInfoToClient(1,pPlayer->GetExID(),1,1,AppFrame::GetText("32"));
				pPlayer->SendPlayerInfo(AppFrame::GetText("32"));// SendSystemInfo();
				return FALSE;
			}
		...
		else if(m_lSourceContainerType == TYPE_SESSION && m_lDestinationContainerType == TYPE_PLAYER)
		{
			...
			//范围限制
			if(	m_lDestinationContainerExtendID < 0 || m_lDestinationContainerExtendID > CS2CContainerObjectMove::PEI_PACK5 )
			{
				bResult	= FALSE;
			}
			...
			if(m_lDestinationContainerExtendID == CS2CContainerObjectMove::PEI_WALLET)
				m_dwDestinationContainerPosition	= 0;
			if(m_lDestinationContainerExtendID == CS2CContainerObjectMove::PEI_YUANBAO)
				m_dwDestinationContainerPosition	= 0;
				*/
		}
		//从玩家身上到Session
		...
		if( pSource -> GetType() == TYPE_REGION ) //来源为场景
		{
			...
			if( pRegion && pPlayer && pGoods )
			{
					...
					abs(pGoods -> GetTileY() - pPlayer -> GetTileY()) <= 2 )
				{
					...
					{
					case eGPV_Cant:
						pPlayer -> SendNotifyMessage(AppFrame::GetText("3"));
						break;
					...
					case eGPV_Allow_IsYourTeams:
						{
							...
							//! 如果是钱，则平分
							if(GoodsSetup::GetGoldCoinIndex() == pGoods->GetBasePropertiesIndex()
								|| GoodsSetup::GetSilverIndex() == pGoods->GetBasePropertiesIndex()
								)
							{
								long lGross = pGoods->GetAmount();
								long lMemberNum = pTeam->GetLucrativeAreaMemberAmount_Alive(pPlayer->GetRegionGUID(), (long)pPlayer->GetPosX(), (long)pPlayer->GetPosY(),TRUE);
								assert(0 != lMemberNum);
								if(0 == lMemberNum)
								{
									pResult = pGoods;
								...
								pTeam->GetLucrativeArea_Alive(RegionTeamates, pPlayer->GetRegionGUID(), (long)pPlayer->GetPosX(), (long)pPlayer->GetPosY(), TRUE);
								for (map<CGUID,CPlayer*>::iterator ite = RegionTeamates.begin(); ite != RegionTeamates.end(); ++ite)
								{
									if (pPlayer->GetExID() != ite->first)
									{
										if(GoodsSetup::GetSilverIndex() == pGoods->GetBasePropertiesIndex())
							...
							{
 								long lLoopNum = pTeam->GetLucrativeAreaMemberAmount_Alive(pPlayer->GetRegionGUID(), (long)pPlayer->GetPosX(), (long)pPlayer->GetPosY(), TRUE);
								list<CGUID> listMember;
								pTeam->GetLucrativeArea_Alive(listMember, pPlayer->GetRegionGUID(), (long)pPlayer->GetPosX(), (long)pPlayer->GetPosY(), TRUE);
								for (long i = 0; i < lLoopNum; ++i)
								{
									...
									if (NULL != pOwner)
									{
										...
										if(bSucced && vecResult.size() == vGoods.size())
										{
											...
											msgLink.AddEx(&vGoodsInfo[0],(long)vGoodsInfo.size());

											//! 聊天内容消息
											char szGetInfo[256] = {0};
											_snprintf(szGetInfo, 256, "%s%s", pOwner->GetName(), AppFrame::GetText("34"));
											ChatGoodsInfo.posBegin = (long)strlen(szGetInfo);
											_snprintf(&(szGetInfo[strlen(szGetInfo)]), 256 - strlen(szGetInfo), "%s", pGoods->GetName());
											ChatGoodsInfo.posEnd   = (long)strlen(szGetInfo);
										...
										{
											pOwner -> SendNotifyMessage(AppFrame::GetText("4"));
										}
									}
								}
								...
								if (NULL == pResult)
								{
									pPlayer->SendNotifyMessage( AppFrame::GetText("5"));
								}
							}
						}
					    break;
				...
				if( pContainer )
				{
					...
					if( pGoods )
					{
						...
						if( pGoods->GetAmount() != m_dwAmount && 
							...
							pGoods -> GetBasePropertiesIndex() != GoodsSetup::GetYuanBaoIndex())
						{
							if( pPlayer -> GetCurrentProgress() == CPlayer::PROGRESS_OPEN_STALL )
							{
								//发送消息给当前玩家.
								pPlayer -> SendNotifyMessage( AppFrame::GetText("2") );
								return pResult;
							}
...
BOOL CC2SContainerObjectMove::Move( CS2CContainerObjectMove* pResult )
{
	...
	if(pSource && pDestination && pGoods)
	{
		...
		CGoodsBaseProperties* pBaseProperty=GoodsSetup::QueryGoodsBaseProperties(pGoods->GetBasePropertiesIndex());
		if(!pBaseProperty)
			return FALSE;
		...
		if(pSource->GetType()==TYPE_PLAYER)
		{
			...
			if(!pPlayer)
				return FALSE;
			if(pPlayer->IsInPersonalShop(pGoods) && pGoods!= pPlayer->GetPackage()->GetWallet().GetGoldGoods())
				return FALSE;
		...
		{	
			...
			{			
				...
				{
					//是否是拾取品
					if(pGoods->GetGoodsBaseType()==GBT_PICKGOODS)
					{
						//直接从场景删除
					...
					{
						if(!pPlayer->IsGoodsUnique(pBaseProperty->GetOriginalName()))
						{
		...
		if(!lstrcmp(szGoodsName, GoodsSetup::GetGoldName()))
		{
		...
		else if(!lstrcmp(szGoodsName, GoodsSetup::GetSilverName()))
		{
			dwPrice = pGoods->GetAmount();
				...
				if( lDropGoodsStartPos != -1 )
				{
					...
					{						
						...
						if(pPlayer)
						{
							...
							const char* strRemovedGoodsName=GoodsSetup::QueryGoodsName(dwRemoveGoodsId);
							CGUID removeGoodsGuid=pGoods->GetExID();
							DWORD dwRemovedGoodsNum=pGoods->GetAmount();
							pPlayer->OnLoseGoods(pGoods);
							GetLogicLogInterface()->logT020_goods_log_pickup(pPlayer,dwRemoveGoodsId,removeGoodsGuid,strRemovedGoodsName,1,dwRemovedGoodsNum);
#ifdef _GOODSLOG1_
							char pszGoodsLog[1024]="";
							char playerGUID[50]="";
		...
		else if(pDestination->GetType()==TYPE_PLAYER)
		{
						...
						if(pRemovedGoods)
						{
							...
							if(pVolContainer->Add(m_dwDestinationContainerPosition,pGoods,&tPreviousContainer,pResult))
							{
								if(m_lSourceContainerType==TYPE_REGION)
								{
									...
									GetLogicLogInterface()->logT020_goods_log_pickup(pPlayer,dwRemoveGoodsId,removeGoodsGuid,strRemovedGoodsName,0,dwRemovedGoodsNum);
								}								
								pGoods=NULL;
								...
								{
									//仓库取出								
									GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,dwRemoveGoodsId,removeGoodsGuid,strRemovedGoodsName,dwRemovedGoodsNum,1);
									pPlayer->OnGetGoods(dwRemoveGoodsId);								
								}
								else if(m_lSourceContainerExtendID<eDCT_Primary && m_lDestinationContainerExtendID >= eDCT_Primary)
								{
									//仓库放入
									GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,dwRemoveGoodsId,removeGoodsGuid,strRemovedGoodsName,dwRemovedGoodsNum,0);
						...
						if(pSource->GetType()==TYPE_PLAYER)
						{
							...
							if(m_lSourceContainerExtendID!=CS2CContainerObjectMove::PEI_EQUIPMENT)
							{
								if(pSrcContainer->CanPutIn(pDesGoods,pPlayer))
								{
									...
									{
										// 仓库到玩家的交换	
										const char* strGoodsName=GoodsSetup::QueryGoodsName(pGoods->GetBasePropertiesIndex());
										const char* strDesGoodsName=GoodsSetup::QueryGoodsName(pDesGoods->GetBasePropertiesIndex());
										GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pGoods->GetBasePropertiesIndex(),pGoods->GetExID(),strGoodsName,pGoods->GetAmount(),1);
										GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pDesGoods->GetBasePropertiesIndex(),pDesGoods->GetExID(),strDesGoodsName,pDesGoods->GetAmount(),0);
									...
									else if(m_lSourceContainerExtendID<eDCT_Primary && m_lDestinationContainerExtendID >= eDCT_Primary && pGoods)
									{
										...
										const char* strGoodsName=GoodsSetup::QueryGoodsName(pGoods->GetBasePropertiesIndex());
										const char* strDesGoodsName=GoodsSetup::QueryGoodsName(pDesGoods->GetBasePropertiesIndex());
										GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pDesGoods->GetBasePropertiesIndex(),pDesGoods->GetExID(),strDesGoodsName,pDesGoods->GetAmount(),1);										
										GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pGoods->GetBasePropertiesIndex(),pGoods->GetExID(),strDesGoodsName,pGoods->GetAmount(),0);	
			...
			else if(m_lDestinationContainerExtendID==CS2CContainerObjectMove::PEI_PACK || eDCT_Secondary == m_lDestinationContainerExtendID)
			{
				...
				if(pDesGoods)
				{
					...
					{
						const char* strRes=AppFrame::GetText("37");
						if(strRes)
						{
						...
						m_dwDestinationContainerPosition),pGoods))
					{
						...
						if(pSrcContainer)
						{
							...
							{
								GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pGoods->GetBasePropertiesIndex(),pGoods->GetExID(),pGoods->GetName(),pGoods->GetAmount(),0);
								GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pDesGoods->GetBasePropertiesIndex(),pDesGoods->GetExID(),pDesGoods->GetName(),pDesGoods->GetAmount(),1);
							}
							else if (eDCT_Secondary == m_lSourceContainerExtendID && eDCT_Primary > m_lDestinationContainerExtendID)
							{
								GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pGoods->GetBasePropertiesIndex(),pGoods->GetExID(),pGoods->GetName(),pGoods->GetAmount(),1);
								GetLogicLogInterface()->logT060_goods_log_depository(pPlayer,pDesGoods->GetBasePropertiesIndex(),pDesGoods->GetExID(),pDesGoods->GetName(),pDesGoods->GetAmount(),0);
							}
			...
			else if(m_lDestinationContainerExtendID==CS2CContainerObjectMove::PEI_EQUIPMENT)
			{
				...
				if(pDesGoods)
				{
					...
					if(CS2CContainerObjectMove::PEI_MEDAL_CONTAINER == m_lSourceContainerExtendID)
					{
						...
						if(pPlayer->m_cEquipment.Swap((eEquipType)m_dwDestinationContainerPosition, pGoods, &pAimMedal))
						{
							RemoveSrcGoods(pResult);
							pResult->SetOperation(CS2CContainerObjectMove::OT_SWITCH_OBJECT);
							pResult->SetSourceContainer(pPlayer->GetType(), pPlayer->GetExID(), m_dwSourceContainerPosition);
					...
					else if(pPlayer->CanMountEquip(pGoods,m_dwDestinationContainerPosition,TRUE)==CPlayer::EQUIPFIT)
					{
						...
						if(!pSrcContainer)
							return FALSE;
						if( pPlayer->m_cEquipment.Swap( static_cast<eEquipType>(
							m_dwDestinationContainerPosition), pGoods, &pSrcGoods, pResult ) )
						{
							if(RemoveSrcGoods(pResult))
							{
				...
				eDCT_Silver == m_lDestinationContainerExtendID)
			{
				...
				DWORD dwMaxMoney=GoodsSetup::QueryGoodsMaxStackNumber( GoodsSetup::GetGoldCoinIndex());
				DWORD dwMaxSilver=GoodsSetup::QueryGoodsMaxStackNumber(GoodsSetup::GetSilverIndex());	
				if(m_lDestinationContainerExtendID==CS2CContainerObjectMove::PEI_WALLET)
				{
		...
		{
			if(bCreateNewFlag && pGoods->GetBasePropertiesIndex()!=GlobalSetup::GetSetup()->dwGoldGoodsId)
				return FALSE;
		...
		if(eOperType==CS2CContainerObjectMove::OT_MOVE_OBJECT)
		{
			...
			{
				GetLogicLogInterface()->logT070_goods_log_bank(pPlayer, 1, dwPrice);
			}					
			else if (eDCT_Gold == m_lDestinationContainerExtendID)
			{
				GetLogicLogInterface()->logT070_goods_log_bank(pPlayer, 0, dwPrice);
			}
		}
