...
#ifdef _DEBUG
		char szError[256];       
		_snprintf(szError, 256, "Build设置状态%d:161",STATE_DIED);   //值%s出错,请检查 , name    
...
void CBuild::InitNameValueMap()
{
	...
	//基本属性
	st.SetType(4);	st.pValue = &m_lMaxHP;				m_mapNameValue["B_MaxHp"]=st;				//生命上限
	st.SetType(4);	st.pValue = &m_lDef;				m_mapNameValue["B_Def"]=st;
	st.SetType(4);	st.pValue = &m_lMagicDef;			m_mapNameValue["B_MDef"]=st;	//魔法防御
}
...
//		else // 未找到该属性名字
//		{
//			AddErrLog("添加了一个未知属性：%s", name.c_str());
//			// 属性类型
...
//				if((*itBuildDirty)[0] == 'C' && (*itBuildDirty)[1] == '_') // 最终值 
//				{
...
//				if(((*itBuildDirty)[0] == 'C' && (*itBuildDirty)[1] == '_')) // 改变值) // 改变值
//				{
