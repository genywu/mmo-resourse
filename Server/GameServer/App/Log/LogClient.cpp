...
bool LogClient::Init(CBaseMessage *pMsg, LogClientInfo &logClientInfo)
{
	...
	uchar	*pData = new uchar[bufsize + 1];
	...
	m_lWriteThreadExit = 1;
	unsigned hThread;
	if(0 == (m_hWriteThread = (HANDLE)_beginthreadex(NULL, 0, FileWriteThread, (void*)this, 0, &hThread)))
	{	
		return false;
	}
...
bool LogClient::UpdateCondition(CBaseMessage *pMsg)
{
	...
	for (TableMap_ite ite = m_TableInfo.begin(); ite != m_TableInfo.end(); ++ite)
	{
		...
		for (size_t j = 0; j < ite->second.m_vecItem.size(); ++j)
		{
...
bool LogClient::SendLogToServer(long LogType, ...)
{
	...
	if (re)
	{
		...
		if (log_here)
		{
			...
			m_LogFile.WriteToBuf(string(szStr));
			SetEvent(m_hEvent);
			re = true;
		}
		...
		if (log_err)
		{
			...
			m_ErrFile.WriteToBuf(string(szStr));
			SetEvent(m_hEvent);
			re = true;
		}
	}
...
//如果是release版，则捕捉异常
	...
	m_LogFile.WriteToBuf(string(szStr));
	SetEvent(m_hEvent);
	//m_LogFile.WriteToBuf(strSQL + string("/r/n"));
...
bool LogClient::LogTxtToHere(const char *pTxt)
{
	...
	m_TxtLogFile.WriteToBuf(string(szTime) + string(pTxt));
	SetEvent(m_hEvent);
	return true;
}
...
//! 把日志参数压入uchar数组
bool LogClient::GetLogByteArray(va_list& args, LogTable &tableInfo, vector<uchar> &vData)
{
	if (1 == tableInfo.m_isLog)
	{
		for (size_t i = 0; i < tableInfo.m_vecItem.size(); ++i)
		{
...
long LogClient::TestCondition(const ItemCondition &condition, void *pCurrValue)
{
	...
	for (size_t i = 0; i < condition.m_vConditionTile.size(); ++i)
	{
...
void LogClient::GetCurrTimeString(string &strTime)
{
    SYSTEMTIME	stTime;
    char		szTime[128] = {0};
    GetLocalTime(&stTime);
...
unsigned __stdcall	LogClient::FileWriteThread(void* pThis)
{
	...
	while (true)
	{
		...
		if (0 == pLogClient->m_lWriteThreadExit)
			break;
	}
...
//////////////////////////////////////////////////////////////////////////
void LogClient::LogFile_ForThread::WriteToBuf(string &strTxt)
{
...
long LogClient::LogFile_ForThread::BufToFile(void)
{
	...
	if(!_ofstream.is_open())
	{
		if(!OpenStream())
		{
			...
			{
				_str_buf.clear();
				Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__,"本地日志文件不能写入，超过缓冲数量，进行清除");
			}
			return 1;
		}
	}
