...
public:
	...
	//! 验证一个会话，验证成功后拷贝值到传入的对象
	//! bRemove：完后是否删除会话
	BOOL TestAndCopySession(SessionInfo &Info, BOOL bRemove = TRUE)
	{
		//! 清理超时会话
		LONG now_time = (long)time(NULL);
		ClearOuttime(now_time);
		...
		for (; ite != m_listSession.end(); ++ite)
		{
			if(ite->Info == Info)
			{
				memmove(&Info, &(ite->Info), sizeof(SessionInfo));
				if (bRemove)
					m_listSession.erase(ite);
				return TRUE;
			}
