...
public:	
	...
	static bool CanTeamOpt(CPlayer *pPlayer);								//! 判断玩家能否进行组队操作
	...
	//const CGUID& GetNextGoodsOwner(void);									//! 得到下一个物品所有者
	...
	//void OnTeamChange(eTeamChange eTC, long lValue);						//! 响应队伍改变

	void MarkPlayerPosChange(const CGUID& PlayerGuid);						//! 标记玩家所在位置发生了改变

	void RadiateAllMemberPos(void);											//! 广播所有成员位置

	//void MarkPlayerIdioinfoChange(const CGUID& PlayerGuid);						//! 标记玩家非队伍数据发生了改变
	...
	//void GetCurrentRegionTeamates_Alive(map<CGUID,CPlayer*>& RegionTeamates,LONG lRegionID);//!得到队伍里边在当前场景的活着的成员
	...
	//long GetCurrentRegionTeamatesAverageLevel_Alive(long lRegionID);		//! 得到区域内的队伍成员平均等级
	...
	//void FindTeamatesListInCurrentRegion(const CGUID& RegionGUID, list<CPlayer*> &PlayerList);	//! 得到这个区域内的一个队伍成员列表
	...
	void RunScript(const char* strScirptName,CPlayer* pPlayer,long lDistance, char* strScirptName2 = NULL);	//! 运行脚本	//获取队伍当前任务 
	
	void SetCurrShareQuest(long questID);

	long GetCurrShareQuest()					{return m_lCurrShareQuestID;}

	void UpdateShareQuestList(tagShareQuestList *quest=NULL,bool flag = false,CGUID pPlayerID=NULL_GUID);						//! 更新共享任务列表
    ...
    /// 队伍处于的相位的ID
    void SetPhaseID(const CGUID& GID )  {  m_TeamPhaseID = GID ;   }
    const CGUID&  GetPhaseID()          {  return  m_TeamPhaseID;  }

	//!				判断是否为一个结婚队伍，如果是，获得另一方的GUID
	const CGUID&	GetMarriageOtherOne(CPlayer *pOnePlayer);

    void SendAllMemberLeaderChangeRgn(const CGUID& Leader,long lRegionID,
                                      long  x, long y);                     //! 队长通知所有队友场景改变
...
private:
	...
	long								m_eGoodsAllot;
	////! 分配顺序表
	//list<CGUID>							m_AllotList;
	//! 上一次的分配目标
	CGUID								m_LastAllotAim;
    /// 队伍处于的相位ID
    CGUID                               m_TeamPhaseID;
	//list<tagMemberInfo>::iterator		m_iteNextGainer;
	//! 在本服务器上的成员数量
