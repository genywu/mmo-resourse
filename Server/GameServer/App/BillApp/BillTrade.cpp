...
//extern bool IncPrsentProcessing(CPlayer* pPlayer, DWORD dwGoodsIndex, DWORD dwNum, DWORD dwTradeMode, long lTotalAmount );

// 帐户预冻
CBillTrade::CBillTrade(const CGUID &guidBuyer, const CGUID &guidSeller, const DWORD dwIP, const CBillClient::tagAccLockReq &reqLock)
...
CBillTrade::~CBillTrade(void)
{}

void CBillTrade::OnTimeOut(const string &strOrderID)
{
	char strLog[512] = "";
	...
	_snprintf( strLog, 512, "Req OI:%s, CI:0, TM:%d : Buyer %s(PT:%s) AccountLockRes 异常超时!", 
...
void CBillTrade::OnCallback(const string strOrderID, const GSBsipAccountLockResDef *pRes)
{
	char strLog[512] = "";
	CPlayer *pBuyer = GetGame()->FindPlayer(m_guidBuyer);
		...
		case 0:			// 预冻成功
			{
				...
				//case BT_INCPRESCET:
				//	{
				//		req.bConfirm	= IncPrsentProcessing(pBuyer, m_tagAccLockReq.lGoodsID,
				//			m_tagAccLockReq.lGoodsNum, CIncrementShopList::TM_YUANBAO, m_tagAccLockReq.lTotalYuanbao );
				//	}
				default:
					{
	...
	/*			const CBillClient::tagAccLockReq reqLock;
				CBillTrade *pBillTradeAck = new CBillTrade(m_guidBuyer, m_guidSeller, pBuyer->GetClientIP(), reqLock);
				pBillTradeAck->SetLeftTime(2000);
				if( req.billType == BT_INCSHOP || req.bConfirm == false )
				{
					GetBillClient()->SendAccountUnlock(&req, pBillTradeAck);
				}
				else
				{
					strcpy_s(req.szSellerID, m_tagAccLockReq.szSellerID);
					GetBillClient()->SendConsignTransfer(&req, pBillTradeAck);
				}*/
			}
			break;
...
void CBillTrade::OnCallback(const string strOrderID, const GSBsipAccountUnlockResDef *pRes)
{
	char strLog[512] = "";
	CPlayer *pBuyer = GetGame()->FindPlayer(m_guidBuyer);
...
void CBillTrade::OnCallback(const string strOrderID, const GSBsipConsignTransferResDef *pRes)
{
	char strLog[512]	= "";
	CPlayer *pBuyer		= GetGame()->FindPlayer(m_guidBuyer);
