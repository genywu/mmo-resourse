...
namespace Business
{
	void CStateGroup::AddPlayer( const CGUID &player_id, int iOperType )
	{
		...
		{
			Log4c::Warn(ROOT_MODULE,FormatText( "Player [%s] has already in business oper state.", GUID_STR( player_id ) ) );
		}
		else
		{
			tagState s;
	...
	void CStateGroup::SendUpdatePriceMsg()
	{
		for( PlayerStateTable::iterator it = m_PlayerTable.begin(); it != m_PlayerTable.end(); )
		{
			...
			else
			{
				...
				default:
					Log4c::Warn(ROOT_MODULE,FormatText( "Unknown business oper state [%d]", s.iState ) );
				}
			}
		}
	}
		...
		case OP_SELL:
			{
				...
				{
					// 有使用‘讨价还价’技能的成功记录
					ret =(DWORD)( dwBasePrice * pow( ( 1.0 + s.iPriceQuo * 0.01 ), (int)s.dwHiggleCount ) );
				}
				else
				{
					ret = dwBasePrice;
	...
	void CStateGroup::SetHiggleRecord( const CGUID &player_id, int iPriceQuo, bool bSuccess )
	{
		...
		if( s.iState == OP_NONE )
		{
			Log4c::Warn(ROOT_MODULE, FormatText( "Player [%s] is not in the business state group", GUID_STR( player_id ) ) );
		}
		else
		{
			if( bSuccess )
