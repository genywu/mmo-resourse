...
BOOL CVolumeLimitGoodsContainer::FindPositionForGoods(CGoods* pGoods, DWORD& dwPosition)
{
	...
	ulong goodsMaxStackNumer = GoodsSetup::QueryGoodsMaxStackNumber(pGoods->GetBasePropertiesIndex());
	if( goodsMaxStackNumer > 1 )
	{
		...
		for( ; it != m_mGoods.end(); it ++ )
		{
			if( it->second )
			{
				...
				{
					if( it->second->GetAmount() + pGoods->GetAmount() <= goodsMaxStackNumer )
					{
...
//		char str[256];
        ...
        Log4c::Trace(ROOT_MODULE,"背包:[%s](%s)在%d位置已有[%s](%s)。",pObject->GetName(),szGuid,
            dwPosition,	pGoods->GetName(), szExGuid );
		return FALSE;
	}
