...
BOOL GemContainer::IsGoodsExisted(DWORD dwGoodsBasePropertiesIndex)
{
	...
	// NOTE: Requires a correct return value to compile.
	BOOL bResult = FALSE;
	if( m_pGoldCoins && dwGoodsBasePropertiesIndex == GoodsSetup::GetGemIndex() )
	{
		bResult = TRUE;
	}
	return bResult;
...
CGoods* GemContainer::GetTheFirstGoods(DWORD dwGoodsBasePropertiesIndex)
{
	// TODO: Add your specialized code here.
	CGoods* pResult = NULL;
	if( dwGoodsBasePropertiesIndex == GoodsSetup::GetGemIndex() )
	{
...
#endif
		CGoodsFactory::CreateGoods( GoodsSetup::GetGemIndex(), dwAmount, vOut,24 );
