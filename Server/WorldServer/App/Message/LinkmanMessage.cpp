...
void OnLinkmanMan_DBS(CMessage *pMsg)
{
	...
	case MSG_DB2W_LINKMAN_PLAYER_DATA_SAVE_NOTIFY:
		{
			CGUID savedPlayerGuid;
			pMsg->GetGUID(savedPlayerGuid);
			LinkmanSystem::GetInstance().OnPlayerDataSave(savedPlayerGuid);
		}
		break;
...
void OnLinkmanMan_GS(CMessage *pMsg)
{
	...
	switch(pMsg->GetType())
	{
	//case MSG_S2W_LINKMAN_ADD:
	//	{
	//		LinkmanSystem::GetInstance().OnGsAdd(pMsg);
	//	}
	//	break;
	//case MSG_S2W_LINKMAN_DEL:
	//	{
	//		LinkmanSystem::GetInstance().OnGsDel(pMsg);
	//	}
	//	break;
