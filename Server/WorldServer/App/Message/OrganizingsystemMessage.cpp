...
void OnOrgasysMessage(CMessage* pMsg)
{
	...
	case MSG_S2W_Faction_ChangeExp_Reques:						//! 修改经验请求
		{
			...
			{
				//! 判断经验最大值
				/*const tagFacLevelSetup *FacLevelSetup = WSOrganizingParam::getInstance().GetLevelSetup(pFaction->GetLvl(eUT_FactionLevel) + 1);
				if(NULL != FacLevelSetup)
				{
					BOOL bHasExpNeed = FALSE;
					for (LONG i = 0; i < FacLevelSetup->vCondition.size(); ++i)
					{
						if(eUCT_FacExp == FacLevelSetup->vCondition[i].lConditionType)
						{
							LONG lMaxDeltaExp = FacLevelSetup->vCondition[i].lValue - pFaction->GetExp();
							lDeltaExp = min(lDeltaExp, lMaxDeltaExp);
							bHasExpNeed = TRUE;
							break;
						}
					}
	...
	case MSG_S2W_SHARE_QUESTLIST_UPDATE:
		{
            ...
            strcpy_s(quest.szName,20,pPlayer->GetName() );
			//得到队伍GUID
			...
			if (pTeam)
			{
				if (0 == btype)
				{
					pTeam->UpdateShareQuestList(lGameServerID,&quest,false,pPlayer);
				}
				else
	...
	case MSG_S2W_Faction_ChangeEnemy:
		{
			...
			if (pFaction)
			{
				...
				dwStartTime=(DWORD)(time(NULL)+dwleftTime+(lTime-1)*24*60*60+
    ...
    case MSG_S2W_QUEST_PLAYERRANKS:
        {   
            GetGame().GetRankManager()->ProcMsg(pMsg);
        }
        break;
	}
}