...
m_bIsFindByName(false)
{
	...
	void CEntityGroup::SetName(const char* pName) 
	{
		...
		{
			Log4c::Warn(ROOT_MODULE, GetText("WS_ENTITYGROUP_10"));
		}
	}
	// virtual interfaces
...
//#ifdef _TESTRUNSTACKINFO_
		...
		//							&& ( (GetCurDbOperType()!=DB_OPER_LOAD) 
		//							&& (tmpItr->second->GetCurDbOperType()!=DB_OPER_LOAD) 
		//							&& (leafItr->second->GetCurDbOperFlag()!=DB_OPER_LOAD) ) )
		//						{
		//							GetGUID().tostring(szOutGuid);
		//							sprintf(szOutPutStr, " {Add() %s[%s] leaf num %d,type %d,flag%d.", 
				...
				if(!isLoadOperFlag) // 不是读取操作
				{
					...
					default:
						{	
							...
							Log4c::Warn(ROOT_MODULE,FormatText("WS_ENTITYGROUP_11", GetCompositeFlag().c_str(), comFlag, DBOperType));
						}
						break;
					}
				}
...
//#ifdef _TESTRUNSTACKINFO_
		...
		//						{
		//							GetGUID().tostring(szOutGuid);
		//							sprintf(szOutPutStr, " {Get() %s[%s] leaf num %d,type %d,flag%d.", 
		//								GetCompositeFlag().c_str(), 
		//								szOutGuid, 
		//								m_mapEntityGroup.size(), 
		//								GetCurDbOperType(), 
...
//#endif
	...
	void CEntityGroup::InitProperty()
	{
		...
		if(itr != mapAttrDef.end())
		{
			...
			m_bHasDetailLeavesFlag = (BYTE)pInfo->lHasDetailLeaves;
			...
			{
				Log4c::Warn(ROOT_MODULE,FormatText("WS_ENTITYGROUP_12", m_CompositeFlag.c_str(), m_lLeafComFlag.c_str()));
			}
