...
bool   COrganizingCtrl::Initialize()
{
	...
	WSTeam::s_tagDefaultSetup.lCorrelativeSkillID	= GlobalSetup::GetSetup()->lCorrelativeSkillID;		//与组队关联的技能ID
	WSTeam::s_tagDefaultSetup.lMinSkillLevel		= GlobalSetup::GetSetup()->lMinSkillLevel;			//可以组队的最小技能等级
	WSTeam::s_tagDefaultSetup.lGoodsAllot			= GlobalSetup::GetSetup()->new_lGoodsAllot;			//默认分配方式
	WSTeam::s_tagDefaultSetup.lMaxMemberNum			= GlobalSetup::GetSetup()->new_lMaxMemberNum;		//默认最大人数
	WSTeam::s_tagDefaultSetup.lSessionTimeoutS		= GlobalSetup::GetSetup()->new_lSessionTimeoutS;	//会话超时秒数
	WSTeam::s_tagDefaultSetup.fIncreasedExp_2		= GlobalSetup::GetSetup()->new_fIncreasedExp_2;		//2人经验加成比例
	WSTeam::s_tagDefaultSetup.fIncreasedExp_3		= GlobalSetup::GetSetup()->new_fIncreasedExp_3;		//3人经验加成比例
	WSTeam::s_tagDefaultSetup.fIncreasedExp_4		= GlobalSetup::GetSetup()->new_fIncreasedExp_4;		//4人经验加成比例
	WSTeam::s_tagDefaultSetup.fIncreasedExp_5		= GlobalSetup::GetSetup()->new_fIncreasedExp_5;		//5人经验加成比例
	WSTeam::s_tagDefaultSetup.fIncreasedExp_6		= GlobalSetup::GetSetup()->new_fIncreasedExp_6;		//6人经验加成比例
	WSTeam::s_tagDefaultSetup.lExperienceRadii		= GlobalSetup::GetSetup()->new_lExperienceRadii;	//经验分配半径（单位：格）
	WSTeam::s_tagDefaultSetup.lExpDiff				= GlobalSetup::GetSetup()->new_lExpDiff;			//经验差
	WSTeam::s_tagDefaultSetup.lExpLimit				= GlobalSetup::GetSetup()->new_lExpLimit;			//玩家经验值份额基本保障
	WSTeam::s_tagDefaultSetup.fExpAmerce			= GlobalSetup::GetSetup()->new_fExpAmerce;			//经验惩罚系数
...
VOID COrganizingCtrl::OnDelFactionFinishMsg(const CGUID &AimGuid, BOOL bIsSysOpt)
{
	...
	{
		if(bIsSysOpt)
			GetLogicLogInterface()->LogT150_faction_log("系统", NULL_GUID, pFaction, 1);
		else
		{
			...
			if(NULL != pPlayer)
				...
				GetLogicLogInterface()->LogT150_faction_log("未找到操作者！", NULL_GUID, pFaction, 1);
		}
...
// 		PutStringToFile("Organize", szTMP);
        ...
        Log4c::Warn(ROOT_MODULE,"解散帮会[%s]失败！", szGUID);
	}
}
