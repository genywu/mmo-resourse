...
public:
	// 根据地图ID得到tagRegion指针
	tagRegion* GetTemplateNormalRegion(long lID/*模板ID*/)				{return (m_mapRegionList.find(lID)!=m_mapRegionList.end())?&m_mapRegionList[lID]:NULL;}
	//根据地图名字得到tagRegion指针
	tagRegion* GetTemplateNormalRegion(const char* strName);
	// 根据地图ID得到tagRegion指针
	tagRegion* GetTemplatePersonalRegion(long lID/*模板ID*/)		{return (m_mapPersonalRgnList.find(lID)!=m_mapPersonalRgnList.end())?&m_mapPersonalRgnList[lID]:NULL;}
	// 根据地图ID得到tagRegion指针
	tagRegion* GetTemplatePersonalHouseRegion(long lID/*模板ID*/)	{return (m_mapPersonalHouseRgnList.find(lID)!=m_mapPersonalHouseRgnList.end())?&m_mapPersonalHouseRgnList[lID]:NULL;}
	// 根据地图ID得到tagRegion指针
	tagRegion* GetTemplateTeamRegion(long lID/*模板ID*/)		{return (m_mapTeamRgnList.find(lID)!=m_mapTeamRgnList.end())?&m_mapTeamRgnList[lID]:NULL;}
	...
	CWorldRegion* CreateRgn(bool isDupRgnDataFlag,
		long scrGSID								/*发送创建请求的GSID*/,
		long templateRgnID							/*模板Region的ID*/,
		eRgnType rgnType, long GSID, 
		const CGUID& ownerID=NULL_GUID				/*NORMAL类型则ID为空*/,
		const CGUID& scriptID=NULL_GUID,
		const CGUID& dbRgnGUID=NULL_GUID, 
		const char* szRetVarName=NULL,
		const char* szRetRgnIDName=NULL);			/*异步返回值变量名*/
