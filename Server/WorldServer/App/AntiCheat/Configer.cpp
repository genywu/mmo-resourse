...
#define ADJUST_RANGE( r ) \
	if( r.second < r.first ) { \
    Log4c::Warn(ROOT_MODULE,"%s : %d < %d.", #r, (int)r.second, (int)r.first );\
	r.second = r.first; \
	}
...
namespace AntiCheat
{
	...
	bool CConfiger::Load()
	{
		...
		if( prFile == NULL )
		{
			Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__, "Load AntiCheat.xml FAILED." );
			return false;
		}
		...
		if (pSystemElem == NULL)
		{
			Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__, "Load AntiCheat.xml System Node Failed");
		}
		else 
		{
		...
		if( pTalkElem == NULL )
		{
			Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__, "Talk element is null" );
		}
		else
		{
	...
	bool CConfiger::LoadSensWords( const char *file )
	{
		...
		if( fp == NULL )
		{
			Log4c::Error(ROOT_MODULE,"%-15s Load %s FAILED.",__FUNCTION__, file );
			return false;
		}
