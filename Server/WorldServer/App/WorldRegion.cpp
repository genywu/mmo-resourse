...
b_OperFlag(false),m_DelTime(0),m_CostValue(0),m_bIsTemplateRgnFlag(false)
{
	...
	m_pVariableList = new VariableList;
	//原地复活标志符
	m_bRelive = false;
...
{
	if(m_pVariableList)
		SAFE_DELETE(m_pVariableList);
}
VariableList* CWorldRegion::GetVariableList(void)
{
	return m_pVariableList;
}
void CWorldRegion::SetVariableList(VariableList* vl)
{
	if(vl==NULL)
		return;
...
void CWorldRegion::UpdateVariableList(VariableList* vl)
{
	if(vl==NULL)
		return ;
	...
	if(prfile)
	{
		...
		while(1)
		{
			...
			if (str=="#")
			{
				...
				if (st.lNum>1)
				{
					st.lNum = (long) (st.lNum * GlobalSetup::GetSetup()->fMonsterNumScale );
					if (st.lNum==0 )
						st.lNum = 1;
				}
...
BOOL CWorldRegion::LoadSetup()
{
	...
	if(prfile==NULL)
	{
        Log4c::Warn(ROOT_MODULE,FormatText("WS_RGNMGR_10", filename));
		return false;
	}
...
bool CWorldRegion::AddToByteArray(vector<BYTE>* pByteArray, bool bExData)
{
	...
	VarList::Encode( m_pVariableList, pByteArray );
...
bool CWorldRegion::CloneAllData(CWorldRegion* pRgn)
{
	if(pRgn)
	{
		...
		m_pCell = new tagCell[m_lHeight*m_lWidth];
