...
BOOL COnCmdConnect::Init(VOID)
{
	...
	if( streamGsInfo.is_open() )
	{
		...
		streamGsInfo.close();
		Log4c::Trace(ROOT_MODULE,GetText("LOGIN_CMDCON_0"));//加载GsInfo.ini成功!
	}
	else
	{
		AppFrame::NotifyMessageBox("LOGIN_CMDCON_1");//不能找到文件GsInfo.ini!
		return FALSE;
	}
...
LPVOID COnCmdConnect::Watch(LPVOID pThis)
{
	...
	local.sin_port = htons(pOnCmdConnect->m_usPort);
