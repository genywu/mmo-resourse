...
ulong CCrc32Static::s_arrdwCrc32Table[256] =
{
...
inline void CCrc32Static::CalcCrc32(const uchar byte, ulong &dwCrc32)
{
	dwCrc32 = ((dwCrc32) >> 8) ^ s_arrdwCrc32Table[(byte) ^ ((dwCrc32) & 0x000000FF)];
}

bool CCrc32Static::GetFileSizeQW(const HANDLE hFile, ulonglong &qwSize)
{
...
ulong CCrc32Static::FileCrc32Filemap(LPCTSTR szFilename, ulong &dwCrc32)
{
	_ASSERTE(szFilename);
	_ASSERTE(lstrlen(szFilename));

	ulong dwErrorCode = NO_ERROR;
	...
	try
	{
		...
		{
			ulonglong qwFileSize = 0, qwFileOffset = 0;
			ulong dwByteCount, dwViewSize;
			ulong dwBaseAddress;
				...
				else
				{
					...
					while(qwFileSize > 0)
					{
						if(qwFileSize < MAX_VIEW_SIZE)
							dwViewSize = LO_UINT_64(qwFileSize);
						else
							dwViewSize = MAX_VIEW_SIZE;

						dwBaseAddress =(ulong)(MapViewOfFile(hFilemap,
							FILE_MAP_READ,
							HI_UINT_64(qwFileOffset),
							LO_UINT_64(qwFileOffset),
							dwViewSize));
...
//计算一块数据的CRC
ulong CCrc32Static::DataCrc32(const void* pBuff,long lLen, ulong &dwCrc32)
{
	_ASSERTE(pBuff);
	_ASSERTE(lLen);
