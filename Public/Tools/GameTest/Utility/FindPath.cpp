...
bool CFindPath::Find(int x1,int y1, int x2, int y2)
{
	...
	while(1)
	{
		...
		//找到路径 或 超时
		if( timeGetTime() > dwStartTime + 2000 || abs(m_mapOpen[p].x-ex) < 1 && abs(m_mapOpen[p].y-ey)  < 1 )
		{
			long lTheSteps = m_mapOpen[p].step+1;
			stPos pPoint;
			int ix = m_mapOpen[p].ox, iy = m_mapOpen[p].oy;
			int i = InClose(ix, iy);

			for(int n = lTheSteps-2; n >= 0; n--)
			{
				pPoint.x = ix;
				pPoint.y = iy;
		...
		long dir = GetLineDir(oldx, oldy, fx, fy);
...
int CFindPath::Try(int lx, int ly, int step, float fStep, int Oldx, int Oldy, int dir, float fAddStep)
{
	...
	// 如果两步的方向一样，那么实际的距离减半
	int nNewDir = GetLineDir(Oldx, Oldy, lx, ly);
	if (nNewDir == dir)
	{
		fStep -= fAddStep / 5.f;
	}
