...
void CUniEditorDlg::DrawJpg(unsigned int unNo)
{
	...
	// 取得应用程序文件名(绝对路径)
	::GetModuleFileName(NULL,strVal,100);
	// 取得应用程度路径
	::PathRemoveFileSpec(strVal);
	//设置当前工作目录
	::SetCurrentDirectory(strVal);
	sprintf(strVal, _T("regions\\%u.jpg"), unNo);
	string str(strVal);
...
BOOL CUniEditorDlg::OnCmdMsg(UINT nID, int nCode, void* pExtra,
			  AFX_CMDHANDLERINFO* pHandlerInfo)
{
	if (nCode == BN_CLICKED && nID == IDC_REFRESH)
	{
		int i = 0;
	}
