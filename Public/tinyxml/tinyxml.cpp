...
const char * TiXmlElement::Attribute( const char * name,const char * node) const
    {
    TIXML_STRING str( name );
    const TiXmlAttribute* new_node = attributeSet.Find( str );
...
// Load memory data.
bool TiXmlDocument::LoadData(const uchar* pszData,int length, TiXmlEncoding encoding)
{
	if ( !pszData ) 
	{
...
bool TiXmlDocument::SaveFile( FILE* fp ) const
    ...
    if ( useMicrosoftBOM ) 
        {
        const uchar TIXML_UTF_LEAD_0 = 0xefU;
        const uchar TIXML_UTF_LEAD_1 = 0xbbU;
        const uchar TIXML_UTF_LEAD_2 = 0xbfU;
