...
bool	CFilesInfo::Load(string& strName)
{
	...
	if(pf == NULL)
	{
        Log4c::Warn(ROOT_MODULE,"[SYS] 装载文件 %s 出错!",m_FileName.c_str());
		return false;
	}
	...
	{
		Clear();
		Log4c::Warn(ROOT_MODULE, "读取文件信息出错.");
		return false;
	}
	Log4c::Trace(ROOT_MODULE, "CFilesInfo::Load() end!");
	return true;
}
...
bool	CFilesInfo::LoadFolderInfo(FILE* pf,tagFileInfo* pParentagFileInfo)
{
	...
	ulong dwSize,dwOrginSize,dwValidSize,dwCrc32,dwPackageType,dwCompressType;
	ulong bFolder;
	while(!feof(pf))
	{
		//fscanf(pf,"%s ",strName);
...
bool	CFilesInfo::Save(string& strName)
{
	...
	{
		Log4c::Warn(ROOT_MODULE, "保存文件 %s 出错!",strName.c_str());
		return false;
	}
	return true;
}
...
tagFileInfo* 
CFilesInfo::AddFileInfo(tagFileInfo* pParentFileInfo,string& strName,ulong dwCrc32,
						ulong dwSize,ulong dwOrginSize,ulong dwValidSize,
						ulong dwPackageType,ulong dwCompressType)
{
	if(pParentFileInfo==NULL)	return NULL;
...
//更新一个文件信息
tagFileInfo* 
CFilesInfo::UpdateFileInfo(string& strFileName,ulong dwCrc32,ulong dwSize,
...
//需要更新父结点的大小和其他信息
tagFileInfo* 
CFilesInfo::UpdateFileInfo(tagFileInfo* ptagInfo,ulong dwCrc32,ulong dwSize,
							ulong dwOrginSize,ulong dwValidSize,bool bFolder,
							ulong dwPackageType,ulong dwCompressType)
{
	if(ptagInfo)
...
//更新打包信息
void	CFilesInfo::UpdateFileInfoWithChild(string& strFileName,ulong dwPackageType,
...
//更新打包信息
void	CFilesInfo::UpdateFileInfoWithChild(tagFileInfo* ptagInfo,ulong dwPackageType,
									ulong dwCompressType,
									bool bForcePack,
									bool bForceCompress)
{
...
void	CFilesInfo::UpdateFilePackInfoWithChild(string& strFileName,ulong dwPackageType)
{
	char *newstring = _strlwr( _strdup( strFileName.c_str() ));
	strFileName = newstring;
	free(newstring);
...
void	CFilesInfo::UpdateFilePackInfoWithChild(tagFileInfo* ptagInfo,ulong dwPackageType)
{
	if(ptagInfo == NULL)	return;
	if(HaveChild(ptagInfo))
	{
...
//返回0，表示添加失败
ulong	CFilesInfo::AddPackFileInfo(string& strPackFileName)
{
	//不能添加空的包名
...
string	CFilesInfo::GetPackFileName(ulong dwID)
{
	itTypeInfo it = m_PackageInfos.begin();
	for(;it != m_PackageInfos.end();it++)
	{
...
ulong	CFilesInfo::GetPackID(string strFileName)
{
	itTypeInfo it = m_PackageInfos.begin();
	for(;it != m_PackageInfos.end();it++)
	{
...
//得到文件的CRC信息码
//返回0 表示没有此文件
ulong	CFilesInfo::GetFileCrc32(string& strFileName)
{
	char *newstring = _strlwr( _strdup( strFileName.c_str() ));
	strFileName = newstring;
	free(newstring);
...
//得到一个文件的信息
bool
CFilesInfo::GetFileInfo(const string& strFileName,ulong& dwSize,
						ulong& dwOrginSize,ulong &dwValidSize,ulong& dwCrc32,
						ulong& dwPackageType,ulong& dwCompressType,bool& bFolder)
{
								   ...
								   CFilesInfo& temptFilesInfo,bool bPackage)
{
	...
	else
	{
		...
		{
			ulong dwSize=0,dwOrginSize=0,dwValidSize=0;
			ulong dwCrc32=2,dwPackageType=1,dwCompressType=1;
			bool bFolder=false;
...
tagFileInfo* CFilesInfo::GetFileInfoByText(string strFileName)
{
    ...
    /*
    basic_string <char>::size_type   IRes;	
    IRes = str.find("\\");
    while( IRes == string::npos && pFileInfo != NULL)
    {
        str = str.erase(0,str.find("\\")+1);
        string strTempt("");
        IRes = str.find("\\");
    ...
    */
	...
	while( (int)str.find("\\") == 0 && pFileInfo != NULL)
	{
		str = str.erase(0,str.find("\\")+1);
		string strTempt("");
		nPos = (int)str.find("\\");
...
void CFilesInfo::UpdateParentFolderInfo(tagFileInfo* ptagFileInfo)
{
	...
	if(HaveChild(ptagFileInfo))
	{
		ulong dwTemptPackage = 0,dwTemptCompress = 0;
...
//统计文件夹信息
void CFilesInfo::UpdateFolderInfo(tagFileInfo* ptagFileInfo,ulong& dwPackage,ulong& dwCompress)
{
	if(ptagFileInfo==NULL)	return;
