...
const long MAX_MAP_VIEW_SIZE = 10*10;
...
private:
	...
	HANDLE	m_hMapObject;
	//逆向
	HANDLE m_hMapObjectAnti;


	//内存影射文件的地址空间
	LPVOID m_pszMapView;
	//逆向
	LPVOID m_pszMapViewAnti;
...
private:
	//检查空间大小是否满足
	bool CheckViewSpace(long lNeedSize,bool bAnti = false);
	//返回下次写数据的位置
	int WriteData(int nPos,const char *pData,int nLen,bool bAnti=false);
	//范围下次读数据的位置
	int ReadData(int nPos,char *pData,int nLen,bool bAnti=false);	
public:
	//初始化
	...
	void WriteAnti(const char* pszFileName,const char* pszText);
	//读数据
	eFileOpType Read(char* pszFileName,char* pszText);

	eFileOpType ReadAnti(char* pszFileName,char* pszText);
};
