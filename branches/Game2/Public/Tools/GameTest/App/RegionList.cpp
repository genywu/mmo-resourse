...
CRegionList::tagRegion* CRegionList::GetRegion(long lRegionID)
{
	...
	for(; it != m_lsWorldMap.end(); ++it)
	{
		...
		// 大地图的各个区域下的场景列表查找
		for(int i=0; i<it->vecArea.size(); ++i)
		{
...
BOOL CRegionList::LoadMap()
{
	if(m_lsWorldMap.size() == 0)
	{
		...
		if(!m_Tdoc.LoadFile())
		{
			PutDebugString("装载配置文件<GameMapConfig>出错！");
			return FALSE;
		}
		// 		//载入技能配置文件
		// 		if( !m_Tdoc.LoadData(prfile->GetData(),prfile->GetDatalen()) )
...
BOOL CRegionList::FindRegionPath(long lSourID,long lTargetID)
{
	...
	if(pSourRegion && pTargRegion)
	{
		...
		// 		{
		// 			char strInfo[512]="";
		// 			_snprintf(strInfo,512,"跨场景开始：从%s->%s",pSourRegion->strName.c_str(),pTargRegion->strName.c_str());
		// 			PutStringToFile("RegionTest",strInfo);
		// 		}
		// 区域相同
		...
		for (int i = 0; i < pSourRegion->vecCurAddrDetails.size(); ++i)
		{
		...
		if(!m_lsRegionPathList.empty())
		{
			...
			// 				{
			// 					_snprintf(strInfo,512,"添加场景ID：%d",(*it).lRegionID);
			// 					PutStringToFile("RegionPath",strInfo);
			// 				}
			// 				PutStringToFile("RegionPath","*****************");
			// 			}
			return TRUE;
		}
	}
...
BOOL CRegionList::AddRegionPath(list<CRegionList::tagRegionPathInfo> & lsRegionPath,tagRegionPathInfo stSourInfo,tagRegion * pTarget)
{
	...
	stSourInfo.lIndex = lsRegionPath.size();
	SearchedPathList[stSourInfo.lRegionID] = stSourInfo;
	lsRegionPath.push_back(stSourInfo);
	// 	if(g_bDebug)
	// 	{
	// 		char strInfo[512]="";
	// 		_snprintf(strInfo,512,"添加场景ID：%d",stSourInfo.lRegionID);
	// 		PutStringToFile("RegionTest",strInfo);
	// 	}
	...
	if(pSourRegion)
	{
		...
		for (int i = 0; i < pSourRegion->vecCurAddrDetails.size(); ++i)
		{
			...
			stPathInfo.lRegionID = pSourRegion->vecCurAddrDetails[i].AddRegionId;
			stPathInfo.stPos = pSourRegion->vecCurAddrDetails[i].stPos;
			stPathInfo.lIndex = lsRegionPath.size();
			if (stPathInfo.lRegionID == 0)
			{
				continue;
			}
			if(stPathInfo.lRegionID == pTarget->lID)
			{
				...
				// 				{
				// 					char strInfo[512]="";
				// 					_snprintf(strInfo,512,"**************找到目标场景ID：%d,中间场景数为：%d**************",stPathInfo.lRegionID,lsRegionPath.size());
				// 					PutStringToFile("RegionTest",strInfo);
				// 				}
				// 如果该节点已存在，直接返回
...
// 		long index = -1;
// 		// 找出距离最近的点
// 		for (int i = 0; i < stMailBoxPos.vecPos.size(); ++i)
// 		{
...
// 			}
// 		}
// 		if(index >= 0 && index < stMailBoxPos.vecPos.size())
// 		{
