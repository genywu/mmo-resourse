...
enum eMsgTYpe
{
	...
	MSG_S2W_PHASESYSTEM             = MSG_S2W + MAX_SUB * 20,
	MSG_S2W_PLAYERPHASE,

    // Auction House 
    MSG_S2W_AH = MSG_S2W + MAX_SUB * 21,
    MSG_S2W_AH_OPER,

    // 
    MSG_S2W_QUEST = MSG_S2W + MAX_SUB * 22,
    MSG_S2W_QUEST_OPER,
	...
	MSG_W2S_PHASESYSTEM             = MSG_W2S + MAX_SUB * 15,
	MSG_W2S_PLAYERPHASE,

    // Auction House
    MSG_W2S_AH = MSG_W2S + MAX_SUB * 16,
    MSG_W2S_AH_OPER,

    MSG_W2S_QUEST = MSG_W2S + MAX_SUB * 17,
    MSG_W2S_QUEST_OPER,
    ...
    // Auction House
    MSG_C2S_AH = MSG_C2S + MAX_SUB * 24,
    MSG_C2S_AH_OPER,

    MSG_C2S_PHASE = MSG_C2S + MAX_SUB*25,
    MSG_C2S_PHASE_OPER,
    MSG_C2S_FOLLOW_Leader,              // 跟随队长

	// new Quest module
	MSG_C2S_QUEST = MSG_C2S + MAX_SUB * 26,
	MSG_C2S_QUEST_OPER,

	/// 玩家交易
	MSG_C2S_PLAYERTRADE					= MSG_C2S + MAX_SUB * 27,
    ...
    MSG_S2C_SHAPE_ATK_IMMUNITY,                             // 免疫效果消息
    ...
    MSG_S2C_OTHER_VALIDATE,                                 //把验证码发给客户端，使之登录ChatServer
	...
	//！ 玩家交易
	MSG_S2C_PLAYERTRADE				= MSG_S2C + MAX_SUB * 21,
    ...
    // Auction Hosue
    MSG_S2C_AH = MSG_S2C + MAX_SUB * 33,
    MSG_S2C_AH_OPER,

    // Phase_S2C
    MSG_S2C_PHASE = MSG_S2C + MAX_SUB * 34,
    MSG_S2C_PHASE_OPER,
    MSG_S2C_FOLLOW_Leader,                  // GS反馈跟随队长模式

	//整改后的任务模块
	MSG_S2C_QUEST = MSG_S2C + MAX_SUB * 35,
	MSG_S2C_OPER,

    //天赋---
    MSG_S2C_GENIUS =MSG_S2C + MAX_SUB * 36, 
	...
	MSG_DB2W_OPER					= MSG_DB2W + MAX_SUB * 4, //WS->DBS
	// ----------------------------------------------------
    ...
    MSG_S2CS = MAX_TYPE * 38,
    MSG_S2CS_PLAYER_ENTER_GAME,             // 玩家上线
    MSG_S2CS_PLAYER_EXIT_GAME,              // 玩家下线
    MSG_S2CS_PLAYER_CHANGE_REGION,          // 玩家切换场景
    MSG_S2CS_PLAYER_JOIN_TEAM,              // 玩家加入队伍
    MSG_S2CS_PLAYER_LEAVE_TEAM,             // 玩家离开队伍
    MSG_S2CS_PLAYER_JOIN_FACTION,           // 玩家加入帮会
    MSG_S2CS_PLAYER_LEAVE_FACTION,          // 玩家离开帮会
    MSG_S2CS_DISBAND_TEAM,                  // 队伍解散
    MSG_S2CS_DISBAND_FACTION,               // 帮会解散

    // ChatServer -> GameServer
    MSG_CS2S = MAX_TYPE * 39,

    // GameClient -> ChatServer
    MSG_C2CS = MAX_TYPE * 40,
    MSG_C2CS_CHAT,                          // Client发送聊天信息到CS
    MSG_C2CS_QUEST_VALIDATE,                // Client发送验证码到CS请求验证

    // ChatServer -> GameClient
    MSG_CS2C = MAX_TYPE * 41,
    MSG_CS2C_CHAT,                          // CS反馈聊天信息到Client
};