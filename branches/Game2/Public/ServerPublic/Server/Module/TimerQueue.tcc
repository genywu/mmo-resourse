...
template <class TYPE> inline
ulong TimerNode<TYPE>::GetTimerValue(void) const
{
    return m_dwTimeValue;
}

template <class TYPE> inline
void TimerNode<TYPE>::SetTimerValue(ulong timervalue)
{
    m_dwTimeValue=timervalue;
}

template <class TYPE> inline
ulong TimerNode<TYPE>::GetInterval(void) const
{
    return m_dwInterval;
}

template <class TYPE> inline
void TimerNode<TYPE>::SetInterval(ulong interval)
{
...
template <class TYPE> inline
void TimerNode<TYPE>::OnTimerOut(ulong curtime)
{
...
template <class TYPE>
void CTimerQueueIterator<TYPE>::GoToNext(ulong nSartSpoke)
{
    //查找第一个非空的条目
    ulong sc = m_TimerQueue.m_nSpokeCount;
    for (ulong  i = nSartSpoke; i < sc; ++i)
    {
...
*/
template <class TYPE>
CTimerQueue<TYPE>::CTimerQueue(	ulong spokecount,
                               ulong resolution,
                               ulong prealloc)
                               :m_pSpokes(0)
...
template <class TYPE>
void CTimerQueue<TYPE>::OpenI(size_t prealloc, ulong spokes, ulong res)
{
...
template <class TYPE>
TimerNode<TYPE>* CTimerQueue<TYPE>::FindSpokeNode(ulong spoke, long timerid) const
{
    if(spoke >= m_nSpokeCount)
    {
       Log4c::Trace(ROOT_MODULE,"定时器轮变量越界(spoke:%d,m_nSpokeCount:%d).",spoke,m_nSpokeCount);
        return 0;
    }
...
TimerNode<TYPE>* CTimerQueue<TYPE>::FindNode(long timerid) const
{
    ...
    ulong spokemask = m_nSpokeCount - 1;
    ulong start = timerid & spokemask;
    TimerNode<TYPE>* n = FindSpokeNode (start, timerid);
    if (n != 0)
        return n;
    // 搜寻剩余的spokes
    for (ulong i = 0; i < m_nSpokeCount; ++i)
    {
        if (i != start)
        {
...
//得到第一个最近的时间值
template <class TYPE>
ulong CTimerQueue<TYPE>::EarliestTime(void) const
{
...
template <class TYPE>
ulong CTimerQueue<TYPE>::CalculateSpoke(ulong expire) const
{
    return static_cast<ulong> ((expire >> m_nResBits) & (m_nSpokeCount- 1));
}

//根据所给的spoke,产生一个独一无二的timerid,如果根节点的计数器溢出,
//这个时候就要遍历所有的节点，以得到一个没有使用的timerid.
template <class TYPE>
long CTimerQueue<TYPE>::GenerateTimerID(ulong spoke)
{
                                 ...
                                 ulong interval)
{
    ...
    if (n != 0)
    {
        ulong spoke = CalculateSpoke(futuretime);
...
{
    ulong expire = n->GetTimerValue ();
    ulong spoke = CalculateSpoke (expire);
    ScheduleI(n, spoke, expire);
}

//提供给 Schedule()和Reschedule()使用的公用函数
template <class TYPE>
void CTimerQueue<TYPE>::ScheduleI(TimerNode<TYPE>* n, ulong spoke, ulong expire)
{
    //检查是否更新最早的时间
...
*/
template <class TYPE>
int CTimerQueue<TYPE>::ResetInterval(long timerid,ulong interval)
{
...
int CTimerQueue<TYPE>::Cancel(const TYPE& type)
{
    ...
    if(!IsEmpty())
    {
        ...
        ulong last = first->GetTimerValue ();
        int recalc = 0;
...
//计算出下一个expire的定时器
template <class TYPE>
void CTimerQueue<TYPE>::RecalcEarliest(ulong last)
{
    if (IsEmpty ())
        return;
...
template <class TYPE>
TimerNode<TYPE>* CTimerQueue<TYPE>::RemoveFirstExpired(ulong now)
{
...
*/
template <class TYPE>
int CTimerQueue<TYPE>::Expire(ulong curtime)
{
    int expcount = 0;
