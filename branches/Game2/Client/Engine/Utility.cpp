...
void utility::Text::Create(CRFile *pRFile)
{
	if(NULL == pRFile)	return;

	Stream stream;
	stream.Create(pRFile->GetData(),pRFile->GetDatalen());
...
BOOL utility::File::FileExist(const char *szFileName)
{
    ...
    string strTempt(szFileName);
    CheckRFileStr(strTempt);
#ifdef __CLIENT__
	if (!bExist)
	{
        bExist = CClientResource::GetInstance()->IsFileExist(strTempt.c_str());
	}
#endif
