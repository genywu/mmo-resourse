...
void ui::Wnd::RenderDefault(void)
{
	...
	if (dwStyle & FWS_DEBUG && m_tWindowInfo.bActive)
	{
		...
		pLayer2D->OutPutText(iStartX + 1,iStartY + 1,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX - 1,iStartY - 1,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX + 1,iStartY - 1,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX - 1,iStartY + 1,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX + 1,iStartY    ,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX - 1,iStartY    ,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX    ,iStartY - 1,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX    ,iStartY + 1,szTemp,lstrlen(szTemp),FWC_BLACK);
		pLayer2D->OutPutText(iStartX,iY - iFontSize,szTemp,lstrlen(szTemp),FWC_WHITE);		
...
void ui::Button::Render()
{
	...
	if (dwStyle & FWS_VISIBLE)
	{
		...
		if (dwStyle & FWS_DISABLE)
		{
			...
			ui::Wnd::SetUIRendState();
			pLayer2D->OutPutText(iTextStartX,iTextStartY,m_tWindowInfo.szName,iLen,cLow);	
		}
		else
		{
			switch(m_eBtnState)
			...
			//default to center
			ui::Wnd::SetUIRendState();
			pLayer2D->OutPutText(iTextStartX + 1,iTextStartY + 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX - 1,iTextStartY - 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX + 1,iTextStartY - 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX - 1,iTextStartY + 1,m_tWindowInfo.szName,iLen,FWC_BLACK);		
			pLayer2D->OutPutText(iTextStartX + 1,iTextStartY,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX - 1,iTextStartY,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX,iTextStartY - 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX,iTextStartY + 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
			pLayer2D->OutPutText(iTextStartX,iTextStartY,m_tWindowInfo.szName,iLen,cText);	
		}
	}
	//NO CHILD
}
...
void ui::MenuBar::Menu::Render()
{
	...
	for(DWORD dwItem = 0; dwItem < m_dwItemCnt;dwItem++)
	{
		...
		if (stItem.m_dwStyle & MBS_CHECKED)
			pLayer2D->OutPutText(iX+ iVeticalBarWidth /2 - iFontWidth/2,iTextStartY,"√",2,cText);

		pLayer2D->OutPutText(iTextStartX + 1,iTextStartY + 1,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX - 1,iTextStartY - 1,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX + 1,iTextStartY - 1,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX - 1,iTextStartY + 1,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX + 1,iTextStartY,stItem.szText,iLen,FWC_BLACK);
		pLayer2D->OutPutText(iTextStartX - 1,iTextStartY,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX,iTextStartY + 1,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX,iTextStartY - 1,stItem.szText,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iTextStartX,iTextStartY,stItem.szText,iLen,cText);	
	}
}
	...
	if ((dwStyle & FWS_VISIBLE) && !(dwStyle & FWS_DISABLE))
	{
		...
		for(DWORD dwMenu = 0; dwMenu < m_dwMenuCnt;dwMenu++)
		{
			...
			if (cMenu.m_dwStyle & MBS_DISABLE)
			{
				...
				pLayer2D->OutPutText(iTextStartX,iTextStartY,cMenu.m_szText,iLen,cBack);	
			}
			else
			{
				...
				ui::Wnd::SetUIRendState();
				pLayer2D->OutPutText(iTextStartX + 1,iTextStartY + 1,cMenu.m_szText,iLen,FWC_BLACK);
				pLayer2D->OutPutText(iTextStartX - 1,iTextStartY - 1,cMenu.m_szText,iLen,FWC_BLACK);
				pLayer2D->OutPutText(iTextStartX + 1,iTextStartY - 1,cMenu.m_szText,iLen,FWC_BLACK);
				pLayer2D->OutPutText(iTextStartX - 1,iTextStartY + 1,cMenu.m_szText,iLen,FWC_BLACK);
				pLayer2D->OutPutText(iTextStartX + 1,iTextStartY,cMenu.m_szText,iLen,FWC_BLACK);
				pLayer2D->OutPutText(iTextStartX - 1,iTextStartY,cMenu.m_szText,iLen,FWC_BLACK);	
				pLayer2D->OutPutText(iTextStartX,iTextStartY + 1,cMenu.m_szText,iLen,FWC_BLACK);	
				pLayer2D->OutPutText(iTextStartX,iTextStartY - 1,cMenu.m_szText,iLen,FWC_BLACK);	
				pLayer2D->OutPutText(iTextStartX,iTextStartY,cMenu.m_szText,iLen,cText);	
			}
		}
...
void ui::QuickMenu::Item::Render()
{
	...
	if ((dwStyle & FWS_VISIBLE) && !(dwStyle & FWS_DISABLE))
	{
		...
		pLayer2D->OutPutText(iStartX + 1,iStartY + 1,m_tWindowInfo.szName,iLen,FWC_BLACK);
		pLayer2D->OutPutText(iStartX - 1,iStartY - 1,m_tWindowInfo.szName,iLen,FWC_BLACK);
		pLayer2D->OutPutText(iStartX + 1,iStartY - 1,m_tWindowInfo.szName,iLen,FWC_BLACK);
		pLayer2D->OutPutText(iStartX - 1,iStartY + 1,m_tWindowInfo.szName,iLen,FWC_BLACK);
		pLayer2D->OutPutText(iStartX + 1,iStartY,m_tWindowInfo.szName,iLen,FWC_BLACK);
		pLayer2D->OutPutText(iStartX - 1,iStartY,m_tWindowInfo.szName,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iStartX,iStartY + 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iStartX,iStartY - 1,m_tWindowInfo.szName,iLen,FWC_BLACK);	
		pLayer2D->OutPutText(iStartX,iStartY,m_tWindowInfo.szName,iLen,m_tWindowInfo.colorText);		
...
void ui::ListBox::Render()
{
	...
	if (dwStyle & FWS_VISIBLE)
	{
		...
		for(int i = 0; i < m_iDisplayMax; i++)
		{
			...
			/*	
			pLayer2D->OutPutText(iTextX + 1,iWAY + 1,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX - 1,iWAY - 1,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX + 1,iWAY - 1 ,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX - 1,iWAY + 1,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX + 1,iWAY,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX - 1,iWAY,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX,iWAY + 1,pItem->GetText(),iLen,FWC_BLACK);
			pLayer2D->OutPutText(iTextX,iWAY - 1,pItem->GetText(),iLen,FWC_BLACK);*/
			pLayer2D->OutPutText(iTextX,iWAY,pItem->GetText(),iLen,m_tWindowInfo.colorText);
			
			if (dwStyle & FWS_LBS_CHECK)
			{
				if (pItem->GetCheck())
					//pLayer2D->DrawSolidQuad(iWAX+4,iWAY+4,iFontSize-8,iFontSize-8,m_tWindowInfo.colorHigh);
					pLayer2D->OutPutText(iWAX, iWAY, "∨", 2, m_tWindowInfo.colorHigh);
			}
			else
			{
...
void ui::EditBox::Render()
{
	...
	/*	pLayer2D->OutPutText(m_iTempTextX + 1,m_iTempTextY + 1,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX - 1,m_iTempTextY - 1,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX + 1,m_iTempTextY - 1,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX - 1,m_iTempTextY + 1,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX + 1,m_iTempTextY,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX - 1,m_iTempTextY,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX,m_iTempTextY + 1,(const char*)szTemp,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(m_iTempTextX,m_iTempTextY - 1,(const char*)szTemp,iTexLen,FWC_BLACK);*/
		pLayer2D->OutPutText(m_iTempTextX,m_iTempTextY,(const char*)szTemp,iTexLen,m_tWindowInfo.colorText);
...
void ui::TabCtl::Label::DrawLabel()
{
	...
	if (!m_bHighLight)
	{
		...
		ui::Wnd::SetUIRendState();
		pLayer2D->OutPutText(iX,iY + 3,m_szText,iTexLen,0x8fFFFFFF);
	}
	else
	{
		pLayer2D->DrawSolidQuad(iX,iY,m_iWidth,m_iHeight,FWC_BACK);
		ui::Wnd::SetUIRendState();
		pLayer2D->OutPutText(iX + 1,iY + 1,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX - 1,iY - 1,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX + 1,iY - 1,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX - 1,iY + 1,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX + 1,iY,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX - 1,iY,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX,iY + 1,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX,iY - 1,m_szText,iTexLen,FWC_BLACK);
		pLayer2D->OutPutText(iX,iY, m_szText,iTexLen,FWC_TEXT);
	}
}
...
void ui::CheckBox::Render()
{
	...
	if (dwStyle & FWS_VISIBLE)
	{
		...
		int iTextStartY = iY + iH/2 - pLayer2D->GetFontSize()/2;
		ui::Wnd::SetUIRendState();
		pLayer2D->OutPutText(iTextStartX,iTextStartY,m_tWindowInfo.szName,strlen(m_tWindowInfo.szName),m_tWindowInfo.colorText);
		pLayer2D->DrawWireQuadRect(iX,iTextStartY-2,iX+12,iTextStartY+10);
		if(m_bChecked)
		{
			const char *checked = "√";
			ui::Wnd::SetUIRendState();
			pLayer2D->OutPutText(iX,iTextStartY,"√",strlen(checked),m_tWindowInfo.colorText);
		}
	...
	for(int i = 0;i < m_iDisplayMax; ++i)
	{
		...
		if(pNode)
		{
			...
			if(pNode->IsExpanded())
			{
				...
				ui::Wnd::SetUIRendState();
				pLayer2D->OutPutText(ix, iy, strText,strlen(strText),m_tWindowInfo.colorText);
			}
			else
			{
				if(pNode->GetChildCount() != 0)
