...
void console::Logfile::Create()
{
	...
	static char szLogFileName[_MAX_PATH];
	 ...
	 _snprintf_s<_MAX_PATH>(szLogFileName,256,"log\\%d_%d_%d[%2.2d_%2.2d_%2.2d].html",st.wYear,st.wMonth,st.wDay,st.wHour,st.wMinute,st.wSecond);
	...
	strcpy_s(m_pszLogFileName,_MAX_PATH,szLogFileName);
    FILE *fp = 0;
	fopen_s(&fp , m_pszLogFileName, "wb");	
	if (fp) 
	{
		//write header
...
//open file
    FILE *fp = 0;
	fopen_s(&fp, m_pszLogFileName, "ab");	
	if (fp) 
	{
		//write header
		fprintf(fp,"<HR COLOR=\"#dfdfe5\">");
...
void console::Manager::Outpute(const void *szFormat,...)
{
	...
	static char szBuffer0[10240]="",szBuffer1[10240]="";
	if (m_dwFilter & CONSOLE_ERROR)
	{
		va_list vapt;
		va_start(vapt,szFormat);
        _vsnprintf_s((char*)szBuffer0,10240,10240,(const char*)szFormat,vapt);
		va_end(vapt);
		...
		strcpy_s(szBuffer1, 10240, "(Engin-Error)");
		strcat_s(szBuffer1, 10240, szBuffer0);
		Log4c::Trace(ROOT_MODULE,szBuffer1);
	}
}
...
void console::Manager::Output(const void *szString,...)
{
	...
	if (m_dwFilter & CONSOLE_OUTPUT)
	{
		...
		_vsnprintf((char*)szBuffer0,10240,(const char*)szString,vapt);	
		va_end(vapt);
