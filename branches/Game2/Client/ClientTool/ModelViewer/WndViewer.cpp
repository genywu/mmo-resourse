...
BOOL WndViewer::Create(const char *pszName, const RECT *pRect, DWORD dwStyle, ui::Wnd *pParent)
{
	...
	RECT rectQ = { 0, 0, 100, 20 };
	m_pQuickMenu->AddItem("打开主模型", &rectQ);		//0
	m_pQuickMenu->AddItem("打开绑定模型", &rectQ);		//1
	m_pQuickMenu->AddItem("打开参考模型", &rectQ);		//2
	m_pQuickMenu->AddItem("打开背景图片", &rectQ);		//3
...
void WndViewer::DisplayTextureProjective()
{
	...
	if (!texprojlist.empty())
	{
		...
		for(std::list<CTextureProjective*>::iterator it = texprojlist.begin();it != texprojlist.end();)
		{
			...
			if (p->IsAlive())
			{
				for (std::list<CTextureProjective::POSDIRANDTIME>::iterator it = p->m_listpostime.begin();it != p->m_listpostime.end();it++)
				{
...
void WndViewer::Render(void)
{
	...
	}else
	{
		...
		if (m_pModel)
		{
			m_cAnimInfo.SetupAnimInfo(m_vRolePos.x, m_vRolePos.y, 0.0f, 0.0f, m_pCamera);
			...
			m_pModel->SetStatic(CGameModelManager::GetInstance()->GetGameModel(m_pModel->GetGameModelID())->CheckStatic());
			m_pModel->SetUseNormal(true);
			m_pModel->SetRenderModelStatic(true);
			...
			//m_pModel->SetModelColorCompel(0x30ffffff);
		...
		pLayer3D->SetAmbient(0xff000000);
		pLayer3D->Render();
		pLayer3D->Clear();
			...
			default:
				{
					if (m_pModel && dwMenu < 5 + m_dwGroupMenuItemCount)			//判断主模型菜单
					{
						...
						bChecked = pMenu->IsChecked(dwMenuItem);
						if (bChecked)
					...
					}else if (m_vBindModels.size() != 0)				//判断绑定模型菜单
					{
						...
						CDisplayModel * pModel = m_vBindModels[m_iLevelModelIndex]->GetDisplayModel();
						...
						pMenu->SetCheckItem(dwMenuItem, true);//!bChecked);
					}
				}
				break;
			}
...
void WndViewer::ShowAxis(void)
{
	...
	//画坐标draw axis (X , Y , Z)
	D3DXMATRIX matTran;
	D3DXMatrixTranslation(&matTran, -20.0f, 30.0f, 0.0f);
...
void WndViewer::ExportAnimationFrame()
{
	...
	pInterface->GetDevice()->CreateTexture(pInterface->GetWndWidth(),
										   pInterface->GetWndHeight(),
										   1,
	...
	pInterface->GetDevice()->CreateDepthStencilSurface(pInterface->GetWndWidth(),
													   pInterface->GetWndHeight(), 
	...
	sprintf(szSavePathName, "%s/%02d%s", m_pWndAnimFrameEx->GetAnimFrameSaveDir(), m_dwAnimFrameIndex++, m_szpFileFormat);
...
void WndViewer::OpenMainModelFile(const char * pszFileName)
{
	...
	CDisplayModel * pModel	   = CGameModelManager::GetInstance()->CreateDisplayModel(pszFileName);
	VERIFY(pModel);
