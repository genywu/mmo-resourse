...
ONode::ONode(void)
{
	...
	ZeroMemory(m_pChildNodeArray, sizeof(ONode *) * 8);
...
ONode::~ONode(void)
{
	...
	SAFEDELETE(m_pChildNodeArray[0]);
	SAFEDELETE(m_pChildNodeArray[1]);
	SAFEDELETE(m_pChildNodeArray[2]);
	SAFEDELETE(m_pChildNodeArray[3]);
	SAFEDELETE(m_pChildNodeArray[4]);
	SAFEDELETE(m_pChildNodeArray[5]);
	SAFEDELETE(m_pChildNodeArray[6]);
	SAFEDELETE(m_pChildNodeArray[7]);
...
void ONode::OptimizeBuild(std::vector<int> &vParentNodeGridIndexArray,std::vector<int> &vParentNodeBuildGridIndexArray,DWORD &dwNodeCount)
{
	...
	for(it = vParentNodeBuildGridIndexArray.begin();it != vParentNodeBuildGridIndexArray.end();it++)
	{
		...
		{
			//计算build点位置
			D3DXVECTOR3 & vBuildOffset = pMapGrid->GetBuild()->vPosOffset;
			D3DXVECTOR3 & vBuildRotation = pMapGrid->GetBuild()->vRotation;
			D3DXVECTOR3 & vBuildScaling = pMapGrid->GetBuild()->vScaling;
