...
VOID TerrainMesh::RecalculateGridTexture(DWORD dwGridIndex, DWORD dwVertexLocation, std::vector<_tagVertex> &vOldVertexArray)
{
	...
	BYTE &bBrushSize = m_tProcessData.bBrushSize;
	//此层根据设定的画笔大小而设置的对称循环分别为0、1、2、4、8
	for(INT nCountRow = -bBrushSize; nCountRow <= bBrushSize; nCountRow++)
	{
		INT nStepRadius;
		switch(m_tProcessData.bBrushType)
		...
		case BT_CIRCLE:
			{
				//当画笔大小等于8、4的情况分别换成以下数组
				//以下数组数据是通过模拟近似圆所配数据
				if (bBrushSize==8)
				{
					int size_8[9]={8,8,7,7,6, 6,4,3,1};
					nStepRadius = size_8[abs(nCountRow)];
				}
				else if (bBrushSize==4)
				{
					int size_8[5]={4, 4,3,2,1};
					nStepRadius = size_8[abs(nCountRow)];
				}
				else
					nStepRadius = bBrushSize - abs(nCountRow);
		...
		case BT_CIRCLE:
			{
				...
				{
					int size_8[9]={8,8,7,7,6, 6,4,3,1};
					nStepRadius = size_8[abs(nIndexDepth)];
				}
				else if (bBrushSize==4)
				{
					int size_4[5]={4, 4,3,2,1};
					nStepRadius = size_4[abs(nIndexDepth)];
				}
				else
			...
			case BT_CIRCLE:
				{
					...
					{
						int size_4[5]={4, 4,3,2,1};
						nStepRadius = size_4[abs(nCountRow)];
					}
					else
						nStepRadius = bBrushSize - abs(nCountRow);
	...
	for(INT nCountRow = -bBrushSize; nCountRow <= bBrushSize; nCountRow++)
	{
		...
		for(INT nCountColumn = -nStepRadius; nCountColumn <= nStepRadius; nCountColumn++)
		{
			...
			if(ValidLocation(nCurrentRow, nCurrentColumn, FALSE))
			{
				...
				case ET_SMOOTH:                      // 平滑
					{
						int numCount=0;       //这个均值的个数
						float allHeight=0.0f; //这个均值的总数
						for (int i=-1;i<2;i++)//循环行数
						{
							for (int j=-1;j<2;j++)//循环列数
							{
								//判断以下面的点是否在有效区域内~如果不在则不需要计算
								if(ValidLocation(nCurrentRow+i, nCurrentColumn+j, FALSE))
								{
									//找到将要计算的Vertex的ID号
									INT nCurrentVertexID = (nCurrentRow+i) * (m_dwWidth + 1) + nCurrentColumn+j;
									_tagVertex *pCurVertex;
									//找到这个成员变量的内部的对应结构体
									pCurVertex = &m_ptVertexList[nCurrentVertexID];
									//累加
									numCount++;
									allHeight +=pCurVertex->vPosition.y;
								}

							}
						}
						//如果等于0，说明没有在地编上选取网格（这种情况不可能发生）
						//写起避免以外
						if (numCount!=0)
						{
							fTemple = allHeight/(float)numCount;
							//获取平滑增量
							float dataY=pVertex->vPosition.y-fTemple;
							//0.95为平滑系数范围是   0 < x < 1
							//减小系数-->增大平滑作用（效果很明显）
							//增大系数-->减小平滑作用（效果不明显）
							fTemple =fTemple+dataY*0.97f;
						}
						else
							fTemple = allHeight;
					}
