...
const char *pszWarEditStatusName[] = {"选择","放置"};

ToolWarEdit::ToolWarEdit(void)
{
...
void ToolWarEdit::MessageProc(BOOL bActive)
{
	//切换
	...
	ListBoxWar * pListBox = pEditWnd->GetListBoxWar();
	...
	if (bActive)
	{
		if (pInputInfo->dwMouseButtonState & MK_LBUTTON)
		{
			...
			int iSelIndex = pListBox->GetSelItem();
			if (iSelIndex < 0) 
				iSelIndex = 0;
			PaintBrush((DWORD)iSelIndex);
		}
	}
}

void ToolWarEdit::Render()
{
	DrawBrush();
	_ShowStatus();
}
void ToolWarEdit::Create()
{
}

void ToolWarEdit::Destroy(void)
{
}

void ToolWarEdit::_ShowStatus(void)
{
	...
	//sprintf(szTemp,"[Q]:%s\n",pszWarEditStatusName[m_eEditStatus]);
 ...
 //   pLayer2D->DrawSolidQuad(iStartX - 2,iStartY ,iTextWidth + 8,iTextHeight + 2 ,0x80bbc5ff);
	...
	//pLayer2D->OutPutText(iStartX,iStartY - 1,szTemp,iTexLen,FWC_BLACK);  
	//sprintf(szTemp,"[#a080FF80Q#FFFFFFFF]:#FFFFFF44%s",pszWarEditStatusName[m_eEditStatus]);
	//pLayer2D->TextOutEx(iStartX,iStartY,szTemp,lstrlen(szTemp));
}

void ToolWarEdit::DrawBrush(void)
{
...
void ToolWarEdit::PaintBrush(DWORD dwWar)
{
	if (m_dwCurSelGrid == 0xFFFFFFFF)
		return;       
	...
	for(int j = - m_iBrushSize; j < m_iBrushSize + 1; j++)
	{
		for(int i = - m_iBrushSize; i < m_iBrushSize + 1;i++)
		{
			...
			if (!(iX < 0 || iX >= (int)dwMapWidth || iZ < 0 || iZ >= (int)dwMapDepth))
			{
				...
				DWORD dwGridIndex = iX + iZ * dwMapWidth;
				//检查是否全部相同
				if (pMap->GetGrid(dwGridIndex)->GetWar() != dwWar)
				{
					bAllLike = FALSE;
					break;
				}
	...
	if (!bAllLike)//有不同，则涂
	{
		CmdMapFillWar *pCmd = new CmdMapFillWar;
		pCmd->SetWar(dwWar);
		for(int j = - m_iBrushSize; j < m_iBrushSize + 1; j++)
		{
			for(int i = - m_iBrushSize; i < m_iBrushSize + 1;i++)
...
DWORD ToolWarEdit::GetSelectGrid(void)
{
	ui::Manager::_tagINPUTINFO *pInputInfo = ui::Manager::GetInstance()->GetInputInfoBuffer();
	//计算选中的格子
