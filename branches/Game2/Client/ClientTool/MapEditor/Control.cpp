...
void RadioButton::Render()
{
	...
	if (dwStyle & FWS_VISIBLE)
	{
		...
		if (m_bDisplayText)
		{
			...
			pLayer2D->OutPutText(iX+iW,iY + iH /2,m_tWindowInfo.szName,lstrlen(m_tWindowInfo.szName),m_tWindowInfo.colorText);
		}
	}
}
...
void CheckBox::Render()
{
	...
	if (dwStyle & FWS_VISIBLE)
	{
		...
		if(m_bChecked)
			pLayer2D->OutPutText(iTextStartX,iTextStartY,"★",2,m_tWindowInfo.colorText);
		pLayer2D->OutPutText(iX+iW+pLayer2D->GetFontSize()/2/2,iTextStartY,m_tWindowInfo.szName,lstrlen(m_tWindowInfo.szName),m_tWindowInfo.colorText);
	}
}
