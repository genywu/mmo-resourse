...
void EditorGLCanvas::setImage( const wxString& name )
{
    ...
    if ( !name.IsEmpty() )
    {
        try
        {
            CEGUI::String defResGrp = "editor_data";
            Imageset& imageset =
                ImagesetManager::getSingleton().createFromImageFile(
                    "__auto_editor_imageset__",
                CEGUIHelper::ToCEGUIString( name ),defResGrp);
            ...
            GLuint tex = static_cast<CEGUI::OpenGLTexture*>(
                    imageset.getTexture())->getOpenGLTexture();
...
void EditorGLCanvas::changeSize( int newWidth, int newHeight )
{
    ...
    m_GUISystem->notifyDisplaySizeChanged(
        CEGUI::Size( static_cast<float>( newWidth ),
                     static_cast<float>( newHeight ) ) );
    m_imagesetStaticImage->notifyScreenAreaChanged();
...
void EditorGLCanvas::OnMouseMotion( wxMouseEvent& event )
{
    ...
    // limit redraws triggered via mouse movements to only the essential ones
