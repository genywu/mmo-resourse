...
public:
    struct imageInfo
    {
        ...
        wxString  width;
        wxString  height;
        /*nikai 2010.9.23 新增偏移设置
        说明：偏移的设置功能只作为方便在编辑器中修改。图形区域还是以x, y, width, height 决定。
        偏移值在客户端生效。
        */
        wxString XOffset;
        wxString YOffset;  
        bool operator < (const imageInfo& op)
        {
            if(this == &op)
                return false;
            int ret = name.Cmp(op.name);
            if(ret > 0)
                return false;
            else
                return true;
        ...
        //struct less1
        //{
        //    bool operator() ( const imageInfo &op1, const imageInfo &op2 ) const
        //    {
        //        return true;
        //    }
        //};
        static bool Nameless(const imageInfo &op1 , const imageInfo &op2)
        {
            if(&op1 == &op2)
                return false;
            int ret = op1.name.Cmp(op2.name);
            if(ret > 0)
                return false;
            else
                return true;
        }
        static bool Xless(const imageInfo &op1 , const imageInfo &op2)
        {
            return op1.x > op2.x; 
        }
        static bool Yless(const imageInfo &op1 , const imageInfo &op2)
        {
            return op1.y > op2.y; 
        }
        static bool Widthless(const imageInfo &op1 , const imageInfo &op2)
        {
            return op1.width > op2.width; 
        }
        static bool Heightless(const imageInfo &op1 , const imageInfo &op2)
        {
    ...
    void SortGridByName(wxGridEvent& event);
    //
    ...
    void addRegion(const wxString& name, const wxRect& dim, int rowindex, const wxPoint &OffsetPoint);
    ...
    //! Return thr currently defined Offset for an image region
    wxPoint getRegionOffset( const wxString& name) const;
