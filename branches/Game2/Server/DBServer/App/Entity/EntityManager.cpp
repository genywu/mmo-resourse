...
*Brief: 对象类:该对象管理所有的对象实例和对象的配置信息数据
*Author:安海川
*Date:  2008.11.03
*/
#include "StdAfx.h"
...
void CEntityManager::Release(void)
{
    ...
    {
        SAFE_DELETE(m_pRankThread);
        ReleaseRankInfo();  
   //     DeleteCriticalSection(&m_csRankInfo);
        m_mapRank.clear();
    }   
	...
	// 等待线程退出
	WaitForMultipleObjects(static_cast<DWORD>(m_mapLoadThread.size()+m_mapSaveThread.size()), &m_vecThreadQuitEvent[0], TRUE, INFINITE);
	...
	map<string, tagEntityBuildInfo*>::iterator deItr = m_mapObjAttrDef.begin();
...
{
	// 创建读线程
	for(DWORD i=0; i<GetGame()->GetSetup()->dwPlayerLoadThreadNumber; i++)
	{
	...
	// 创建写线程
	    for(DWORD i=0; i<GetGame()->GetSetup()->dwPlayerSaveThreadNumber; i++)
	{
    ...
    if(GetGame()->GetSetup()->bOpenRanks)
    {
    //    InitializeCriticalSection(&m_csRankInfo);

        InitRanksData();
        m_pRankThread = new DBRankThread(strProvider, strDataSource, 
            strInitialCatalog, strUserID, strPassword);
        if(m_pRankThread)
        {
            m_pRankThread->GetAdoBasePtr()->OpenCn();
            m_pRankThread->Begin();
        }
    }  
}   
...
CEntityGroup* CEntityManager::FindRootEntity(const string& flag, const CGUID& guid)
{
	...
	{
		map<CGUID, CEntityGroup*>::iterator enItr = itr->second.find(guid);
...
void CEntityManager::ProcessMsg(CMessage* pMsg)
{
 	...
 	if(DbOperFlag == 1)
	{
		...
		if( (strcmp(rootComFlag, "[account]") == 0) )
		{
			if(szRootName[0] == '\0')
			{
				Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_7",DbOperFlag,DBOperType));//ProcessMsg:[%d][%d] Account账号数据无效！
			...
			case DB_OPER_ONLY_MEMSAVE:
				{
					if(pFather)
					{
						...
						Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_8", szComFlag));//ProcMsg DB_OPER_DELETE_INSERT 添加子结点%s时未找到其父节点
				}
				break;
			case DB_OPER_DELETE:
				{
					if(pFather)
					{
						...
						Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_10", szComFlag));//ProcMsg DB_OPER_DELETE 添加子结点%s时未找到其父节点
				}
				break;
			case DB_OPER_INSERT:
				{
					if(pFather)
					{
						...
						Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_INFO_9", szComFlag));//ProcMsg DB_OPER_INSERT 添加子结点%s时未找到其父节点
				}
				break;
			case DB_OPER_SAVE:
				{
					if(pFather)
					{
						...
						Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_11", szComFlag));//ProcMsg DB_OPER_SAVE 添加子结点%s时未找到其父节点
				}
				break;
			case DB_OPER_LOAD:
				{
					if(pFather)
					{
						...
						Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_12", szComFlag));//ProcMsg DB_OPER_LOAD 添加子结点%s时未找到其父节点
				}
				break;
			default:
				{
					Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_13", rootComFlag, szComFlag, DBOperType));//ProcessMsg(): 根节点%s 子结点%s 的操作类型%d 无效!
					SAFE_DELETE_ARRAY(buf);
					return;
				}
				break;
		...
		else // 是根结点
		{
			...
			default: // Procedure
				{	
					...
					{
						Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_15", rootComFlag, DBOperType));//ProceMsg:%s type %d is err!
					}
					else
					{
						if(DbOperFlag == 1)
...
bool CEntityManager::PushMailThreadData(ThreadData* loadData)
{
	...
	lMails = static_cast<long>(m_listMailThreadData.size());
...
long CEntityManager::GetSaveThreadOperDataSize(list<CGUID>& saveBeginGuidList)
{
	...
	lRet = static_cast<long>(m_listSaveThreadOperData.size());
...
bool CEntityManager::PushLoadThreadData(ThreadData* loadData)
{
	...
	if(loadData && loadData->pEntity)
	{
		...
		lOperingLoadEntitys = static_cast<long>(m_listLoadThreadOperData.size());
		m_listLoadBeginEntityGuid.push_front(loadData->pEntity->GetGUID());
		lBeginLoadNum = static_cast<long>(m_listLoadBeginEntityGuid.size());
	}
...
bool CEntityManager::PushSaveThreadData(ThreadData* loadData)
{
	...
	if(loadData && loadData->pEntity)
	{
		...
		lOperingSaveEntitys = static_cast<long>(m_listSaveThreadOperData.size());
		m_listSaveBeginEntityGuid.push_back(loadData->pEntity->GetGUID());	
		lBeginSaveNum = static_cast<long>(m_listSaveBeginEntityGuid.size());
	}
	LeaveCriticalSection(&m_SaveCriticalSection);
	return true;
}
...
bool CEntityManager::GetThreadData(ThreadData** loadData, long dwThreadID, eThreadType type)
{
	...
	{
		Log4c::Trace(ROOT_MODULE,"%s",FormatText("GetThreadData() thread[%d] type is Init!", dwThreadID));
	}
	return true;
}
//! 弹出一个正在被线程处理的guid
bool CEntityManager::PopBeginEntityGuid(const CGUID& guid, eThreadType type)
{
	...
	{
		EnterCriticalSection(&m_LoadCriticalSection);
		list<CGUID>::iterator itr = find(m_listLoadBeginEntityGuid.begin(), m_listLoadBeginEntityGuid.end(), guid);
	...
	{
		EnterCriticalSection(&m_SaveCriticalSection);
		list<CGUID>::iterator itr = find(m_listSaveBeginEntityGuid.begin(), m_listSaveBeginEntityGuid.end(), guid);
	...
	{
		Log4c::Trace(ROOT_MODULE,"PopBeginEntityGuid() thread type is Init!");
	}
...
bool CEntityManager::PushRetThreadData(ThreadData* loadData)
{
	...
	if(loadData && loadData->pEntity)
	{
		if(loadData->pEntity->GetCurDbOperType() == DB_OPER_LOAD)
								 ...
								 const char* szTable)
{
	...
	// 检测是否存在
    CheckTableInDb(szTable, strNode);
}
...
void CEntityManager::InitPropertyIniData()
{
    ...
    /// 检测表是否存在
    CheckTableInDb("TBL_PLAYER_BASE", "[loginplayer]");
    ...
    LoadObjAttrInfo("structinfo\\goodsgroup_leaves\\[gp_equip_god].ini");
    LoadObjAttrInfo("structinfo\\goodsgroup_leaves\\[gp_equip_land].ini");
    LoadObjAttrInfo("structinfo\\goodsgroup_leaves\\[gp_equip_human].ini");
    ...
    /// 检测表是否存在
    CheckTableInDb("TBL_PLAYER_SPRITE", "[player_sprite]");
	...
	AttrInfoList.push_back( "structinfo/[agentorders].ini" );
	LoadObjInfo( AttrInfoList, NULL, "TBL_AGENTORDERS" );
	LoadObjAttrInfo("structinfo/[agentordersgroup].ini");
	AttrInfoList.clear();
    ...
    /// 检测表是否存在
    CheckTableInDb("TBL_FACTION_APPLY", "[faction_apply]");

	// 初始化Faction_Member对象
	LoadObjAttrInfo("structinfo\\[faction_member].ini");
	LoadObjAttrInfo("structinfo\\[factionmembergroup].ini");
    /// 检测表是否存在
    CheckTableInDb("TBL_FACTION_MEMBER", "[faction_member]");
    ...
    /// 检测表是否存在
    CheckTableInDb("TBL_UNION_BASE", "[union]");
    ...
    pRoot = GetGame()->GetEntityManager()->NewBaseEntity(string("[agentorders]"), NULL_GUID);
	CreateTblProcedure(pRoot);
	SAFE_DELETE(pRoot);
...
void CEntityManager::InitRanksData()
{
    list<string> listFileCfg;   
    list<string> listFileName;
    string strTempFolder("Ranks/*.*");
    CheckRFileStr(strTempFolder);
    string strTemp(strTempFolder);
    int nPos = (int)strTempFolder.find_last_of("\\");
    strTemp.erase(0, nPos+1);
    if(strTemp == "*.*")
    {
        strTempFolder.erase(nPos, strTempFolder.length()-nPos);
    }
    if(CClientResource::GetInstance()->IsFileExist(strTempFolder.c_str()))
    {
        string strTempExt(".rank");
        CClientResource::GetInstance()->FindFileList(strTempFolder, strTempExt, listFileName);
        strTempExt = ".ini";
        CClientResource::GetInstance()->FindFileList(strTempFolder, strTempExt, listFileCfg);
    }
    else
    {
        FindFile("Ranks/*.rank", ".rank", &listFileName);
        FindFile("Ranks/*.ini", ".ini", &listFileCfg);
    }

    for(list<string>::iterator itor = listFileName.begin(); itor != listFileName.end(); ++itor)
    {
        LoadRanksFile(itor->c_str());
    }

    for(list<string>::iterator itor = listFileCfg.begin(); itor != listFileCfg.end(); ++itor)
    {
        LoadObjAttrInfo(itor->c_str(), true);
    }

}

void CEntityManager::LoadRanksFile(const char* szPath)
{
    CRFile* prfile = NULL;
    if( NULL == szPath ||
    ...
    tagRankConfig stRankConfig;
    string strTmp;
    ReadTo(stream, "#");
    stream >> strTmp >> stRankConfig.lRankID;

    ReadTo(stream, "#");
    stream >> strTmp >> stRankConfig.lTopRecord;
    
    ReadTo(stream, "#");
    stream >> strTmp >> stRankConfig.lRefreshTime;
    ...
    while(1)
    {
        stream.getline(strContend,1024);
        strTmp = strContend;
        int iPos = (int)strTmp.find("<End>");
        if( iPos >= 0 )
            break;
        stRankConfig.strCurSQL += strContend;
        stRankConfig.strCurSQL += "\n";
    }   

    ReadTo(stream, "#");
    stream >> strTmp >> stRankConfig.strUpdateTable;
    while(1)
    {
        stream.getline(strContend,1024);
        strTmp = strContend;
        ...
        if( iPos >= 0 )
            break;
        stRankConfig.strUpdateSQL += strContend;
        stRankConfig.strUpdateSQL += "\n";
    }   

    m_mapRank[stRankConfig.lRankID] = stRankConfig;
    tagRankInfo *pRankInfo = new  tagRankInfo(stRankConfig.lRankID);
    m_mapRankInfo[pRankInfo->lRankID] = pRankInfo;
}

/// 检测表是否存在数据库中，否则创建该表
void CEntityManager::CheckTableInDb(const string& strTableName, const string& strInitName)
{
    if(m_pAccessThread)
    {
        _RecordsetPtr rs;
        ...
        try
        {
            m_pAccessThread->GetAdoBasePtr()->ExecuteRs(sql, rs);
        }
        catch(_com_error& e)
        {
            if(e.Error() == 0x80040E37) // DB_E_NOTABLE
            {
                ...
                CreateOtherTableInDb(strTableName, strInitName);
            }
        }
    }
}
...
void CEntityManager::LoadObjStructInfo(const char* szPath)
{
	...
	if( prfile )
	{
		...
		// 去掉文件夹路径
		size_t pos = deManagerName.find('[');
		deManagerName.erase(0, pos);
		pos = deManagerName.find('_');
		deManagerName.erase(pos, deManagerName.size());
		deManagerName.append("]");
		map<string, tagEntityBuildInfo*>::iterator itr = m_mapObjAttrDef.find(deManagerName);
		if(itr == m_mapObjAttrDef.end())
		{
			Log4c::Trace(ROOT_MODULE,"%s",FormatText("DB_ENTITYMGR_14",deManagerName.c_str()));//未先创建该对象[%s]根结点!
			return;
		}
...
/*
*   参数 
*   bCheckTable true:需要检测表是否存在数据库中，不存在则创建
*   bSend2WS false: 只检测创建表，不将该表信息缓存至表信息列表中,并需要释放临时对象指针
*/
const char* CEntityManager::LoadObjAttrInfo(const char* szPath, bool bCheckTable)
{
	CRFile* prfile = NULL;
	if( NULL == szPath ||
		!(prfile = rfOpen(szPath)) ) 
    ...
    if(bCheckTable)
    {
        CheckTableInDb(entityBuildInfo->strTblName, deManagerName);
    } 
    
	return deManagerName.c_str();
}
//!	创建复合索引
void CEntityManager::CreateOtherTableComIndexInDb(const string& tableName, vector<tagDbIdxInfo>& vecIdx)
{
...
void CEntityManager::CreateOtherTableIndexInDb(const string& tableName, vector<tagDbIdxInfo>& vecIdx, bool isClustered)
{
	string strCreateTableSql;
...
void CEntityManager::CreateOtherTableInDb(const string& tableName, const string& otherIniName)
{
	...
	map<string, tagEntityBuildInfo*>::iterator itr = m_mapObjAttrDef.find(otherIniName);
	if(itr != m_mapObjAttrDef.end())
...
void CEntityManager::ExecSqlString(const string& sqlStr)
{
	...
	{
        m_pAccessThread->GetAdoBasePtr()->PrintErr("ExecSqlString() Err!", e);
		return;
	}
}
...
void CEntityManager::CreateTblProcedure(CBaseEntity* pRoot)
{
    ...
    CreateProcedure(strInsertProcName, strProcData);
    ...
    // 创建存储过程
    CreateProcedure(strUpdateProcName, strProcData);	
	
	AddTblProcedure(pRoot, DB_OPER_SAVE, strUpdateProcName);
}

/// create procedure
void CEntityManager::CreateProcedure(const string& strProcedureName, const string& strCreateSQL)
{
    string strClearSQL = "SET ANSI_NULLS ON\r\n\
                         SET QUOTED_IDENTIFIER ON\r\n\
                         \r\n\
                         IF EXISTS (\r\n\
                         SELECT * \r\n\
                         FROM INFORMATION_SCHEMA.ROUTINES\r\n\
                         WHERE SPECIFIC_NAME = N'"+ strProcedureName + "'\r\n\
                         )\r\n\
                         DROP PROCEDURE "+ strProcedureName +";\r\n";
    try
    {
        // clear procedure first
        bool bRet = m_pAccessThread->GetAdoBasePtr()->ExecuteCn(strClearSQL.c_str());
        if(bRet)
        {
            // then recreate the procedure
            bRet = m_pAccessThread->GetAdoBasePtr()->ExecuteCn(strCreateSQL.c_str());
        }
    }
    catch (_com_error& e)
    {
       Log4c::Trace(ROOT_MODULE,"CreateProcedure() error!! Procedure's name[%s]", strProcedureName.c_str());
        m_pAccessThread->GetAdoBasePtr()->PrintErr(strProcedureName.c_str(), e);
    }
}
...
void CEntityManager::RunDeleteTimeoutMailProc(void)
{
	...
	{
        m_pAccessThread->GetAdoBasePtr()->PrintErr("Run DeleteTimeoutMailProc error！", e);
		Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_1"));//运行清除超时邮件存储过程失败
		return;
	}
	Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_0"));//运行清除超时邮件存储过程成功!
}
		...
		else
		{
			...
			if(enItr != itr->second.end())
			{
				...
				else
				{
					...
					Log4c::Trace(ROOT_MODULE,"AddOneRootEntity:old ptr[%s],type[%s] has found.", szGuid, entity->GetCompositeFlag().c_str());
				}
	...
	if(acc)
	{
		...
		if(szCdkey != "")
		{
			...
			if(itr != m_mapAccounts.end())
			{
				...
				{
					Log4c::Trace(ROOT_MODULE,"AddAccount: old ptr[%s] has found.", szCdkey);
				}
			}
			else
			{
		...
		{
			Log4c::Warn(ROOT_MODULE,"AddAccount: szCdkey is NULL.");
		}
	}
	return false;
}
...
Account* CEntityManager::FindAccount(const string& strCdkey)
{
	// 现在vector中查找是否已经存在
...
void CEntityManager::LoadProcedureInfo(const char* szPath)
{
	if(szPath)
	{
		...
		if(file)
		{
            ...
            // 创建存储过程
            CreateProcedure(procName, strCreatePro);
...
BYTE CEntityManager::GetDbUseageFlag(const string& strComFlag, const string& strName)
{
	...
	if(itr != m_mapObjAttrDef.end())
	{
		map<string, tagEntityAttrInfo>::iterator attrItr = itr->second->pDataMgr.find(strName);
		if(attrItr != itr->second->pDataMgr.end())
			return static_cast<BYTE>(attrItr->second.lUseage);
	}
	return 0;
}
...
long CEntityManager::GetLoadThreadDataSize(void)
{
	long lRet = 0;
	EnterCriticalSection(&m_LoadCriticalSection);
	lRet = static_cast<long>(m_listLoadThreadOperData.size());
	LeaveCriticalSection(&m_LoadCriticalSection);
	return lRet;
}
//! 遍历所有对象，产生消息
...
{
	static long lMaxThreadDataNum  = static_cast<long>(m_mapSaveThread.size());
	...
	if(GetSaveThreadOperDataSize(tmpBeginGuid) < lMaxThreadDataNum)
	{
		...
		if(m_listDirtyEntityName.size() != 0)
		{
			...
			while(accItr != m_listDirtyEntityName.end())
			{
				...
				// 判断是否有重复
				list<string>::iterator tmpCheckItr = find(tmpCheckNameList.begin(), tmpCheckNameList.end(), *accItr);
		...
		if(m_listDirtyEntityGuid.size() != 0)
		{
			...
			while(guidItr != m_listDirtyEntityGuid.end())
			{
				if(lCurTravelNum == GetGame()->GetSetup()->dwTravleMaxSize) break;
				// 判断是否在线程处理中
				list<CGUID>::iterator threadGuidItr = find(tmpBeginGuid.begin(), tmpBeginGuid.end(), *guidItr);
				...
				// 判断是否有重复
				threadGuidItr = find(tmpCheckGuidList.begin(), tmpCheckGuidList.end(), *guidItr);
				...
				while(engroupitr != m_mapEntitys.end())
				{
					...
					map<CGUID, CEntityGroup*>::iterator enItr = engroupitr->second.find(*guidItr);
...
void CEntityManager::ProcessRetThreadData(void)
{
	...
	for(; itr != retDataList.end(); itr++)
	{
		...
		if(!pBaseEntity)
		{
			Log4c::Warn(ROOT_MODULE,"ProcRetData() entity ptr is null!");
			continue;
		}
		...
		if((*itr)->retFlag != S_OK)
		{
			...
			{
				// 重新存盘一次
				CEntityGroup* player = (CEntityGroup*)NewBaseEntity(string("[player]"), pBaseEntity->GetGUID());
				CEntityGroup* tplayer = FindRootEntity(string("[player]"), pBaseEntity->GetGUID());
				if(player && tplayer)
				{
				...
				// 重新计时删除时间
				const char* szCdkey = pBaseEntity->GetStringAttr(string("Account"));
				if(szCdkey)
				{
		...
		if(pBaseEntity->GetCurDbOperType() == DB_OPER_CREATEROLE_PROCEDURE)
		{
			...
			if(pBaseEntity->GetRetQueryDataVec().size() == 2)
			{
					...
					if( NameIsExist == 0 && charNum == 1 )
					{
						...
						if(szCdkey)
						{
							...
							if(pAcc)
							{
								...
								lgitr++;
								lgitr->second->SetGUID(tmpGuid);
								CEntityGroup* pPlayer = GetGame()->GetEntityManager()->FindRootEntity(string("[player]"), tmpGuid);
								if(!pPlayer)
								{
				...
				else
				{
					map<CGUID, CBaseEntity*>::iterator lgitr = ((CEntityGroup*)pBaseEntity)->GetEntityGroupMap().begin();
					const char* szCdkey = lgitr->second->GetStringAttr(string("Account"));
					Log4c::Trace(ROOT_MODULE,"CreatePlayer: [%s], [%d] failed!", szCdkey, (*itr)->retFlag);
					pBaseEntity->ReleaseChilds();
				}
				// 发送消息
				CMessage msg(MSG_DB2W_OPER_ENTITY);
		...
		else
		{
			...
			if(pBaseEntity->GetCurDbOperType() == DB_OPER_LOAD)
			{
				if((*itr)->retFlag == S_OK)
				{
					...
					{
						CEntityGroup* pPlayer = GetGame()->GetEntityManager()->FindRootEntity(string("[player]"), pBaseEntity->GetGUID());
						if(!pPlayer)
						{
...
{
	list<CGUID>::iterator itr = find(m_listDirtyEntityGuid.begin(), m_listDirtyEntityGuid.end(), guid);
...
void CEntityManager::AddDirtyEntityName(const char* szName)
{
	if(szName)
	{
		...
		list<string>::iterator itr = find(m_listDirtyEntityName.begin(), m_listDirtyEntityName.end(), tmpStr);
...
void CEntityManager::SaveAllEntity(void)
{
	...
	Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_2"));//保存玩家详细数据成功(等待线程结束)！
	...
	Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_3"));//保存玩家基本数据成功(等待线程结束)！
	...
	Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_4"));//保存全局变量数据成功(等待线程结束)！
	...
	Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_5"));//保存帮会数据成功(等待线程结束)！
...
bool CEntityManager::ClearEntitys(void)
{
	...
	for(; accItr != m_mapAccounts.end(); accItr++)
		delete accItr->second;
	m_mapAccounts.clear();
	Log4c::Trace(ROOT_MODULE,GetText("DB_ENTITYMGR_6"));//清除全部缓存数据成功！
	return true;
}
...
{
	list<string>::iterator itr = find(m_listDirtyEntityName.begin(), m_listDirtyEntityName.end(), strAcc);
	if(itr != m_listDirtyEntityName.end())
		return true;
	return false;
}
bool CEntityManager::FindEntityIsInOperQueue(const CGUID& guid)
{
	...
	itr = find(m_listSaveBeginEntityGuid.begin(), m_listSaveBeginEntityGuid.end(), guid);
	if(itr != m_listSaveBeginEntityGuid.end())
		return true;
...
// 释放排行数据信息
void CEntityManager::ReleaseRankInfo()
{
...
void CEntityManager::ClearRankInfo(long lRankID)
{
...
// 刷新排行信息
void CEntityManager::ReLoadRanks()
{
    if(m_pRankThread)
    {
        m_pRankThread->SetReLoad(true);
    }
}
