...
BOOL GameFaction::AddToByteArray(vector<BYTE>& ByteArray)
{
	...
	{
		_AddToByteArray(&ByteArray, &m_vIconData[0], (long)m_vIconData.size());
	}
	...
	////! 留言列表
	//_AddToByteArray(&ByteArray, (LONG)m_vFacLeaveWord.size());
	//for (LONG i = 0; i < m_vFacLeaveWord.size(); ++i)
	//{
	//	_AddToByteArray(&ByteArray, &(m_vFacLeaveWord[i]), sizeof(tagOrgLeaveWord));
	//}
...
BOOL GameFaction::DecodeFromMsgSet(DBReadSet &DbReadSet, map<CGUID, CGUID> *pMapApplicant)
{
	...
	if(0 != lIocnSize)
	{
		...
		CCrc32Static::DataCrc32(&m_vIconData[0], (long)m_vIconData.size(), m_dwCRC32IconCode);
	}
	else
	{
		m_dwCRC32IconCode = 0;
...
void GameFaction::AddIconData_ToArray(vector<BYTE> *pByteArray)
{
	...
	if(0 != m_vIconData.size())
		_AddToByteArray(pByteArray, &m_vIconData[0], (long)m_vIconData.size());
}
...
{
	m_FacBaseData.lMemberNum = (long)m_mapMemInfo.size();
	pMsg->AddEx(&m_FacBaseData, (long)sizeof(tagFacBaseData));
}
...
void GameFaction::GetApplyInfo(tagApplyInfo &ApplyInfo)
{
	...
	//! 成员数量
	ApplyInfo.lMemberNum		= (long)m_mapMemInfo.size();
	//! 最大成员数量
...
void GameFaction::OnBoonRun(void)
{
	...
	for ( ; ite != m_mapMemInfo.end(); ++ite)
	{
		...
		if(NULL != pPlayer)
		{
			...
			DWORD dwAddExp = (DWORD)(pFacExpLevelSetup->fBaseExp * PlayerList::GetFacBoonExpScale(pPlayer->GetLevel()) + 0.5);
...
BOOL GameFaction::MemberOnlineChange(const CGUID& MemberGuid, BOOL bOnline, LONG lLastOnlineTime)
{
	...
	if(m_mapMemInfo.end() != ite)
	{
        ite->second.bIsOnline = bOnline ? false : true;
...
LONG GameFaction::GetBaseBattle(const char* cBaseName)
{
	map<string,tagBaseInfo>::iterator it=m_mapBaseInfo.find(cBaseName);
	if (it!=m_mapBaseInfo.end())
	{
...
void GameFaction::SetJoin(CGUID& PlayerGuid,LONG lFlags)
{
	...
	if (it!=m_mapMemInfo.end())
	{
        it->second.bJoinBattle = lFlags ? true : false ;
		//更新成员资料
...
LONG GameFaction::GetTickets(const char* cBaseName)
{
	map<string,tagBaseInfo>::iterator it=m_mapBaseInfo.find(cBaseName);
	if (it!=m_mapBaseInfo.end())
	{
