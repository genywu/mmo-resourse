...
#define CHECK_ENABLE if( !GetInst( CConfiger ).IsEnable() ) return
	...
	void CPlayerManager::Release()
	{
			...
			it != m_playerTable.end(); ++ it )
		{
			...
			OBJ_RELEASE( CPlayerProperty, it->second );
		}
		m_playerTable.clear();
	}
	...
	{	
		...
		if (playerpro != NULL)
		{
			...
			/// 释放对象空间
			OBJ_RELEASE(CPlayerProperty, playerpro);
			/// 删除map资源
	...
	void CPlayerManager::PlayerDecode( CPlayer *player, DBReadSet &db )
	{
		...
		const CGUID &playerid = player->GetExID();
		CPlayerProperty *playerpro = OBJ_CREATE_PVOID( CPlayerProperty, (void*)(&player->GetExID()) );
		...
		{
			OBJ_RELEASE(CPlayerProperty, itePlayer->second);
