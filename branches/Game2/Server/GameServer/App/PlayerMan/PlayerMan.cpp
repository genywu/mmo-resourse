...
void CPlayerMan::Release(void)
{
	...
	{
		OBJ_RELEASE(tagPlayerInfo, itr_Map->second);
	}
...
bool CPlayerMan::AddPlayer(CPlayer* pPlayer)
{
	...
	{
		pInfo = OBJ_CREATE(tagPlayerInfo);
	}

	if( NULL == pInfo )
	{
		Log4c::Trace(ROOT_MODULE, "玩家(%s)信息为NULL.",pPlayer->GetName());
		return false;
	}
...
bool CPlayerMan::DelPlayer(CPlayer* pPlayer)
{
	...
	if( NULL == pInfo )
	{
		...
		Log4c::Trace(ROOT_MODULE, "玩家(%s %s)信息为NULL.",pPlayer->GetName(), strName);
							 ...
							 CPlayer* pPlayer)
{
	...
	default:
		{
			Log4c::Trace(ROOT_MODULE, "TYPE:0X%X, 玩家%s PLAYER_ELEM_TYPE非法.",eType,pPlayer->GetName());
			return -1;
		}
	}
							  ...
							  CPlayer* pPlayer)
{
	if( NULL == pPlayer )
	{
		Log4c::Trace(ROOT_MODULE, "NULL == pPlayer.");
		return false;
	}
	...
	if( NULL == pInfo )
	{
		Log4c::Trace(ROOT_MODULE, "TYPES(0X%x) : 玩家(%s)信息为NULL.",eTypes, pPlayer->GetName());
		return false;
	}
...
bool CPlayerMan::ChangeGS(const CGUID& rGuid)
{
	...
	if( NULL == pInfo )
	{
		char strName[64];
		rGuid.tostring(strName);
		Log4c::Trace(ROOT_MODULE,"玩家(%s)信息为NULL.",strName);
		return false;
	}
...
bool CPlayerMan::Query(CMessage* pMsg)
{
	...
	if( NULL == pInfo )
	{
		Log4c::Warn(ROOT_MODULE,"通信异常: Player(%s)信息为NULL, MsgType : %d",pPlayer->GetName(),lMsgType);
		return false;
	}

	if ( pPlayer->GetLevel() < static_cast<unsigned short>(GlobalSetup::GetSetup()->lMLevelEnablePlayerSearch) )
	{
		char szGUID[128];
		pPlayer->GetExID().tostring(szGUID);
		Log4c::Warn(ROOT_MODULE, "玩家信息非法搜索: 玩家:'%s'[ GUID:%s 等级:%d ],因等级太低限制其玩家信息搜索 ",pPlayer->GetName(), szGUID, pPlayer->GetLevel());
		return false;
	}
	...
	if( PRT_CUR_REGION == lVal )			// 当前场景
	{
		...
		{
			Log4c::Warn(ROOT_MODULE,"查找异常: 场景(%d)没有玩家.",lVal);
			return false;
		}
	...
	else if( PRT_CUR_AREA == lVal )				// 临近区域
	{
		...
		for( long i=-1; i<2; ++i )
		{
			for( long j=-1; j<2; ++j )
			{
				...
				{
#ifdef _DEBUG
					Log4c::Warn(ROOT_MODULE,"查找异常: 区域(%d)没有玩家.",lTemp);
#endif
				}
				else
				{
	...
	while( itrGUID!=PlayerList.end() )
	{
		...
		if( itrPlayer == m_mapPlayers.end() ||
			...
			GMList::GetInfoByName( pInfo->strName.c_str() ) )
		{
...
bool CPlayerMan::FillPlayerInfo(CPlayer* pPlayer, tagPlayerInfo& rPlayerInfo)
{
	if( NULL == pPlayer )
	{
		Log4c::Warn(ROOT_MODULE,"玩家为NULL.");
		return false;
	}
...
bool CPlayerMan::DelPlayerMap(tagPlayerInfo* pInfo, bool bComplete)
{
	if( NULL == pInfo )
	{
		Log4c::Warn(ROOT_MODULE, "玩家(信息)为NULL.");
		return false;
	}

#ifdef _DEBUG
	Log4c::Trace(ROOT_MODULE,"删除玩家成功: %s, %d, %d, %d, %d: %d, %d. %d.",
	...
	{
		pInfo->ExID = ExID;
		OBJ_RELEASE(tagPlayerInfo, pInfo);
