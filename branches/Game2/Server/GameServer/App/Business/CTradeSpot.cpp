...
namespace Business
{
	...
	void CTradeSpot::ReleaseGoodsTable()
	{
		...
		{
			OBJ_RELEASE(CBusinessGoods, it->second);
		}
		m_GoodsTable.clear();
	}
	...
	void CTradeSpot::DecodeFromDataBlock( DBReadSet &db )
	{
		...
		for( long i = 0; i < lGoodsSize; ++ i )
		{
			...
			DWORD dwIndex = GoodsSetup::QueryGoodsIDByOriginalName( goods.strOrigName );
			if( dwIndex == 0 )
			{
				// not exist
				Log4c::Warn(ROOT_MODULE,FormatText( "Goods [%s] does NOT exist in goodslist.dat", goods.strOrigName ) );
				continue;
			}
			GoodsTemplateTable::iterator it = m_GoodsTable.find( dwIndex );
			if( it == m_GoodsTable.end() )
			{
				CBusinessGoods::tagParam param;
				param.bp = &goods;
				param.pSpot = this;
				CBusinessGoods *pGoods = OBJ_CREATE_PVOID(CBusinessGoods, (void*)&param );
	...
	void CTradeSpot::BuildGoodsCategory()
	{
		...
		for( GoodsTemplateTable::iterator it = m_GoodsTable.begin(); it != m_GoodsTable.end(); ++ it )
		{
			...
			{
				Log4c::Warn(ROOT_MODULE,FormatText( "Goods [%d] is not business goods.", it->first ) );
			}
			else
			{
	...
	void CTradeSpot::GetGoodsList( CPlayer *pPlayer, DBWriteSet &db )
	{
		...
		for( GoodsTemplateTable::iterator it = m_GoodsTable.begin(); it != m_GoodsTable.end(); ++ it )
		{
			if( IsGoodsVisible( it->second, pPlayer ) )
			{
				...
				goods.PriceScale = (int)( goods.dwPrice / (float)pGoods->GetBaseProperty().dwBasePrice ) * 100;
		...
		{
			db.AddToByteArray( &goods_list[0], (long)(goods_list.size() * sizeof( tagGoodsClient ) ) );
		}
	}
	...
	void CTradeSpot::GetGoodsChecking( CPlayer *pPlayer, DBWriteSet &db )
	{
		...
		for( GoodsTemplateTable::iterator it = m_GoodsTable.begin(); it != m_GoodsTable.end(); ++ it )
		{
			if( IsGoodsVisible( it->second, pPlayer ) )
			{
				...
				goods.iPriceScale = (int)( goods.dwPrice / (float)pGoods->GetBaseProperty().dwBasePrice ) * 100;
		...
		for( DWORD i = 0; i < dwVolume; ++ i )
		{
			...
			if( pGoods != NULL )
			{
				...
				if( pBusinessGoods != NULL )
				{
					...
					goods.iPriceScale = (int)( goods.dwPrice / (float)pBusinessGoods->GetBaseProperty().dwBasePrice ) * 100;
	...
	void CTradeSpot::GetGoodsSellPrice( CPlayer *pPlayer, DBWriteSet &db )
	{
		...
		{
			db.AddToByteArray( &price_list[0], (long)(price_list.size() * sizeof( tagGoodsSellClient ) ) );
		}
	}
	...
	CBusinessGoods *CTradeSpot::GetGoods( int id )
	{
		...
		Log4c::Warn(ROOT_MODULE,FormatText( "Cannot find the goods by id [%d]", id ) );
		return NULL;
	}
	...
	void CTradeSpot::DecreaseGoodsVolume( CBusinessGoods *pGoods, DWORD dwAmount )
	{
		...
		for( GoodsTemplateTable::iterator it = m_GoodsTable.begin(); it != m_GoodsTable.end(); ++ it )
		{
			...
			{
				// 对同类商品库存的影响
				int sub = (int)(( 1.0f * goods_property.dwBasePrice / pOtherGoods->GetBaseProperty().dwBasePrice ) * dwAmount *
	...
	void CTradeSpot::IncreaseGoodsVolume( CBusinessGoods *pGoods, DWORD dwAmount )
	{
		...
		for( GoodsTemplateTable::iterator it = m_GoodsTable.begin(); it != m_GoodsTable.end(); ++ it )
		{
			...
			{
				// 对同类商品库存的影响
				int add = (int)(( 1.0f * goods_property.dwBasePrice / pOtherGoods->GetBaseProperty().dwBasePrice ) * dwAmount *
					m_Property.fCategoryVolumeQuo);
	...
	void CTradeSpot::EncodeToSave( DBWriteSet &db )
	{
		...
		if( tmpBuf.size() > 0 )
		{
			db.AddToByteArray( &tmpBuf[0], (long)tmpBuf.size() );
		}
	...
	void CTradeSpot::DecodeGoodsDBProperty( DBReadSet &db )
	{
		...
		if( buf_size > 0 )
		{
			...
			for( long i = 0; i < goods_count; ++ i )
			{
				...
				{
					Log4c::Warn(ROOT_MODULE, FormatText( "Load invalid goods [%d] property from database", goods_index ) );
				}
			} // for
		} // if
	}
