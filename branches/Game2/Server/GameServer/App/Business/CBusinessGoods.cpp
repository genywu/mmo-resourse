...
namespace Business
{
	...
	CBusinessGoods::CBusinessGoods(void* pParam)
		: m_pFather(((tagParam*)pParam)->pSpot)
	{
		m_PriceState = PRICE_EQUAL;
		SetBaseProperty( (*((tagParam*)pParam)->bp) );
	}
	...
	void CBusinessGoods::SetBaseProperty( const tagGoods &bp )
	{
		...
		m_Index = GoodsSetup::QueryGoodsIDByOriginalName( bp.strOrigName );
	...
	void CBusinessGoods::UpdatePrice()
	{
		...
		if( m_Volume > 0 && m_Volume <= m_MaxVolume )
		{
			...
			m_dwBuyPrice = (DWORD)(m_BaseProperty.dwBasePrice * fBuyAddonQuo);

			float fSellAddonQuo = m_BaseProperty.fSellMaxQuo - m_dwSellCount * m_fSellQuo;
			m_dwSellPrice= (DWORD)(m_BaseProperty.dwBasePrice * fSellAddonQuo);
		}
		else if( m_Volume <= 0 && m_Volume > m_BaseProperty.dwMinVol )
		{
			...
			m_dwBuyPrice = (DWORD)(fBuyAddon * m_BaseProperty.dwBasePrice );
		}
		else if( m_Volume > m_MaxVolume && m_Volume <= (int)m_BaseProperty.dwMaxVol )
		{
			// 高于库存 (最大库存容量, 承载量max]
			...
			m_dwSellPrice = (DWORD)(fSellAddon * m_BaseProperty.dwBasePrice);
		}
		else if( m_Volume > (int)m_BaseProperty.dwMaxVol )
		{
			// 暴跌，公式和高于库存相同
