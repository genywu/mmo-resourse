...
CExState2::CExState2( tagExState &stState )
{
	CState();
	//##
	m_lType				= STATE_TYPE_STANDARD;
	m_lID				= SKILL_EX_STATE_2;
...
DWORD CExState2::GetRemainedTime()
{
...
BOOL CExState2::OnUpdateProperties()
{
...
BOOL CExState2::Begin( CMoveShape* pUser, CMoveShape* pSufferer )
{
	if( pSufferer == NULL )
		return FALSE;
	CState::Begin( pUser, pSufferer );
	m_pCurrentVisualEffect = new CExState2VisualEffect;
	m_pCurrentVisualEffect -> BeginVisualEffect( TRUE );
	return TRUE;
}

BOOL CExState2::Begin( CMoveShape* pUser, LONG lX, LONG lY )
{
...
BOOL CExState2::Begin( CMoveShape* pUser, OBJECT_TYPE otType, LONG lID, LONG lRegion )
{
...
VOID CExState2::OnChangeRegion( LONG lRegionID )
{
	//##这个状态存在于User身上.
...
VOID CExState2::Serialize( vector<BYTE>& vOut )
{
	//##是否需要将技能的id和等级都压入？
...
VOID CExState2::Unserialize( BYTE* pBytes, LONG& lPos )
{
...
VOID CExState2VisualEffect::UpdateVisualEffect( CState* pParent, DWORD dwCmd )
{
	if( pParent && !m_bEnded )
	{
