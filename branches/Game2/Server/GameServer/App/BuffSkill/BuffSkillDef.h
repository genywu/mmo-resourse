...
{
     SKILL_ATTTYPE_NO=0,
     SKILL_ATTTYPE_OUTENERGY=1,///< 外功
     SKILL_ATTTYPE_ENERGY,     ///< 内功
     SKILL_ATTTYPE_CURSE,      ///< 诅咒
     SKILL_ATTTYPE_POX,        ///< 瘟疫
     SKILL_ATTTYPE_ILLNESS,    ///< 疾病
     SKILL_ATTTYPE_POISON,     ///< 毒
     SKILL_ATTTYPE_FIRE,       ///< 火焰(火)
     SKILL_ATTTYPE_SHADOW,     ///< 暗影(金)
     SKILL_ATTTYPE_FROST,      ///< 冰霜(水)
     SKILL_ATTTYPE_NATURE,     ///< 自然(木)
     SKILL_ATTTYPE_HOLINESS,   ///< 神圣(土)
};
...
struct stStaticModuParam
{
	...
	long lConsMp;
	//最小攻击距离
	float MinAtkDistance;
	//最大攻击距离
	float MaxAtkDistance;
	//动作时间
	long lActTime;
	//
	list<long> lBuffID;
...
struct stModuParam
{
    ...
    //发送技能状态Buff列表
    hash_set<CGUID,hash_guid_compare> SendGeniusList;
	//根据TimerID找到相应的参数
    ...
    eSkillAttackType    GetAttackType()     {return eAttackType;}   //技能攻击类型
	...
	VariableList*		GetVariableList()	{return pVariableList;}	//变量列表	
	...
	void Clear();
    //计算冷却时间
    void CalcCoolDown(CMoveShape* shape,long id,long lvl);
    //阶段减少
	void ProcNumSub();
	// 添加到CByteArray
