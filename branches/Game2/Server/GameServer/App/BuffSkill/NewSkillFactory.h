...
class CNewSkillFactory //:public CGameEvent
{
...
private:
	...
	typedef map<string,VariableList*>::iterator				    itVariableList; 
	static map<string,VariableList*>							g_mapVariableList;	
...
public:
    ...
    //初始化天赋
    static  void                                    InitGeniusChange(CNewSkillProperties::tagGeniusChange*ptgGeniusChange,vector<COperator*>&vOperator);
	//初始化具体条件
    ...
    ///一个具体条件
	static  CCondition								*InitCondition(DWORD dwID,DWORD dwLv,DWORD dwConditionID,bool vRet=false);	//条件
    ...
    static  CConOdds                                *InitOddsCondition(CNewSkillProperties::tagCondition *ptgCondition);        //几率条件
    static  CConDispel                              *InitDispelCondition(CNewSkillProperties::tagCondition *ptgCondition);      //驱散条件
	...
	static CMPFUN									*GetCmpFun(string name,long type=0);
    ...
    static eSkillAttackType                         GetSkillAttType(string type);
	...
	static VariableList							    *GetVariableList( const string &strFileName);
	static void										AddVariableList( const string &strFileName, VariableList *pVariableList);
	//计算hash值
