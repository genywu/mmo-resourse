	...
	case MSG_W2S_RGN_VIEWRGNVAR:
		{
			...
			if( script != NULL )
			{
				...
				{
					double val = (double)pMsg->GetLONG64();
					script->PushRetValue( val );
				}
	...
	case MSG_W2S_RGN_VAR_GET:
		{
			...
			char varName[128];
			pMsg->GetStr( varName, sizeof( varName ) );
			Script *script = GetInst( ScriptSys ).GetScript( scriptID );
			if( script != NULL )
			{
				long varType = VarList::VarType( varName );
				...
				else if( varType == VariableList::VAR_NUM )
				{
					CGUID val;
					pMsg->GetGUID( val );
					script->PushRetValue( val );
	...
	case MSG_W2S_RGN_VAR_GET_QUEST:
		{
			...
			if(rgnManager)
			{
				...
				if(rgn)
				{
					...
					//		msg.Add("");
					//}
					long varType = VarList::VarType( rgnVarName );
					if( varType == VariableList::VAR_NUM )
					{
						double val = rgn->GetVariableList()->GetVarValue<double>( rgnVarName );
						msg.AddEx( &val, sizeof( val ) );
					}
					else if( varType == VariableList::VAR_STRING )
					{
						const tString &val = rgn->GetVariableList()->GetVarValue<tString>( rgnVarName );
						msg.Add( val.Str() );
					}
					else if( varType == VariableList::VAR_GUID )
					{
						const CGUID &val = rgn->GetVariableList()->GetVarValue<CGUID>( rgnVarName );
						msg.Add( val );
					}
					// ??
					msg.Add( retVarName );
	...
	case MSG_W2S_RGN_VIEWRGNDATA:
		{
			...
			if(flag == 1)
			{
				...
				if( script != NULL )
				{
					...
					{
					case 0:
						script->PushRetValue( gsid );
						break;
					case 1:
						script->PushRetValue( rgnType );
						break;
					case 2:
						script->PushRetValue( pos );
						break;
					case 3:
						script->PushRetValue( rgnID );
						break;
					case 4:
						...
						break;
					case 5:
						script->PushRetValue( name );
						break;
					case 6:
						script->PushRetValue( ownerID );
						break;
					}
					GetInst( ScriptSys ).ResumeScript( script, 1 );
				}
