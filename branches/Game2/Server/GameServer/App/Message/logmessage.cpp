	...
	case MSG_W2S_LOG_ANSWER_PLAYERDATA://worldserver返回玩家的详细属性
		{
			...
			if (lFlag==0)
			{
				...
				Log4c::Trace(ROOT_MODULE,"返回玩家的详细属性时 (%s) 没有正常登陆,已被勒令断开!", szPlayerID);
			...
			if (lFlag==-1)
			{
				...
				Log4c::Trace(ROOT_MODULE,"返回玩家的详细属性时 (%s) 该玩家在线!", szPlayerID);
				GetGame()->GetNetServer()->DisConn(lC2SSocketID);
				return;
			}
			...
			if (lFlag==-2)
			{
				...
				Log4c::Warn(ROOT_MODULE,"返回玩家的详细属性时 (%s) GameServer非法!", szPlayerID);
				GetGame()->GetNetServer()->DisConn(lC2SSocketID);
				return;
			}
