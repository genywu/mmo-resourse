...
void CBillClient::SendAccountAuthen( tagAccAuthenReq *pReq, CCallbackAccountAuthen *pCallback )
{
	...
	if (nRet != 0)
	{
        Log4c::Error(ROOT_MODULE,"%-15s OI:%s, CI:%s, %d : 发送Player %s AccountAuthen消息失败!",__FUNCTION__,req.order_id, "0", nRet, req.user_id );
		return;
	}
...
void CBillClient::SendAccountLock( tagAccLockReq *pReq, CCallbackAccountProcess *pCallback )
{
	...
	if (nRet != 0)
	{
        Log4c::Error(ROOT_MODULE,"%-15s OI:%s, CI:%s, %d : 发送Player %s AccountLock消息失败!",
            __FUNCTION__,req.order_id, req.context_id, nRet, req.user_id );
		return;
	}
...
void CBillClient::SendAccountUnlock( tagAccUnlockReq *pReq, CCallbackAccountProcess *pCallback )
{
	...
	if (nRet != 0)
	{
        Log4c::Error(ROOT_MODULE,"%-15s OI:%s, CI:%s, %d : 发送Player %s AccountUnlock消息失败!",
            __FUNCTION__,req.order_id, req.context_id, nRet, req.user_id );
		return;
	}
...
void CBillClient::SendConsignTransfer( tagAccUnlockReq *pReq, CCallbackAccountProcess *pCallback )
{
	...
	if (nRet != 0)
	{
        Log4c::Error(ROOT_MODULE,"%-15s OI:%s, CI:%s, %d : 发送Player %s ConsignTransfer消息失败!",
            __FUNCTION__,req.order_id, req.context_id, nRet, req.user_id_src );
		return;
	}
...
void CBillClient::SendAwardAuthen( tagAwdAuthenReq *pReq, CCallbackAwardAuthen *pCallback )
{
	...
	if (nRet != 0)
	{
        Log4c::Error(ROOT_MODULE,"%-15s OI:%s, CI:%s, %d : 发送Player %s AwardAuthen消息失败!",
            __FUNCTION__,req.order_id, req.context_id, nRet, req.user_id );
		return;
	}
...
void CBillClient::SendAwardAck( tagAwdAckReq *pReq, CCallbackAwardAck *pCallback )
{
	...
	if (nRet != 0)
	{
        Log4c::Error(ROOT_MODULE,"%-15s OI:%s, CI:%s, %d : 发送Player %s AwardAck消息失败!",
            __FUNCTION__,req.order_id, req.context_id, nRet, req.user_id );
		return;
	}
