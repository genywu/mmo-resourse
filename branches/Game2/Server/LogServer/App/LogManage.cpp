...
BOOL LogManage::AffirmClientNo(LONG dClientNo, LONG lClientIp, LONG lSocket)
{
	for (DbInfoIte ite = m_vecDbInfo.begin(); ite != m_vecDbInfo.end(); ++ite)
	{
		if(ite->m_worldNo == dClientNo)
		{
			...
			AppFrame::DelFromList("clients", szClientInfo);
...
{
	for (size_t i = 0; i < m_vecDbInfo.size(); ++i)
	{
...
BOOL LogManage::AddData(LONG lDbID, LONG lTableType, BYTE* pData)
{
	...
	//char szDbIP[32] = {0};
	char szDbID[32] = {0};
	//sprintf(szDbIP, "%d", pLogDbInfo->m_lClientIp);
	sprintf(szDbID, "%d", lDbID);
	strValueList = /*string(szDbIP) + string(",") + */string(szDbID);

	//! 处理客户端传过来的数据
	LONG lPos = 0;
	for (size_t i = 0; i < logTable.m_vecItem.size(); ++i)
	{
		//if (i != logTable.m_vecItem.size() - 1)
		//{
		...
		//	if(!GetValueByType_WithDB(pVoid, logTable.m_vecItem[i], strItem))
		//		return FALSE;
		//	strValueList += strItem;
		//}
	}
...
BOOL LogManage::LoadTableConfig(VOID)
{
	...
	for (TiXmlElement* pTable = pDB_Table_list->FirstChildElement("Table"); pTable!=NULL; pTable=pTable->NextSiblingElement())
	{
		...
		for (TiXmlElement* pItem=pTable->FirstChildElement("item"); pItem!=NULL; pItem=pItem->NextSiblingElement())
		{
			...
			else//包装好的字段集
			{
				...
				for (size_t i = 0; i < vecObjItems.size(); ++i)
				{
					LogItem logItem;
	...
	//// 读取数据表结构
	//ifstream stream;
	//stream.open(szTableCfgFile);
	//if( !stream.is_open() )
	//	return OutErrMsgBox("Setup/TableCfg.ini文件打开失败！");

	//string strTemp;
	//stream >> strTemp; //文件注释

	//LONG		flag = 0, table_count = 0;
	//LogTable	tmpTable, *pCurrTable = NULL;
	//LogItem		tmpItem;
	...
	//	{
	//		if (0 < table_count)
	//			if(0 == pCurrTable->m_vecItem.size())
	//				return OutErrMsgBox("Setup/TableCfg.ini中有为空的表格！");	//有空表
	//		pCurrTable = NULL;
	//		stream 
	//			>> tmpTable.m_szTableName 
	//			>> tmpTable.m_tableType 
	//			>> tmpTable.m_szShowName;

	//		if (1 > tmpTable.m_tableType)
	//			return OutErrMsgBox("Setup/TableCfg.ini中日志表格类型定义不能小于1！");	//类型值非法
	//		if (m_TableMap.end() != m_TableMap.find(tmpTable.m_tableType))
	//			return OutErrMsgBox("Setup/TableCfg.ini中日志表格类型定义重复！");	//类型重复

	//		m_TableMap[tmpTable.m_tableType] = tmpTable;
	//		pCurrTable = &m_TableMap[tmpTable.m_tableType];
	...
	//	{
	//		if (NULL == pCurrTable)
	//			return OutErrMsgBox("Setup/TableCfg.ini中表格和字段的组合顺序有错误！");	// 结构顺序错误
	//		char szType[128] = {0};
	//		stream
	//			>> tmpItem.m_szItemName
	//			>> szType
	//			>> tmpItem.m_szShowName;

	//		if(eDTC_ErrorType == (tmpItem.m_eType = GetDTC_byStr(szType)))
	//			return OutErrMsgBox("Setup/TableCfg.ini中字段数据类型说明非法！");	// 字段类型定义错误

	//		pCurrTable->m_vecItem.push_back(tmpItem);
	//		memset(&tmpItem, 0, sizeof(LogItem));
	//	}
	//	else if(0 == flag)
	//	{
	//		if (0 != table_count)
	...
	//				if(0 != pCurrTable->m_vecItem.size())
	//					return TRUE;
	//		return OutErrMsgBox("Setup/TableCfg.ini文件格式不对！");
	//	}
	//	else
	//	{
	//		return OutErrMsgBox("Setup/TableCfg.ini文件格式不对！");
...
BOOL	LogManage::LoadControlConfig(VOID)
{
	...
	/*
	//! 读取XML包装的类型
	for(TiXmlElement* pTableControl = pControl_Condition->FirstChildElement("TableControl"); 
		pTableControl != NULL; 
		pTableControl = pTableControl->NextSiblingElement())
	{
		LONG tableType = atoi(pTableControl->Attribute("table_type"));
		if (m_TableMap.end() == m_TableMap.find(tableType))
		{
			return OutErrMsgBox("控制条件错误，无此类型表格");
		}
		LogTable &logTable = m_TableMap[tableType];

		//! 保存控制
		logTable.m_SendControl = atoi(pTableControl->Attribute("send_control"));
		m_vSendControl.push_back(pair<LONG, LONG>(tableType, logTable.m_SendControl));
		assert( eSC_NOTLOG >= (unsigned LONG)logTable.m_SendControl);

		for(TiXmlElement* pCondition = pTableControl->FirstChildElement("Condition"); 
			pCondition != NULL; 
			pCondition = pCondition->NextSiblingElement())
		{
			ChooseCondition tmpChooseCondition;
			tmpChooseCondition.m_tableType = tableType;
			LONG item_index = atoi(pCondition->Attribute("item_index"));
			if (item_index > logTable.m_vecItem.size())
			{
				return OutErrMsgBox("控制条件错误，字段索引超过字段数量");
			}
			tmpChooseCondition.m_itemIndex = item_index - 1;

			//! 比较办法
			const char*	pC = pCondition->Attribute("eCondition");
			if (0 == stricmp(pC, "Even"))
			{
				tmpChooseCondition.m_eCondition = eC_Even;
			}
			else if (0 == stricmp(pC, "More_Than"))
			{
				tmpChooseCondition.m_eCondition = eC_More_Than;
			}
			else if (0 == stricmp(pC, "Less_Than"))
			{
				tmpChooseCondition.m_eCondition = eC_Less_Than;
			}
			else
			{
				return OutErrMsgBox("控制条件错误，非法条件");
			}

			//! 获得类型
			tmpChooseCondition.m_valueType = logTable.m_vecItem[item_index - 1].m_eType;
			//! 获得比较值
			const char*	pValue = pCondition->Attribute("value");
			switch(tmpChooseCondition.m_valueType)
			{
			case eDTC_INT32:
			case eDTC_DATE:
				{
					LONG lValue = atoi(pValue);
					_AddToByteArray(&(tmpChooseCondition.m_vData), lValue);
				}
				break;
			case eDTC_FLOAT:
				{
					float fValue = atof(pValue);
					_AddToByteArray(&(tmpChooseCondition.m_vData), fValue);
				}
				break;
			case eDTC_GUID:
				{
					CGUID guid(pValue);
					_AddToByteArray(&(tmpChooseCondition.m_vData), guid);
				}
			    break;
			case eDTC_STR128:
			case eDTC_STR512:
				_AddToByteArray(&(tmpChooseCondition.m_vData), pValue);
				break;
			default:
			    break;
			}
			
			m_vChooseCondition.push_back(tmpChooseCondition);
			logTable.m_vecItem[item_index - 1].m_conditionId = m_vChooseCondition.size() - 1;
		}
...
VOID	LogManage::ConditionAddToByteArray(vector<byte>& outVector)
{
	...
	for (size_t i = 0; i < m_vSendControl.size(); ++i)
	{
	...
	for (size_t i = 0; i < m_vItemCondition.size(); ++i)
	{
		...
		for (size_t n = 0; n < m_vItemCondition[i].m_vPosList.size(); ++n)
		{
		...
		for (size_t j = 0; j < m_vItemCondition[i].m_vConditionTile.size(); ++j)
		{
...
VOID LogManage::CreateTableInDb(BOOL bOutToFile, string file_path)
{
	...
	for (TableMap_ite ite = m_TableMap.begin(); ite != m_TableMap.end(); ++ite)
	{
		...
		//! 生成语句身
		for (size_t i = 0; i < m_TableHead.size(); ++i)
		{
			strCreateTableSql += 
		...
		for (size_t i = 0; i < ite->second.m_vecItem.size(); ++i)
		{
			strCreateTableSql += 
