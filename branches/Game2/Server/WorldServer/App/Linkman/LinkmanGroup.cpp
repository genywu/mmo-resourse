	...
	if (pPlayer != NULL)
	{
		...
		else
		{
			...
			if(!pTagRgn)
			{
				...
				if(NULL != pFather)
					Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__,FormatText("WS_CONTACT_40", szGuid, pPlayer->GetRegionID(), pFather->GetName()));
				else
					Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__,FormatText("WS_CONTACT_40", szGuid, pPlayer->GetRegionID()));
			}
		}
		...
		if (pPlayer)
		{
			...
			if(!tLoginInfo)
			{
				Log4c::Trace(ROOT_MODULE,FormatText("WS_CONTACT_10", pPlayer->GetAccount()));
				return;
			}
...
tagLinkman_data* LinkmanGroup::CreateLinkman_data(const CGUID &PlayerGuid, const char *pPlayerName)
{
	...
	//! 创建数据对象
	tagLinkman_data* pLinkmanData = new tagLinkman_data(); //(tagLinkman_data*)GetGame().GetMemPool().Multi_Alloc(sizeof(tagLinkman_data), _FILE_AND_LINE_);
