...
BOOL	WSOrganizingParam::ReLoad()
{
	...
	try
	{
		...
		m_tagGlobalSetup.lDemiseTime					= atoi(pFacGlobe->Attribute("DemiseTime"));
		m_tagGlobalSetup.lDisbandFactionTime			= atoi(pFacGlobe->Attribute("auto_disband_term"));
		m_tagGlobalSetup.lNegativeTime				= atoi(pFacGlobe->Attribute("auto_degrade_term"));
		m_tagGlobalSetup.lBaseBattleStart			= atoi(pFacGlobe->Attribute("BaseBattleStart"));
		m_tagGlobalSetup.lBaseBattleLast				= atoi(pFacGlobe->Attribute("BaseBattleLast"));
		m_tagGlobalSetup.byClearBaseTime				= (BYTE)atoi(pFacGlobe->Attribute("ClearBaseTime"));
		...
		for (; pExpLevel!=NULL; pExpLevel = pExpLevel->NextSiblingElement("ExpLevel"))
		{
			...
			FacExpLevelSetup.fBaseExp	= (float)atof(pExpLevel->Attribute("baseExp"));
	...
	{
		Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__,GetText("WS_ORGCTL_40"));
		return FALSE;
	}
...
BOOL WSOrganizingParam::CodeToMsgSet(DBWriteSet &DbWriteSet)
{
	...
	{
		//! 工会全局配置
		DbWriteSet.AddToByteArray(&m_tagGlobalSetup, sizeof(tagGlobalSetup));
		...
		for (size_t i = 0; i < m_BussinessLevelSetup.size(); ++i)
		{
			...
			for (size_t j = 0; j < m_BussinessLevelSetup[i].vCondition.size(); ++j)
			{
		...
		for (size_t i = 0; i < m_SpecialLevelSetup.size(); ++i)
		{
			...
			for (size_t j = 0; j < m_SpecialLevelSetup[i].vCondition.size(); ++j)
			{
...
void WSOrganizingParam::AddLevel(DWORD dwFacLv,eUpgradeType eType,DWORD dwLv)
{
	...
	if (it!=m_TechInfo.end())
	{
		...
		{
		case eUT_ExpLevel:
			it->second._lSubExpLv   =max(it->second._lSubExpLv,lLv);
			break;
		case eUT_SpecialityLevel:													
			it->second._lSpecialityLv=max(it->second._lSpecialityLv,lLv);
			break;
		case eUT_BattleLevel:	
			it->second._lBattleLv   =max(it->second._lBattleLv,lLv);
			break;
		case eUT_BussinessLevel:	
			it->second._lBussinessLv=max(it->second._lBussinessLv,lLv);	
			break;
		case eUT_NobleLevel:
			it->second._lNoblelv    =max(it->second._lNoblelv,lLv);
			break;
		}
	}
	else
...
float WSOrganizingParam::GetBussBase(DWORD dwLv)
{
	...
	if(m_BussinessBase.find(dwLv)!=m_BussinessBase.end())
		lBase=(float)m_BussinessBase[dwLv];
	return lBase;
}