...
void CVariableList::Release()
{
	...
	for(size_t i=0; i<m_pVarList.size(); i++)
	{
...
void CVariableList::ReLoadVarList(char *filename, char *index, const char* pData)
{
	...
	{
		stVariable* pTmpVar = new stVariable;
		...
		if(tmp[0] == '#') // 字符串
		{
			...
			if(hasFindBlank) // 字符串数组
			{
				...
				if(arrSize)
				{
					long sizet = blankPos + (long)strlen(arrSize) + 1;
		...
		{
			assert(0);
			//ShowMessage("数组变量 %s 下标小于０！", m_pVarList[i]->Name);
			return;
		}

		// 原有数据查找
		bool isFindVarName = false;
		for(size_t j=0; j<m_pVarList.size(); j++)
		{
...
void CVariableList::LoadVarList(char *filename, char *index, const char* pData)
{
	...
	for(long i=0; i<lTotalVarNum; i++)
	{
		...
		{
			assert(0);
			Log4c::Warn(ROOT_MODULE,FormatText("WS_VARLIST_10", pVar->Name));
			return;
		}
	}
}
...
void CVariableList::AddVar(const char* name, double value)
{
	...
	stVariable* pVarList = new stVariable;
	
	//创建变量名
	long nameSize = (long)strlen(name);
...
const char* CVariableList::GetVarValue(const char *name)
{
	...
	for(size_t i=0; i<m_pVarList.size(); i++)
	{
		if( stricmp(name, m_pVarList[i]->Name) == 0 )
		{
			...
			//else if(m_pVarList[i]->Name[0]=='#' && m_pVarList[i]->Array==-1)	//空串
			//	return "";
			else
				return NULL;
		}
...
bool CVariableList::DecordFromByteArray( BYTE* pByte, long& pos, long bufSize )
{
	...
	for(int i=0; i<lVarNum; i++)
	{
		...
		else if(lArray > 0)	// 数组
		{
			...
			//AddVar(strName,(char*)(&pByte[pos]));
			//TODO:此处应该根据字符串长度计算pos

			//pos+=lstrlen((char*)(&pByte[pos]))+1;
			char szValue[1024];
