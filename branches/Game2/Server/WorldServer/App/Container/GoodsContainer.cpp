...
BOOL CGoodsContainer::Add(DWORD dwPosition, CGoods* pObject, LPVOID pVoid)
{
	...
	if( pObject )
	{
		...
		ulong goodsMaxStackNum = GoodsSetup::QueryGoodsMaxStackNumber(pGoods->GetBasePropertiesIndex());
		if( pGoods )
		{
			...
			if( pGoods->GetBasePropertiesIndex() ==
				...
				goodsMaxStackNum > 1 )
			{
				DWORD dwRemainedSlot = goodsMaxStackNum - pGoods->GetAmount();
				//##如果加入的数量少于等于这个，则直接加入。
...
// 		PutStringToFile("debug-DB", str);
        ...
        Log4c::Trace(ROOT_MODULE, "基本容器:[%s](%s)在%d位置已有[%s](%s)。",pObject->GetName(),szGuid,
            dwPosition,	pGoods->GetName(), szExGuid );
...
CBaseObject* CGoodsContainer::Remove(DWORD dwPosition, DWORD dwAmount, LPVOID pVoid)
{
	...
	if( pGoods && dwAmount > 0 )
	{
		...
		{
			if( GoodsSetup::QueryGoodsMaxStackNumber(pGoods->GetBasePropertiesIndex()) >  1 )
			{
