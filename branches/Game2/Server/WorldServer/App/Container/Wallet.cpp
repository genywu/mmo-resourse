...
// 		PutStringToFile("debug-DB", str);
        ...
        Log4c::Trace(ROOT_MODULE,"钱包:[%s](%s)在%d位置已有[%s](%s)。",pObject->GetName(),szGuid,
            dwPosition,	pGoods->GetName(), szExGuid );
		return FALSE;
	}
...
BOOL CWallet::IsFull()
{
	...
	if( m_pGoldCoins )
	{
		if( m_pGoldCoins->GetAmount() >= GoodsSetup::QueryGoodsMaxStackNumber(m_pGoldCoins->GetBasePropertiesIndex()) )
		{
			bResult = TRUE;
		}
	}
...
BOOL CWallet::Serialize( vector<BYTE>* pStream, BOOL b )
{
	if( m_pGoldCoins )
	{
        ...
        m_pGoldCoins->Serialize( pStream, b ? true : false );
	}
	else
	{
...
BOOL CWallet::Serialize( DBWriteSet& setWriteDB, BOOL b  )
{
	if( m_pGoldCoins )
	{
		...
		m_pGoldCoins->Serialize( setWriteDB,  b ? true : false  );
	}
	else
	{
...
BOOL CWallet::AddGoldCoins(DWORD dwAmount, LPVOID pVoid)
{
	...
	{
		if( m_pGoldCoins->GetAmount() + dwAmount <= GoodsSetup::QueryGoodsMaxStackNumber(m_pGoldCoins->GetBasePropertiesIndex()) )
		{
...
#endif
		CGoodsFactory::CreateGoods( m_dwWalletIndex, dwAmount, vOut,23 );
...
BOOL CWallet::AddGoldCoinOfLargess(CGoods* pObject, DWORD dwPosition, LPVOID pVoid)
{
	...
	if( pObject )
	{
		...
		if( pGoods )
		{
			...
			{
				DWORD dwRemainedSlot = GlobalSetup::GetSetup()->dwGoldCoinLimit -
					pGoods->GetAmount();
				//##如果加入的数量少于等于这个，则直接加入。
