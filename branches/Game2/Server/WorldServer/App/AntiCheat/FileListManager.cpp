...
#define LOG_REQUEST_INVALID( t ) Log4c::Warn(ROOT_MODULE, "Request NULL file list [%s].", (const char*)(t) )
	...
	bool CFileListManager::LoadFile( const std::string &file, FileBufferList &file_list )
	{
		...
		if( rf == NULL )
		{
			Log4c::Error(ROOT_MODULE,"%-15s Load file [%s] failed.",__FUNCTION__, file.c_str() );
			return false;
		}
		Buffer buf;
	...
	const CFileListManager::Buffer &CFileListManager::GetRandBack() const
	{
		...
		return m_BackList[random( (int)m_BackList.size() )];
	}
	...
	const CFileListManager::Buffer &CFileListManager::GetRandSmall() const
	{
		...
		return m_SmallList[random( (int)m_SmallList.size() )];
	}
	...
	const CFileListManager::Buffer &CFileListManager::GetRandX() const
	{
		...
		return m_XList[random( (int)m_XList.size() )];
	}
	
	const CFileListManager::Buffer &CFileListManager::GetSelSign( int sel ) const
	{
		if( m_SelList.size() == 0 || sel > (int)m_SelList.size() )
		{
