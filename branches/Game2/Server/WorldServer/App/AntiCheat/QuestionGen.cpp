...
#define GETA( c ) (DWORD)( ( c & 0xff000000 ) >> 24 )
	...
	{
		if( !img.ReadBitmapFromMemory( (const BYTE*) buf.buf, (long)buf.size ) )
		{
#ifdef RECORD_NAME
			Log4c::Error(ROOT_MODULE,"%-15s Create BMP from memory failed : %s",__FUNCTION__, buf.name.c_str() );
#else
			Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__, "Create BMP from memory failed." );
#endif
			return img;
		}
		...
		{
#ifdef RECORD_NAME
			Log4c::Error(ROOT_MODULE,"%-15s Alpha value is invalid : %s",__FUNCTION__, buf.name.c_str() );
#else
			Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__, "Alpha value is invalid." );
#endif
		}
		return img;
	}
	...
	static PointF GetRandPosInULRect( float w, float h, RectF &rect )
	{
		PointF ret_p ;
		ret_p.X =  rect.Width < w ? rect.X :  (float)random( (int)(rect.Width - w) ) + rect.X ;
		ret_p.Y =  rect.Height < h ? rect.Y : (float)random( (int)(rect.Height -h) )+ rect.Y ;
		return ret_p;
	}

	static void BlendSelSign( CImage &back, RectF &render_rect, int sel )
	{
		CFileListManager &file_manager = GetInst( CFileListManager );
		CImage img;
		CreateImgFromBuffer( file_manager.GetSelSign( sel ), img );
        long  xPos = img.GetWidth();
        long  yPos = img.GetHeight();
		PointF render_pos = GetRandPosInULRect( (float)xPos, (float)yPos, render_rect );
		back.AlphaBlend( &img, (long)render_pos.X, (long)render_pos.Y, 1.0f );
	}
	...
	static PointF GetAlphaImgBlendPos( CImage &dest, CImage &alpha )
	{
		...
		{
			Log4c::Warn(ROOT_MODULE,"Alpha image's size is invalid.[%u,%u].",alpha.GetWidth(), alpha.GetHeight() );
			pos.X = pos.Y = 0;
		}
		else
		{
			pos.X = (float)random( dest.GetWidth() - alpha.GetWidth() + 1 );
			pos.Y = (float)random( dest.GetHeight() - alpha.GetHeight() + 1 );
		}
		return pos;
	}
	...
	static void GenSmallImage( int img_count, CImage &back, RectF &render_rect, int sel, CImage &img_alpha )
	{
...
#define ROT_ANGLE ( random( 20 ) + 160 )
		...
		float fFirstRot =  (float)(ROT_ANGLE);
		float fFirstAlpha = RAND_ALPHA;
		PointF pos = RotateImage( img_first, img_alpha, img_count == 1 ? ( random( 60 ) - 30.0f ) : fFirstRot );
		PointF render_pos = GetRandPosInULRect( (float)img_first.GetWidth(), (float)img_first.GetHeight(), render_rect );
		RectF img_rect( render_pos.X + 20, render_pos.Y + 20, (float)img_first.GetWidth() - 40, (float)img_first.GetHeight() - 40 );
		back.AlphaBlend( &img_first, (long)render_pos.X, (long)render_pos.Y, 
		...
		{
			img_cover = new CImage();
			CreateImgFromBuffer( file_manager.GetRandSmall(), *img_cover, true );
			img_cover->RotateEx( fFirstRot + fRotQuo, (long)rot_pos.X, (long)rot_pos.Y );
			img_cover->SetAlphaMap( &img_alpha, (long)pos.X, (long)pos.Y );
			back.AlphaBlend( img_cover, (long)render_pos.X, (long)render_pos.Y, fFirstAlpha + fAlphaQuo );
			delete img_cover;
		}
...
#define DumpImage( img ) 
	...
	CQuestionManager::Question CQuestionManager::GenOneQuestion()
	{
		...
		if( !back.ExportJpegToMemory( (BYTE*&) ques.data.buf, (DWORD&) ques.data.size, 85 ) )
		{
			ques.data.buf = 0;
			ques.data.size = 0;
			Log4c::Error(ROOT_MODULE,"%-15s %s",__FUNCTION__, "Export jpeg failed." );
		}
