...
void CWorldServerSession::SendSaveFactionMsg(vector<CEntityGroup*> &vCEntityGroup)
{
	...
	if(facMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end())
	{
		...
		{
			Log4c::Warn(ROOT_MODULE, GetText("WS_WSSESSION_10"));
			return;
		}
	...
	for (size_t i = 0; i < vCEntityGroup.size(); ++i)
	{
...
void CWorldServerSession::SendSaveUnionMsg(CEntityGroup* pUnion)
{
	...
	if(facMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end())
	{
		if(facMapItr->second.size() == 0)
		{
			Log4c::Warn(ROOT_MODULE, GetText("WS_WSSESSION_11"));
			return;
		}
		pFacGroup = (CEntityGroup*)facMapItr->second.begin()->second;
		GetGame().GetEntityManager()->ResetEntityAllDbOperFlag(pFacGroup, false);
...
void CWorldServerSession::SendInsertFactionMsg(CEntityGroup* pFaction)
{
	...
	if(facMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end())
	{
		if(facMapItr->second.size() == 0)
		{
			Log4c::Warn(ROOT_MODULE, GetText("WS_WSSESSION_12"));
			return;
		}
		pFacGroup = (CEntityGroup*)facMapItr->second.begin()->second;
		GetGame().GetEntityManager()->ResetEntityAllDbOperFlag(pFacGroup, false);
...
void CWorldServerSession::SendInsertUnionMsg(CEntityGroup* pUnion)
{
	...
	if(facMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end())
	{
		if(facMapItr->second.size() == 0)
		{
			Log4c::Warn(ROOT_MODULE, GetText("WS_WSSESSION_13"));
			return;
		}
		pFacGroup = (CEntityGroup*)facMapItr->second.begin()->second;
		GetGame().GetEntityManager()->ResetEntityAllDbOperFlag(pFacGroup, false);
...
void CWorldServerSession::SendLoadAgentOrdersMsg()
{
	CMessage msg( MSG_W2DB_OPER_ENTITY );
	CEntityGroup *pVarGroup = NULL;
	string strGroup = "[agentordersgroup]";
	tagEntityBuildInfo* pInfo = GetGame().GetEntityManager()->GetEntityBuildInfo(strGroup);
	if(pInfo)
		pVarGroup = GetGame().GetEntityManager()->CreateRootEntity(strGroup, pInfo->guid);
	if( pVarGroup == NULL )
...
void CWorldServerSession::SendSaveAgentOrdersMsg()
{
	CMessage msg( MSG_W2DB_OPER_ENTITY );
	
	CEntityGroup* pVarGroup = NULL;
	CEntityManager::DBEntityComponentMapItr rgnMapItr = GetGame().GetEntityManager()->GetBaseEntityMap().find( string( "[agentordersgroup]" ) );
	if( rgnMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end() )
	{
		pVarGroup = (CEntityGroup*)(rgnMapItr->second.begin()->second);
	}
	...
	// 填充数据到entity group
	GetInst( AH::OrderMgr ).SaveToDB( pVarGroup );

	msg.Add( GetExID() );
	msg.Add( "[agentordersgroup]" );
	msg.Add( pVarGroup->GetGUID() );
	msg.Add( "" );
	msg.Add( (BYTE)0 );
													   ...
													   BYTE byConstellation,bool bSelectRecommCountry)
{
	...
	if(pProcedure)
	{
		...
		if(!tLoginInfo)
		{
			Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_14", szCdkey));
			CMessage msgx(MSG_W2L_LOG_CREATEROLE);
			msgx.Add((char)LOGIN_CREATEROLE_FAILED);
			msgx.Add(szCdkey);
			msgx.SendToServer(NF_LOGIN_SERVER);
		...
		if(dbPlayer)
		{
			...
			dbPlayer->SetBufAttr(string("AutoSupplyData"),  &psDataBA[0], (long)psDataBA.size());
			...
			dbPlayer->SetBufAttr(string("PlayerSetup"),  &psetupServerBA[0], (long)psetupServerBA.size());
			...
			if(!pSpriteGroup)
			{
				Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_15", szCdkey));
			...
			if( NULL == pPetList )
			{
				Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_16", szCdkey));
				GetGame().DelRawPlayer(pPlayer);
				GetGame().GetEntityManager()->DelOneRootEntity(string("[procedure]"), ProcGuid);
				return;
			}
...
void CWorldServerSession::SendUpdateLoginPlayerTimeMsg(const char* szCdkey, const CGUID& guid)
{
	...
	char nRemainDays = char(GlobalSetup::GetSetup()->dwDelDays);
	CMessage retmsg(MSG_W2L_LOG_DELETEROLE);
	retmsg.Add((char)LOGIN_DELETEROLE_SUCCEED);
	retmsg.Add(guid);
	retmsg.Add(szCdkey);
	retmsg.Add(nRemainDays);
	retmsg.SendToServer(NF_LOGIN_SERVER);//send to loginserver
}
...
void CWorldServerSession::NakeSendSavePlayerDataMsg(long bitValue, CPlayer* player, CEntityGroup* pPlayer, bool isSaveDbPlayerFlag)
{
	...
	//if(bitValue & SAVE_DETAIL_ATTRPROPERTY)
	{
        ...
        GetGame().GetEntityManager()->DetailGeniusDataCPlayerToCDPlayer(player,pPlayer);
    ...
    if( ( bitValue & SAVE_DETAIL_EQUIPMENTGOD) )
    {  
        GetGame().GetEntityManager()->DetailEquipGodCPlayerToCDBPlayer(player, pPlayer);
        CEntityGroup* pGroup = (CEntityGroup*)GetGame().GetEntityManager()->FindLeafByComFlag(pGoodsGroup, string("[gp_equip_god]"));
        if(pGroup)
        {
            GetGame().GetEntityManager()->ResetEntityAllDbOperFlag(pGroup, true);
            GetGame().GetEntityManager()->SetEntityAllDbOperType(pGroup, DB_OPER_DELETE_INSERT);
    ...
    if( ( bitValue & SAVE_DETAIL_EQUIPMENTLAND) )
    {
        GetGame().GetEntityManager()->DetailEquipLandCPlayerToCDBPlayer(player, pPlayer);
        CEntityGroup* pGroup = (CEntityGroup*)GetGame().GetEntityManager()->FindLeafByComFlag(pGoodsGroup, string("[gp_equip_land]"));
        if(pGroup)
        {
            GetGame().GetEntityManager()->ResetEntityAllDbOperFlag(pGroup, true);
            GetGame().GetEntityManager()->SetEntityAllDbOperType(pGroup, DB_OPER_DELETE_INSERT);
    ...
    if( ( bitValue & SAVE_DETAIL_EQUIPMENTHUMAN) )
    {
        GetGame().GetEntityManager()->DetailEquipHumanCPlayerToCDBPlayer(player, pPlayer);
        CEntityGroup* pGroup = (CEntityGroup*)GetGame().GetEntityManager()->FindLeafByComFlag(pGoodsGroup, string("[gp_equip_human]"));
        if(pGroup)
        {
            GetGame().GetEntityManager()->ResetEntityAllDbOperFlag(pGroup, true);
            GetGame().GetEntityManager()->SetEntityAllDbOperType(pGroup, DB_OPER_DELETE_INSERT);
	...
	// 好友数据
	CEntityGroup* pFriendGroupOwner = (CEntityGroup*)GetGame().GetEntityManager()->FindLeafByComFlag(pPlayer, string("[playerfriendgroupOwner]"));
	if(!pFriendGroupOwner) return;
	CEntityGroup* pFriendGroupAim = (CEntityGroup*)GetGame().GetEntityManager()->FindLeafByComFlag(pPlayer, string("[playerfriendgroupAim]"));
	if(!pFriendGroupAim) return;
	GetGame().GetEntityManager()->CPlayerFriendToCDBFriendGroup(player, pFriendGroupOwner, pFriendGroupAim);
...
void CWorldServerSession::SendSaveLoginPlayerDataMsg(const CGUID& guid)
{
	...
	if(szCdkey == NULL)
	{
		...
		Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_17", szGuid));
		return;
	}
	Account* pAcc = GetGame().GetEntityManager()->FindAccount(szCdkey);
	if(pAcc == NULL)
...
void CWorldServerSession::SendLoadPlayerDataMsg(const char* szCdkey, const CGUID& guid)
{
	...
	guidItr = pPlayer->GetGuidByComFlagMap().find(string("[playerfriendgroupOwner]"));
	if(guidItr != pPlayer->GetGuidByComFlagMap().end())
	{
		map<CGUID, CBaseEntity*>::iterator enItr = pPlayer->GetEntityGroupMap().find(guidItr->second);
		if(enItr != pPlayer->GetEntityGroupMap().end())
	...
	if(pFriendGroupOwner)
	{
		CTempEntityProperty* pAimGuid = new CTempEntityProperty(string("OwnerGUID"), DATA_OBJECT_GUID);
		pAimGuid->SetGuidAttr( guid);
		pAimGuid->SetDbRelateType(DB_RELATE_NULL);
		pFriendGroupOwner->GetQueryDataVec().push_back(pAimGuid);
	}

	// 为FriendGroup设置查询条件
	CEntityGroup* pFriendGroupAim = NULL;
	guidItr = pPlayer->GetGuidByComFlagMap().find(string("[playerfriendgroupAim]"));
	if(guidItr != pPlayer->GetGuidByComFlagMap().end())
	{
		map<CGUID, CBaseEntity*>::iterator enItr = pPlayer->GetEntityGroupMap().find(guidItr->second);
		if(enItr != pPlayer->GetEntityGroupMap().end())
...
void CWorldServerSession::ProcessLoadAccount(long retFlag, CEntityGroup* pEntity)
{
	...
	if(!pLoginPlayerGroup)
	{
		Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_18", pAcc->GetName()));
		...
		for(; itr != pLoginPlayerGroup->GetEntityGroupMap().end(); itr++)
		{
			...
			if(lPlayer)
			{
				...
				if(!lPlayer->GetStringAttr(string("Name")))
					Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_19",pAcc->GetName()));
				// 填充
				...
				logMsg.Add((short)lPlayer->GetLongAttr(string("HAIR")));			
				...
				logMsg.Add((short)lPlayer->GetLongAttr(string("Shoulder")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("Wrist")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("Leg")));
				...
				logMsg.Add((short)lPlayer->GetLongAttr(string("Card")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("GodGoods")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("Effigy")));	
				logMsg.Add((short)lPlayer->GetLongAttr(string("LifeCowry")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("SoulGuard")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("Rider")));
				...
				logMsg.Add((short)lPlayer->GetLongAttr(string("ShoulderLevel")));	
				logMsg.Add((short)lPlayer->GetLongAttr(string("WristLevel")));	
				logMsg.Add((short)lPlayer->GetLongAttr(string("LegLevel")));
				...
				logMsg.Add((short)lPlayer->GetLongAttr(string("CardLevel")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("GodGoodsLevel")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("EffigyLevel")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("LifeCowryLevel")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("SoulGuardLevel")));
				logMsg.Add((short)lPlayer->GetLongAttr(string("RiderLevel")));
				//--Start 计算删除时间
				long delTime[6];
				long m_DelTime = 0;
			...
			Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_20", szGUID, pAcc->GetName(), retFlag));
		}
		logMsg.SendToServer(NF_LOGIN_SERVER);
	...
	if(pBaseEntity && retFlag == S_OK)
	{
		...
		if(rgnMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end())
		{
			...
			for(; itr != pCountryGroup->GetEntityGroupMap().end(); itr++)
			{
				...
				Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_21", szGuid));
				//GetCountryHandler()->Append(country);
			}
		}
	}
...
void CWorldServerSession::ProcessLoadGlobalVar(long retFlag,CEntityGroup* pBaseEntity)
{
	if(pBaseEntity && retFlag == S_OK)
	{
		if(GetGame().GetGeneralVariableList())
		{
			...
			if(rgnMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end())
			{
				...
				Log4c::Trace(ROOT_MODULE,FormatText("WS_WSSESSION_22",pVarGroup->GetEntityGroupMap().size()));
			}
		}
	}
	else
	{
		Log4c::Warn(ROOT_MODULE, GetText("WS_WSSESSION_23"));
		return;
	}
	...
	if(GetGame().GetGeneralVariableList())
	{
		...
		msg.Add((long)LOAD_GENVARS);
		DBWriteSet db;
		msg.GetDBWriteSet( db );
		VarList::Encode( GetGame().GetGeneralVariableList(), db );
...
bool CWorldServerSession::ProcessCreatePlayerCheck(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	if(pBaseEntity && retFlag == S_OK)
	{
        ...
        GetGame().GetPlayerEquipID(pPlayer, dwHead, dwNecklace, dwWing, dwBody, dwBack, dwGlove,
            dwShoulder,dwWrist,dwLeg,dwBoot, dwLring,dwRring,dwWeapon, 
            dwCard, dwGodGoods, dwEffigy, dwLifeCowry, dwSoulGuard,
            nHeadLevel, nNecklaceLevel, nWingLevel, nBodyLevel, nBackLevel, nGloveLevel, nShoulderLevel,
            nWristLevel, nLegLevel, nBootLevel, nLringLevel, nRringLevel, nWeaponLevel, nCardLevel, 
            nGodGoodsLevel, nEffigyLevel, nLifeCowryLevel, nSoulGuardLevel);
        ...
        msg.Add(nCardLevel);
        msg.Add(nGodGoodsLevel);
        msg.Add(nEffigyLevel);
        msg.Add(nLifeCowryLevel);
        msg.Add(nSoulGuardLevel);
		// 发送创建成功消息到LS
...
bool CWorldServerSession::ProcessLoadPlayerData(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	if(pBaseEntity && retFlag == S_OK && szCdkey)
	{
		...
		if(NULL == lPlayer)
		{
			Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_24", szGUID, szCdkey));
			// 向客户端发送失败消息
			CMessage msg(MSG_W2L_LOG_ANSWER_PLAYERDATA);
			// 添加验证代码
			msg.Add((long)0xFFFFFFFF);
		...
		if(tDel>0)
		{
			Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_25", szGUID, szCdkey));
...
bool CWorldServerSession::ProcessSaveLoginPlayerData(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	pBaseEntity->GetGUID().tostring(szGuid);
	if(retFlag == S_OK)
		Log4c::Trace(ROOT_MODULE,FormatText("WS_WSSESSION_26", szGuid));
	else
		Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_27", szGuid));
	return true;	
}
...
bool CWorldServerSession::ProcessSaveFaction(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	{
        Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_28", szGuid));
	}
	return true;
}
bool CWorldServerSession::ProcessSaveUnion(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	{
        Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_29", szGuid));
	}
	return true;
}
...
bool CWorldServerSession::ProcessInsertFaction(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	{
        Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_30", szGuid));
	}
	return true;
}
bool CWorldServerSession::ProcessInsertUnion(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	{
        Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_31", szGuid));
	}
	return true;
}
...
bool CWorldServerSession::ProcessDelFaction(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	if(S_OK == retFlag)
	{
		//GetOrganiCtrl()->OnDelFactionFinishMsg(AimGuid);
	}
	else
	{
		Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_32", szGuid));
	}
...
bool CWorldServerSession::ProcessDelUnion(long retFlag,CEntityGroup* pBaseEntity)
{
	...
	{
		Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_33", szGuid));
	}
...
bool CWorldServerSession::ProcessInsertOnlineMail(long retFlag, CEntityGroup* pBaseEntity)
{
	...
	GetGame().GetMailManager()->SendSaveMailRet(mailID,lRet);// SendMailMsgToGS(mailID,nRet);
...
bool CWorldServerSession::ProcessDeleteOnlineMails(long retFlag, CEntityGroup* pBaseEntity)
{
	if(pBaseEntity && retFlag == S_OK)
	{
		...
		for(size_t i=0; i<GetTempDataVec().size(); i++)
		{		
			CGUID tmpGuid;
			if (i==0)
			{
...
{
	if(GetTempDataVec().size() != 1) return false;
	if(pBaseEntity && retFlag == S_OK)
	{
		...
		msg.Add(time);
		msg.SendToServer(NF_LOGIN_SERVER);
		Log4c::Trace(ROOT_MODULE,FormatText("WS_WSSESSION_34", szCdkey));
	}
	return true;
}
// 发送删除玩家邮件的存储过程
		...
		if(pEntity->GetRetQueryDataVec().size() == 1 && pEntity->GetRetQueryDataVec()[0])
		{
			if(pEntity->GetRetQueryDataVec()[0]->GetLongAttr() == 1) // 成功
			{
				...
				else
				{
					...
					Log4c::Warn(ROOT_MODULE,FormatText("WS_WSSESSION_35", szGuid));
				}
