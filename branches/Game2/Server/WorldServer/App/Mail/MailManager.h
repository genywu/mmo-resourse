...
public:
	...
	void	GetGoodsFromMail(CGUID &guid,unsigned long lIndex);									//从信中取物品
	...
	//void	SendMailToGsOnServerChange(CPlayer *pPlayer,long lSocketID);				//切换服务器后的邮件转移
	bool	StartRegisterTimer(const CGUID &PlayerGuid, CMail *pMail);					//启动定时器
	void	RegisterTimer(const CGUID &PlayerGuid,const  CGUID &MailGuid,time_t lFuture);	//注册定时器					
	...
	void	UnRegisterCheckTime(const CGUID &PlayerGuid);								//注销邮件检查定时器
