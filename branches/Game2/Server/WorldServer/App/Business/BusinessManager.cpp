...
namespace Business
{
	...
	bool CBusinessManager::Load()
	{
		...
		if( prFile == NULL )
		{
			Log4c::Warn(ROOT_MODULE, "Load busines/business.xml FAILED." );
			return false;
		}
		...
		if( pElem == NULL )
		{
			Log4c::Warn(ROOT_MODULE, "No system config element, set default." );
		...
		Log4c::Trace(ROOT_MODULE, "Load business config file finished." );
		if( !IsEnable() )
		{
			Log4c::Trace(ROOT_MODULE, "Business system has been DISABLED!" );
		}
		return ret;
	}
	...
	void CBusinessManager::EncodeToEntityGroup( CEntityGroup *entity_root )
	{
		...
		for( TradeSpotTable::iterator it = m_Spots.begin(); it != m_Spots.end(); ++ it )
		{
			...
			entity_child->SetBufAttr( "goods_list", &goods_list[0], (long)goods_list.size() );
			
			// country invest list
			CTradeSpot::RawBufType &country_invest = pSpot->QueryCountryInvestRawBuf();
			entity_child->SetBufAttr( "country_invest", &country_invest[0], (long)country_invest.size() );
	...
	void CBusinessManager::DecodeFromEntityGroup( long ret, CEntityGroup *entity )
	{
		...
		{
			Log4c::Warn(ROOT_MODULE,"Load data from database failed!" );
			return;
		}
		...
		if( entityMapItr != GetGame().GetEntityManager()->GetBaseEntityMap().end() )
		{
			...
			if( pVarGroup == NULL )
			{
				Log4c::Warn(ROOT_MODULE, "pVarGroup == NULL, load data from db failed" );
				return;
			}
			...
			for( EntityGroupTable::iterator it = entity_table.begin(); it != entity_table.end(); ++ it )
			{
				...
				if( pSpot == NULL )
				{
					Log4c::Warn(ROOT_MODULE, FMT_STR( "Unknown trade spot [%d]", id  ) );
				}
				else
				{
					// goods list buffer
					...
					Log4c::Trace(ROOT_MODULE, FMT_STR( "Load trade spot [%d] ok", id ) );
				}
			}// for
		}// if
		...
		if( IsEnable() )
		{
            GetGame().LoadEx(LOAD_FBUSINESS, LOADFLAG_NIL, -1);
		}
	}
	...
	void CBusinessManager::DecodeFromGS( DBReadSet &db )
	{
		...
		for( long i = 0; i < spot_count; ++ i )
		{
			...
			else
			{
				...
				Log4c::Trace(ROOT_MODULE,FMT_STR( "Decode trade spot [%d] ok.", id ) );
			}
		}
	}
	...
	bool CBusinessManager::LoadTradeArea( TiXmlElement *elm )
	{
		...
		while( elm != NULL )
		{
			...
			m_Areas.insert( std::make_pair( p.Id, new CTradeArea( p ) ) );
