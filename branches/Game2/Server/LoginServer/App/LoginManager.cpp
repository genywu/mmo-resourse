...
bool LoginManager::ReLoadNoQueue(VOID)
{
	...
	{
		ListFile.close();
		Log4c::Warn(ROOT_MODULE,"LOGIN_LOGINMGR_14");//Can't open NoQueueCdkeyList.txt
		return FALSE;
	}
	char strTempt[256];
	while (!ListFile.eof())
	...
	Log4c::Trace(ROOT_MODULE,GetText("LOGIN_LOGINMGR_0"));//加载不排队帐号成功!
...
VOID LoginManager::ProcessQueue(VOID)
{
	...
	//! 处理防沉迷计时
	OnTimeOutFcmCdkey(static_cast<DWORD>(time(NULL)));
	...
	for (; iteWS != m_mapAllWaitInWsQueue.end(); ++iteWS)
	{
		...
		if(0 < dwCanLoginNum)
		{
			for (DWORD i = 0; i < dwCanLoginNum; ++i)
			{
...
{
	//! 正在验证的账号<账号， 验证超时时间>
	Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_1",m_cqueueValidatingCdKey.size()));//正在验证的账号[%d]个\r\n


	//! 通过验证的账号
	Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_2",m_cqueuePassedAccount.size()));//可以请求基本信息的玩家有[%d]个\r\n
	//! 所有WS的排队列表
	map<LONG, WaitInWsQueue>::iterator ite = m_mapAllWaitInWsQueue.begin();
	for (; m_mapAllWaitInWsQueue.end() != ite; ++ite)
	{
		Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_3",ite->first, ite->second.size()));//WS[%d]上的排队列表有[%d]个\r\n
	}
	//! 一分钟内从WS退出的账号（用于小退验证）
	Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_4",m_cqueueJustOutCdKey_FromWS.size()));//正在小退过程中的账号有[%d]个\r\n

	//! 可以进入游戏的玩家
	Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_5",(LONG)m_mapCanInGamePlayer.size()));//可以请求详细信息的玩家有[%d]个\r\n

	//! 为防沉迷账号的计时记录
	Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_6",(LONG)m_multimapFcmTimeCdkey.size()));//进行防沉迷计时的玩家有[%d]个\r\n
}
...
VOID LoginManager::OnClientLoginRequest(tagAccountInfo_ToAS &AccountInfo_ToAS)
{
	...
	{
		DWORD lCurrTime = static_cast<DWORD>(time(NULL));
...
VOID LoginManager::OnClientQuestPlayerBaseList(const char *pCdKey, const char *pWsName)
{
	...
	do 
	{
		...
		if(0 == AccountInfo_ToAS._dwIp)//! IP为0，说明没有找到对象
		{
			...
			Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_11", pCdKey));//玩家[%s]请求基本角色信息时，还不存在于已通过验证的列表名单中，弹出玩家！
#endif
			break;
		}
		...
		if(-1 == lWsID)
		{
			...
			GetGame()->KickOut(pCdKey);
#ifdef _OUT_LOGIN_FLOW_INFO_
			Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_12", pCdKey));//玩家[%s]请求基本角色信息时，发送了非法的WS名字信息，弹出玩家！
#endif
			break;
		}
		...
		if(!GetGame()->WsIsReady(lWsID))
		{
			...
			msg.SendToClient(AccountInfo_ToAS._lSocket);
#ifdef _OUT_LOGIN_FLOW_INFO_
			Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_13", pCdKey);//玩家[%s]请求基本角色信息时，对应的WS还没有准备好！
#endif
			break;
		}
		...
		else
		{
			//! 配置了错误的WS数据，WS名字过长
			assert(false);
			Log4c::Warn(ROOT_MODULE,"LOGIN_LOGINMGR_15");//Server setup error!!!, World's name too long!!!
			GetGame()->KickOut(pCdKey);
			break;
		}
		...
		else
		{
			...
			if(NULL == pWaitInWsQueue)
			{
				...
				GetGame()->KickOut(pCdKey);
#ifdef _OUT_LOGIN_FLOW_INFO_
				Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_7", pCdKey));//玩家[%s]请求基本角色信息时，没有找到所请求的WS，属于非法消息，弹出玩家！
#endif
			}
			else
			{
	...
	if(bNormalClear)
	{
		Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_8", pCdKey));//玩家[%s]发送基本角色信息，正常清除CD-KEY！
	}
#endif
...
VOID LoginManager::OnClientNetClose(const char *pCdKey, BOOL bInWs)
{
	...
	if(m_cqueuePassedAccount.find(pCdKey))
		Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_9", pCdKey));//响应客户端[%s]断开，从已通过验证的队列中清除该客户端
#endif
	m_cqueuePassedAccount.erase(string(pCdKey));
	...
	if(ePLR_Lost != AccountInfo_FromAS._LoginRerult._dwLoginResult)
	{
		...
		if(1 != dwRe)	
		{
			if(0 != dwRe)
			{
				...
				Log4c::Trace(ROOT_MODULE, "登陆验证完成时请求时IP被首次限制：IP：%u.%u.%u.%u，账号：%s", (ULONG)arrIp[0], (ULONG)arrIp[1], (ULONG)arrIp[2], (ULONG)arrIp[3], AccountInfo_FromAS._szCdkey);
			}	
			return;
		}
		...
		if(m_cqueueValidatingCdKey.find_and_erase(AccountInfo_FromAS._szCdkey) || 2 == uValidateType)
		{
			...
			if (!m_cqueuePassedAccount.find(string(AccountInfo_FromAS._szCdkey)))
			{
					...
					) 
				{
					...
					if(GetGame()->OutInfo())
					{
						Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_LOGINMGR_10", AccountInfo_FromAS._szCdkey));//玩家【Account：%s】通过验证，进入等待列表
					}
#endif
					//! 加入通过验证列表
...
{
	//! 记录连续错误次数
	DWORD dwCurr = static_cast<DWORD>(time(NULL));
...
VOID LoginManager::SendSecurityToC(tagLSSecurityData& LSSecurityData)
{
	...
	{
		CHAR cPos = (CHAR)(rand() % SECURITY_CARD_SIZE);
		while(setTmp.end() != setTmp.find(cPos))
		{
			cPos = (CHAR)(rand() % SECURITY_CARD_SIZE);
		}
...
VOID LoginManager::AddFcmCdkey(const char *pCdKey, DWORD dwHoldSecond)
{
	...
	DWORD dwEndTime = dwHoldSecond + static_cast<DWORD>(time(NULL));
...
DWORD LoginManager::OnIpLogin(LONG lIP, LPCTSTR pCdKey)
{
	...
	//! 检测同一IP登陆的同一账号次数
	if((long)m_LimitIpSetup.m_MaxIpLoginOncCdKeyCount <= iteIpInfo->second[pCdKey])
...
DWORD LoginManager::OnIpCreate(LONG lIP)
{
	...
	if((long)m_LimitIpSetup.m_MaxIpCreateCount <= iteIpInfo->second)
