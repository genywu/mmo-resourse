...
void OnServerMessage(CMessage* pMsg)
{
	...
	case MSG_W2L_SERVER_SENDWORLDINFO:
		{
			...
			Log4c::Trace(ROOT_MODULE,FormatText("LOGIN_SRVMSG_0",str));//WorldServer [%s] connected...ok!
	...
	case MSG_W2L_SERVER_SENDCDKEYLIST:
		{
			throw;//! 这条消息已经作废了，不要用
			...
			if( lWorldNumber )
			{
				...
				GetGame()->SetWsReadyState(lWorldNumber, TRUE);
