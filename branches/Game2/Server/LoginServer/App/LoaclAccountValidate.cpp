...
MultiObjPool<LoaclAccountValidate, LockOfWin32>		LoaclAccountValidate::c_MultiObjPool;
...
{
	//return new LoaclAccountValidate(AccountInfo_ToAS, dwSocket);

	LoaclAccountValidate* pRe = c_MultiObjPool.Multi_Alloc(_FILE_AND_LINE_);
	if(NULL != pRe)
...
VOID LoaclAccountValidate::ReleasValidateOperator(LoaclAccountValidate **ppRVO)
{
	...
	if(NULL == ppRVO || NULL == *ppRVO)
		return;
	(*ppRVO)->~LoaclAccountValidate();
...
LPVOID LoaclAccountValidate::ValidateAccountFromDB(LPVOID pThis)
{
	...
	{
		Log4c::Warn(ROOT_MODULE,"LOGIN_LOCALACCOUNTVAL_0");//Get CnPtr failed, end!
	...
	catch(_com_error &e)
	{
		...
		if (c_pCnPool->RepairCnPtr(cnPtr))
		{
			...
			Log4c::Trace(ROOT_MODULE,GetText("LOGIN_LOCALACCOUNTVAL_1"));//Repair CnPtr failed, end!
	}
	...
	try
	{
		...
		if (varOutCard.vt == (VT_ARRAY | VT_UI1))
		{
			...
			{
				pVO->m_ValidateData._AccountData._LoginRerult._InfoCode = LOGIN_DB_OPT_FAILED;
				Log4c::Warn(ROOT_MODULE,FormatText("LOGIN_LOCALACCOUNTVAL_2", pVO->m_ValidateData._AccountData._szCdkey));//user[%s]'s card data is bad!!!
			}
		}
	}	
	catch(_com_error &e)
...
LPVOID LoaclAccountValidate::SaveASResult(LPVOID pThis)
{
	...
	if(!c_pCnPool->GetCnPtr(cnPtr))
	{
		Log4c::Warn(ROOT_MODULE,"LOGIN_LOCALACCOUNTVAL_3");//Get CnPtr failed, end!
	...
	for (size_t i = 0; i < vSaveData.size(); ++i)
	{
		_CommandPtr		CmdPtr;
		...
		catch (_com_error &e)
		{
			...
			{
				Log4c::Warn(ROOT_MODULE,"LOGIN_LOCALACCOUNTVAL_4");//Repair CnPtr failed, loop continue!
				CmdPtr.Release();
				continue;
			}
		}
