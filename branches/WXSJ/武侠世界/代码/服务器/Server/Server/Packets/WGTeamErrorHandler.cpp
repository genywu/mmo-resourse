...
__ENTER_FUNCTION
	...
	if( pGamePlayer==NULL )
	{
		g_pLog->FastSaveLog( LOG_FILE_3, "WGTeamErrorHandler::Execute pGamePlayer==NULL" );
		return PACKET_EXE_CONTINUE;
	}

	if (pGamePlayer->m_HumanGUID != pPacket->GetGUID())
	{
		g_pLog->FastSaveLog( LOG_FILE_3, "WGTeamErrorHandler::Execute pGamePlayer->m_HumanGUID[%d] != pPacket->GetGUID()[%d]",pGamePlayer->m_HumanGUID, pPacket->GetGUID());
		return PACKET_EXE_CONTINUE;
	}
	...
	if( !pScene )
	{
		g_pLog->FastSaveLog( LOG_FILE_3, "WGTeamErrorHandler::Execute pHuman->getScene() == NULL" );
		return PACKET_EXE_CONTINUE;
	}
