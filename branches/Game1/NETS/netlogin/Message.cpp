...
//#include "loginserver\DebugInfo\CrashHandler.h"
//#include "loginserver\DebugInfo\SystemInformation.h"
//#include "loginserver\DebugInfo\LogHtml.h"
	...
	case BASEMSG_Socket_Accept:
		{
			...
			{
				char pszInfo[1024]="";
				_snprintf(pszInfo,1024,"WorldServer[Socket:%d,IP:%s] Connect",GetSocketID(),GetStringIP());
				AddLogText(pszInfo);
	...
	case BASEMSG_Socket_Close:
		{
			if(GetNetFlag()&NF_Server_Client)
			{
				...
				if(szCdkey)
				{
					...
					GetGame()->OnNetClientClose(szCdkey);
				}	
			...
			{
				char pszInfo[1024]="";
				_snprintf(pszInfo,1024,"WorldServer[Socket:%d,IP:%s] Close",GetSocketID(),GetStringIP());
				AddLogText(pszInfo);
				GetGame()->OnNetWorldClose(GetWorldID());
				CMessage::UnMapWorldIDSocket(GetSocketID());
			}
