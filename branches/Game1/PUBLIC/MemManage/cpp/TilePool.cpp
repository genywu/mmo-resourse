...
namespace XM_Tools
{
	...
	*/
	...
	{
		sprintf(pInfoBuf, "CTilePool:\r\n");
		for (unsigned long i = 0; i < TILE_POOL_NUM; ++i)
		{
			sprintf(&pInfoBuf[strlen(pInfoBuf)], "	【%u】byte大小的内存块已分配【%u】块、已使用【%u】块、实际消耗内存【%u】KB\r\n", 
				MIN_GRANULARITY * (i + 1), m_arrMemList[i].uTileSum, m_arrMemList[i].uUseCount, m_arrMemList[i].uSize / 1024);
		}
	}
