...
#ifdef _CLIENT
#include "..\Client\FrostEngine\render.h"
...
;	}
	...
	int CrashFilter(LPEXCEPTION_POINTERS lpEP,DWORD dwExceptionCode,char* dumpfilename)
	{
		...
		if(!dumpfilename)
			sprintf(log_filename, "log\\dump[%02d-%02d(%02d.%02d)].log", stTime.wMonth, stTime.wDay, stTime.wHour, stTime.wMinute);
		else
			sprintf(log_filename, "log\\dump-%s[%02d-%02d(%02d.%02d)].log", dumpfilename,stTime.wMonth, stTime.wDay, stTime.wHour, stTime.wMinute);
		//if(dumpfilename)
