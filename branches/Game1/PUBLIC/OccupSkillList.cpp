...
// 获得该职业下的指定技能的最大等级
long COccupSkill::GetOccupSkillMaxLev(DWORD dwSkillID)
{
	for (vector<tagOccupSkill*>::iterator it= m_vecSkill.begin();
		it!=m_vecSkill.end();++it)
	{
...
bool COccupSkillList::Init(const char *filename)
{
	...
	if (pNode==NULL)
	{
		_snprintf(strError, 128, CStringReading::LoadString(IDS_WS_SKILL,STR_WS_SKILL_SEVENTH));
		MessageBox(NULL,strError,"",MB_OK); 
		return false;
	}
	//类型转换
...
//编码
bool COccupSkillList::AddToByteArray(vector<BYTE>  &pByteArray)
{
	 //技能列表个数
...
bool COccupSkillList::DecordFromByteArray(BYTE* pByte, long& pos)
{
	//技能列表个数
	Release();
...
bool COccupSkillList::OccuJudgeStudy(DWORD dwID,DWORD dwLv,long lOCcupID)
{
...
bool CChangeOccu::Init(const char *filename)
{
	if(filename)
	{
		...
		{
			//输出错误信息
			MessageBox(NULL,"filenameError","",MB_OK); 
			rfClose(prfile);
			return false;
		}
		rfClose(prfile);
		...
		if (pNode==NULL)
		{
			MessageBox(NULL,"ChangeOccuError","",MB_OK); 
			return false;
		}
		//类型转换
		TiXmlElement* pElem = pNode->ToElement();
...
void CChangeOccu::GetChangeOccuCondision(BYTE bOccu,string &str,int &num)
{
...
// 根据开始和结束的职业，查找转职路径
void CChangeOccu::GetChangeOccuPath(vector<BYTE> &vecOccu,BYTE bStart,BYTE bEnd)
{
	vecOccu.clear();
	// 本职业
	...
	BYTE bStartLen = strlen(start);
	BYTE bEndLen = strlen(end);
	start[bStartLen] = '\0';
	end[bEndLen] = '\0';
	// 查找中转节点
	...
	tagChangeOccu stChangeOccu;
	for (int i=0; i<m_vecSubOccu.size(); ++i)
	{
