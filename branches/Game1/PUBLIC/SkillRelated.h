...
enum tagUseFailed
{
	...
	SKILL_USE_FAILED_WEAPON_DAMAGE,             //武器持久为零时,不能使用技能
...
struct tagExState
{
	WORD	type;		// 类型
	DWORD	level;		//等级					2
	DWORD	keeptime;	//状态持续时间			4

	WORD	maxhp;		//增加MAXHP上限			2
	WORD	maxmp;		//增加MAXMP上限			2

	WORD	minatk;		//增加目标最小ATK		2
	WORD	maxatk;		//增加目标最大ATK		2

	WORD	elm;		//增加目标元素攻击力	2
	WORD	def;		//增加目标DEF			2
	WORD	elmdef;		//增加目标元素抵抗力	2

	WORD	cch;		//增加目标CCH			2
	WORD	fullmiss;	//增加FULLMISS系数		2
	WORD	atkavoid;	//增加AttackAvoid系数	2
	WORD	elmavoid;	//增加ElementAvoid系数	2
	WORD	hit;		//命中修正				2
	WORD	dodge;		//增加目标DODGE			2

	DWORD	item_idx;	// 道具索引编号
	DWORD	item_num;	// 道具数量
	DWORD	frequency;	// 单位毫秒的频率
};
