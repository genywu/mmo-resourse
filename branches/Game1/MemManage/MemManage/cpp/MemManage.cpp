...
namespace XM_Tools
{
	...
	void CMemManage<_TLock>::Release(void)
	{
		...
		if(0 < m_mapAllocInfo.size())
		{
			...
			for (stdext::hash_map<void*, std::string>::iterator ite = m_mapAllocInfo.begin(); m_mapAllocInfo.end() != ite; ++ite)
			{
	...
	{
		unsigned long UserSize = lSize;
		lSize += sizeof(CListNode<unsigned char>);
		...
		pRe = ((char*)pRe) + sizeof(CListNode<unsigned char>);
	...
	void CMemManage<_TLock>::AsyncFree(void* pBuf, unsigned long lSize, const char *pInfo)
	{
		//! 调试信息
		m_InfoLock.Lock();
		stdext::hash_map<void*, std::string>::iterator ite = m_mapAllocInfo.find(pBuf);
		...
		lSize += sizeof(CListNode<unsigned char>);
		pBuf = ((char*)pBuf) - sizeof(CListNode<unsigned char>);
	...
	void* CMemManage<_TLock>::AsyncAlloc(unsigned long lSize)
	{
		//! 预留出节点内存不与用户混用
		unsigned long UserSize = lSize;
		lSize += sizeof(CListNode<unsigned char>) + sizeof(long);
		...
		long *pSize = (long*)(((char*)pRe) + sizeof(CListNode<unsigned char>));
		*pSize = lSize;

		pRe = ((char*)pRe) + sizeof(CListNode<unsigned char>) + sizeof(long);
	
		if(NULL != pRe)
		{
#ifdef FILL_MEM
			memset(pRe, FILL_VALUE, UserSize);
#endif

#ifdef FREE_CHECK
			m_InfoLock.Lock();
			stdext::hash_set<void*>::iterator itr = m_AllocedMem.find(pRe);
			if (m_AllocedMem.end() != itr)
			{
				char szTmp[1024] = {0};
				sprintf(szTmp, "AsyncAlloc ERROR！！！分配的内存已在分配列表中【Size：%u】\r\n", UserSize);
				m_OutInfoFun(m_pOutFile, szTmp);
			}
			else
				m_AllocedMem.insert(pRe);
			m_InfoLock.UnLock();
#endif
		}
	...
	{
		m_InfoLock.Lock();
		stdext::hash_set<void*>::iterator itr = m_AllocedMem.find(pBuf);
		if (m_AllocedMem.end() == itr)
		{
			char szTmp[1024] = {0};
			sprintf(szTmp, "ERROR！！！释放指针不在已分配列表【%s】\r\n", pInfo);
			m_OutInfoFun(m_pOutFile, szTmp);
			m_InfoLock.UnLock();
			return;
		}
		m_AllocedMem.erase(itr);
		m_InfoLock.UnLock();

		lSize += sizeof(CListNode<unsigned char>) + sizeof(long);
		long *pSize = (long*)(((char*)pBuf) - sizeof(long));
		if (*pSize != lSize)
		{
			char szTmp[1024] = {0};
			sprintf(szTmp, "ERROR！！！指针大小错误【%s】，释放大小【%d】，实际大小【%d】\r\n", pInfo, lSize, *pSize);
			m_OutInfoFun(m_pOutFile, szTmp);
			lSize = *pSize;
		}
		pBuf = ((char*)pBuf) - sizeof(CListNode<unsigned char>) - sizeof(long);
	...
	void CMemManage<_TLock>::AsyncFree(void* pBuf, unsigned long lSize)
	{
		...
		{
			char szTmp[1024] = {0};
			sprintf(szTmp, "ERROR！！！指针大小错误，释放大小【%d】，实际大小【%d】\r\n", lSize, *pSize);
			m_OutInfoFun(m_pOutFile, szTmp);
			lSize = *pSize;
		}
		pBuf = ((char*)pBuf) - sizeof(CListNode<unsigned char>) - sizeof(long);
