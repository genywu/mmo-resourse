...
BOOL CYuanBao::Add(CBaseObject* pObject, LPVOID pVoid)
{
	// TODO: Add your specialized code here.
	...
	if( pGoods )
	{
		bResult = CYuanBao::Add( 0, pGoods, pVoid );
	}
	return bResult;
}

//##ModelId=41C92C390148
BOOL CYuanBao::Add(DWORD dwPosition, CGoods* pObject, LPVOID pVoid)
{
	// TODO: Add your specialized code here.
	// NOTE: Requires a correct return value to compile.
	BOOL bResult = FALSE;
...
BOOL CYuanBao::AddFromDB( CGoods* pObject, DWORD dwPosition )
{
	// TODO: Add your specialized code here.
	// NOTE: Requires a correct return value to compile.
	...
	if( pGoods ) //m_pGoldCoins存在了
	{
		...
		sprintf(str, "元宝:[%s](%s)在%d位置已有[%s](%s)。",pObject->GetName(),szGuid,
...
BOOL CYuanBao::OnObjectAdded(CContainer* pContainer, CBaseObject* pObj, LPVOID pVoid)
{
	return TRUE;
}

//##ModelId=41CA26E101A5
BOOL CYuanBao::OnObjectRemoved(CContainer* pContainer, CBaseObject* pObj, LPVOID pVoid)
{
	return TRUE;
}

BOOL CYuanBao::Serialize( vector<BYTE>* pStream, BOOL b )
{
	if( m_pGoldCoins )
	{
...
BOOL CYuanBao::Unserialize( BYTE* pStream, LONG& lOffset, BOOL b )
{
	Release();
	...
	if( bGoodsAmount > 0 )
	{
		...
		CBaseObject::SetConstructFlag(80);
#endif
...
BOOL CYuanBao::AddGoldCoins(DWORD dwAmount, LPVOID pVoid)
{
	...
	else if( m_pGoldCoins == NULL && dwAmount )
	{
		vector<CGoods*> vOut;
#ifdef __MEMORY_LEAK_CHECK__
		CBaseObject::SetConstructFlag(56);
#endif
...
CBaseObject* CYuanBao::Find( LONG oType, const CGUID& guid )
{
