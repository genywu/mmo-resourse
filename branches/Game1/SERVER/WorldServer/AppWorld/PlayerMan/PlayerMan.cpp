...
void CPlayerMan::Initial(void)
{
	...
	PutoutLog("PM", LT_DEFAULT, "玩家管理器已初始化.");
}
...
void CPlayerMan::Release(void)
{
	...
	{
		OBJ_RELEASE(tagPlayerInfo,*itr_Set);
	}
							 ...
							 CPlayer* pPlayer)
{
	...
	if( NULL == pInfo )
	{
		...
		{
			char strLog[512];
			sprintf(strLog, "重新添加玩家(%s)信息.",
				pPlayer->GetName());
			PutoutLog("PM", LT_ERROR, strLog);
			return 0;
		}

		sprintf(strLog, "TYPE(0X%x) : 玩家(%s)信息为NULL.",
			eType, pPlayer->GetName());
		PutoutLog("PM", LT_ERROR, strLog);
		return -1;
	}
	...
	default:
		{
			PutoutLog("PM", LT_ERROR, "PLAYER_ELEM_TYPE非法.");
			return -1;
		}
	}
	return 0;
							 ...
							 long lNewVal)
{
	...
	if( NULL == pPlayer )
	{
		sprintf(strLog, "TYPE:0X%X(%ld), NULL == pPlayer.",
			eType,
			lNewVal);
		PutoutLog("PM", LT_ERROR, strLog);
		return -1;
...
void CPlayerMan::FreePlayer(tagPlayerInfo* pInfo)
{
	if( NULL == pInfo )
	{
		PutoutLog("PM", LT_ERROR, "玩家信息为NULL.");
		return;
	}

	if( m_PrePlayers.size() >= m_lMaxPlayerNum * m_lGSNum )
	{
...
// 从查询结果中移除GM
void CPlayerMan::RemoveGM(LIST_GUID& list)
{
	ITR_LIST_GUID itr = list.begin();
	for (; itr != list.end();)
	{
		tagPlayerInfo* info = FindPlayer(*itr);
		if (info != NULL &&
			CGMList::GetInfoByName(info->strName.c_str()))
		{
			itr = list.erase(itr);
		}
		else
