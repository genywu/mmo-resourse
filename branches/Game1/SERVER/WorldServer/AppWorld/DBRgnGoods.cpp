...
CDBRgnGoods::CDBRgnGoods(const CGUID& id)
...
// virtual interfaces
void CDBRgnGoods::ProcessMsg(DB_OPERATION_TYPE op, long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	switch(op)
	{
	case DB_OPER_SAVE:
...
void CDBRgnGoods::Load(long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	...
	for(int i=0; i<(int)attrNum; i++)
	{
		...
		else
		{
			...
			std::map<string, CDataEntityManager*>::iterator itr = GetGame()->GetDBEntityManager()->GetObjAttrDef().find(string("phrgnobj"));
...
void CDBRgnGoods::AddEntityDataToMsg(DB_OPERATION_TYPE opType, vector<BYTE>& pBA)
{
...
// 检查更新属性里是否有指定表名的属性名
bool CDBRgnGoods::FindTableUpdateFlag(const string& tableName)
{
