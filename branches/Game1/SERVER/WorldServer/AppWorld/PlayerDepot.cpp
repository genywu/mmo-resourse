...
{
	//! 密码
	_GetBufferFromByteArray(pByteArray, lPos, m_szPassword, PASSWORD_SIZE);
	//! 扩展栏位标识
...
{
	//! 密码
	setWriteDB.AddToByteArray(m_szPassword, PASSWORD_SIZE);
	//! 扩展栏位标识
	long lFlag = 0;
	for (long i = 0; i < DEPOT_SUBPACK_NUM; ++i)
	{
...
{
	//! 密码
	setReadDB.GetBufferFromByteArray( m_szPassword, PASSWORD_SIZE);
	//! 扩展栏位标识
...
void PlayerDepot::SetPassword(const char* pPassword)
{
	...
	long lPwsSize = strlen(pPassword);
	lPwsSize = (lPwsSize < PASSWORD_SIZE) ? lPwsSize : PASSWORD_SIZE;
	memset(m_szPassword, 0, PASSWORD_SIZE);
