...
public:
	...
	//////////////////////////////////////////////////////////////////////////
public:
	CVariableList* GetVariableList(void);
	void SetVariableList(CVariableList* vl);
	void UpdateVariableList(CVariableList* vl);
private:
...
public:
	...
	BYTE GetStrongPointFlag(void) { return m_bStrongPintFlag; }
	// 设置据点是否开启
	void SetStrongPointFlag(BYTE flag) { m_bStrongPintFlag = flag; }
	// 取得据点个数
	long GetStronPointNum(void) { return m_vecStrongPoint.size(); }
	// 取得据点状态值
...
private:
    ...
    //是否允许稳定挂机 0表示不能，1表示能
    bool    m_bStableHook;
    //是否允许家族成员退出、家族解散、成员邀请等家族成员变动的操作标识，1不能，0能
    bool    m_bNoChangeFactionMember;
	//场景的空间类型
...
public:
    ...
    void SetCommonHook(bool bCommon) {m_bCommonHook = bCommon;}
    bool GetCommonHook()   {return m_bCommonHook;}

    void SetStableHook(bool bStable) {m_bStableHook = bStable;}
    bool GetStableHook()   {return m_bStableHook;}

    void SetNoChangeFactionMemberFlag(bool bNoChangeMember) {m_bNoChangeFactionMember = bNoChangeMember;}
    bool GetNoChangeFactionMemberFlag(void) {return m_bNoChangeFactionMember;}
