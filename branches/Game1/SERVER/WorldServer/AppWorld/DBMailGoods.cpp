...
// virtual interfaces
void CDBMailGoods::ProcessMsg(DB_OPERATION_TYPE op, long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	switch(op)
	{
	case DB_OPER_LOAD:
...
void CDBMailGoods::Load(long msgid, BYTE* msgBuf, long& msgBufPos, long bufSize)
{
	...
	for(int i=0; i<(int)attrNum; i++)
	{
		...
		else
		{
			...
			std::map<string, CDataEntityManager*>::iterator itr = GetGame()->GetDBEntityManager()->GetObjAttrDef().find(string("mailgoods"));
...
void CDBMailGoods::AddEntityDataToMsg(DB_OPERATION_TYPE opType, vector<BYTE>& pBA)
{
...
// 初始化属性配置
void CDBMailGoods::InitProperty()
{
	CDataEntityManager* pdeManager = NULL;
	std::map<string, CDataEntityManager*>::iterator itr = GetGame()->GetDBEntityManager()->GetObjAttrDef().find(string("mailgoods"));
	if(itr != GetGame()->GetDBEntityManager()->GetObjAttrDef().end())
	...
	{
		AddLogText("CDBMailGoods::InitProperty() Err,未找到相关配置信息!");
	}
}

// 检查更新属性里是否有指定表名的属性名
bool CDBMailGoods::FindTableUpdateFlag(const string& tableName)
{
