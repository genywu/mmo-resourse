...
CMailSystem::~CMailSystem()
{
	...
	for (map<long,tagTimerVar*>::iterator it=m_mapTimer.begin(); it!=m_mapTimer.end(); ++it)
	{
		...
		OBJ_RELEASE(CMail::tagMail,ptgmail);
		CBaseObject::ReleaseTimerVar(&((*it).second));
	}
	m_mapTimer.clear();
	...
	{		
		OBJ_RELEASE(stTimerParam,(*it).second);
	}
...
void CMailSystem::SendMail(CMail *pMail,long lSysID)
{
	...
	if (!GetGame()->GetDbIsReady())
	{
		...
		if (pWriter)
		{
			FailedSendMail(pWriter,MAIL_SENDING_FAILED_INVALID_STATE);			
		}
		//////////////////////////////////////////////////////////////////////////
	...
	if (pMail)
	{
		...
		time_t tRemainTime =  t_CurrentTime + CGlobeSetup::GetSetup()->fMaxRemainTime[lType]*86400 ;
	...
	for (list<string>::iterator it=pMail->GetReceiverList().begin(); it!=pMail->GetReceiverList().end(); ++it)
	{
		...
		else
		{
			...
			tagMailParam* pNewMailParam = OBJ_CREATE(tagMailParam);
			InitMailParam(pNewMailParam,pMail->GetBaseParm());
			CMail *pNewMail = MP_NEW CMail(pNewMailParam);
...
//从信中取物品
void CMailSystem::GetGoodsFromMail(CGUID &guid,long lIndex)
{
...
void CMailSystem::PayMail(CGUID &guid)													
{
	...
	if (it!=m_mapMail.end())
	{
		...
		{	
			...
			time_t tRemainTime =  t_CurrentTime + CGlobeSetup::GetSetup()->fMaxRemainTime[pMail->GetType()]*86400 ;
			pMail->SetRemainTime(tRemainTime);	
...
bool	CMailSystem::OnChangeMailDest(CMail *pMail)
{
	...
	{
		if ((pMail->GetSGood().size()>0 || pMail->GetMGoodsContainer().size()>0 || pMail->GetGold()>0) && pMail->GetType()!=SYSTEM_MAIL && pMail->GetType()<SENDGOLD_MAIL)
		{
			...
			else if (pMail->GetType() == REQUEST_MAIL)
			{
				pMail->ClearSGoods();
			}
			time_t t_CurrentTime;
			...
			wsprintf(strSysMsg,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_THIRD));
			wsprintf(strSuject,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_EIGHTH),pMail->GetSubject().c_str());
			wsprintf(strText,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_TWELVE),pMail->GetText().c_str());



			time_t tRemainTime =  t_CurrentTime + CGlobeSetup::GetSetup()->fMaxRemainTime[SYSTEM_MAIL]*86400 ;
...
void	CMailSystem::RejectMail(CPlayer *pPlayer, CGUID &guid)
{
	...
	if (pMail && pMail->GetReject()  && pMail->GetType()!=SYSTEM_MAIL && pMail->GetType()<SENDGOLD_MAIL)
	{
		...
		char   szSysMsg[128];
		char   strText[1280];
		sprintf(strSysMsg,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_THIRD));
		sprintf(szSysMsg,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_FOURTH),pMail->GetReceiver().c_str());
		sprintf(strText,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_ELEVEN),pMail->GetText().c_str());
		...
		if(pMail->GetType()==REQUEST_MAIL || pMail->GetType()==PAYGOLD_MAIL)
		{
			pMail->ClearSGoods();
			pMail->SetGold(0);
		}
...
long CMailSystem::OnTimeOut(DWORD timerid,DWORD curtime,const void* var)
{
	...
	else if (pTimerVar->TimerType == ChildObjType_SysMail)
	{
		...
		if (it!=m_mapTimerParam.end())
		{
			if ((*it).second->lCalledCount<=(*it).second->lCount)
			{
				...
				SendMail(pSysMail,lSysMailID);

			}
		}
	}
	else if (pTimerVar->TimerType == ChildObjType_Mail_CheckTime)
	{
		list<CGUID> lMailGuid;
		lMailGuid.push_back(ptgmail->MailGuid);
...
{
	tagTimerVar *ptgTimerVar = CBaseObject::CreateTimerVar();

	ptgTimerVar->TimerType = ChildObjType_Mail_CheckTime;
	CMail::tagMail *ptgMail = OBJ_CREATE(CMail::tagMail);
	ptgMail->PlayerGuid = PlayerGuid;
	ptgMail->MailGuid = NULL_GUID;
	ptgTimerVar->pvar = ptgMail;

	long lTimerID = Schedule(this, ptgTimerVar, lFuture + timeGetTime(),lFuture);	
...
void	CMailSystem::UnRegisterCheckTime(const CGUID &PlayerGuid)		
{
	...
	if (tit!=m_mapCheckMailTimer.end())
	{
		...
		if (it!=m_mapTimer.end())
		{
			...
			OBJ_RELEASE(CMail::tagMail,ptgMail);
			CBaseObject::ReleaseTimerVar(&ptgTimerVar);
			it = m_mapTimer.erase(it);
		}
...
void	CMailSystem::RegisterTimer(const CGUID &PlayerGuid,const CGUID &MailGuid,long lFuture)		
{
	...
	ptgTimerVar->TimerType = ChildObjType_Mail;
	CMail::tagMail *ptgMail = OBJ_CREATE(CMail::tagMail);
	ptgMail->PlayerGuid = PlayerGuid;
	ptgMail->MailGuid = MailGuid;
	ptgTimerVar->pvar = ptgMail;

	long lTimerID = Schedule(this, ptgTimerVar, lFuture + timeGetTime());	
	m_mapTimer[lTimerID] = ptgTimerVar;
}
...
{
	if (pMail->GetBaseParm()->ltgTimer.size()>0)
	{
		...
		long lFuture = (tSendTime - tCurrentTime)*1000;

		if (lFuture>0)
		{	
			stTimerParam *pSysTimer = OBJ_CREATE(stTimerParam);
			if (pSysTimer)
			{
				...
				tagTimerVar *pTimvar = CBaseObject::CreateTimerVar();

				pTimvar->TimerType = ChildObjType_SysMail;
				CMail::tagMail *ptgMail = OBJ_CREATE(CMail::tagMail);
				ptgMail->lMailID = dwID;
				pTimvar->pvar = ptgMail;					
				long lTimerID =	Schedule(this,pTimvar,pSysTimer->nStartTime+pSysTimer->lFuture,lIntev);
...
void	CMailSystem::InitMailParam(tagMailParam *pParam,tagMailParam *pPamramList)
{
	...
	for (list<tagCGoods*>::iterator it=pPamramList->lCGoods.begin();it!=pPamramList->lCGoods.end();++it)
	{
		...
		pParam->lCGoods.push_back(ptgCGoods);
	}

}
//初始化系统邮件
void	CMailSystem::InitSysMail()
{
	ClearSysMail();
...
void	CMailSystem::InitSysMailParam(tagMailParam *pParam,tagMailParam *pPamramList)
{
	...
	{
		tagTimer *ptgTimer = OBJ_CREATE(tagTimer);
...
void	CMailSystem::FindNoReceiver(CGUID &MailGuid)										
{
	...
	if (mit!=m_mapTempMail.end())
	{
		...
		{	
			...
			wsprintf(strSysMsg,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_THIRD));
			wsprintf(strSuject,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_EIGHTH),pMail->GetSubject().c_str());
			wsprintf(strText,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_NINE),pMail->GetText().c_str());
	...
	if (it!=m_mapTempMail.end())
	{
		...
		{			
			if ((pMail->GetSGood().size()>0 || pMail->GetMGoodsContainer().size()>0 || pMail->GetGold()>0) && pMail->GetType()!=SYSTEM_MAIL && pMail->GetType()<SENDGOLD_MAIL)
			{
				...
				wsprintf(strSysMsg,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_THIRD));
				wsprintf(strSuject,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_EIGHTH),pMail->GetSubject().c_str());
				wsprintf(strText,CStringReading::LoadString(IDS_WGS_SYSMAIL,STR_WS_SYSMAIL_TEN),pMail->GetText().c_str());
