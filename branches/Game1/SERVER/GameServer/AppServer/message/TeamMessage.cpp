...
void OnWSTeamMessage(CMessage *pMsg){
	...
	case MSG_W2S_TEAM_GetSrcSwapRgnID:
		{
			...
			if(vRgnGuidData.size() > sizeof(LONG))//! 判断是否有需要交换的数据
			{
				...
				else
				{
					...
					msgSwap.AddEx(&vRgnGuidData[0], vRgnGuidData.size());
					msgSwap.Send(false);
				}
			}
	...
	case MSG_W2S_TEAM_Recommend_ByName:
		...
		break;
	case MSG_W2S_TEAM_ChangeOptControl:
		{
			CGUID TeamID;
			pMsg -> GetGUID(TeamID);
			long eSet = pMsg->GetLong();
			GSTeam* pTeam = ((GSTeam*)(GetOrganiCtrl()->GetGameTeamByID(TeamID)));
			if(NULL != pTeam) pTeam->SetOptControl(eSet);
		}
		break;
	case MSG_W2S_TEAM_SentaiUpdate:
		{
			CGUID TeamID;
			pMsg -> GetGUID(TeamID);
			long NewPoints = pMsg->GetLong();
			GSTeam* pTeam = ((GSTeam*)(GetOrganiCtrl()->GetGameTeamByID(TeamID)));
			if(NULL != pTeam) pTeam->SetSentaiPoints(NewPoints);
		}
		break;
	default:
		break;
...
void OnClientTeamMessage(CMessage *pMsg)
{
	...
	if (pPlayer->IsDied())
	{
		pPlayer->SendNotifyMessage(CStringReading::LoadString(209, 69));
		return;
	}
	...
	case MSG_C2S_TEAM_QUERY_JOIN:
		{
			...
			if(NULL != pInvitee)
				if(pInvitee->IsDied())
				{
					pMsg->GetPlayer()->SendNotifyMessage(CStringReading::LoadString(209, 68));
					return;
				}
			...
			if(pCaptain && pInvitee)
			{
				...
				if( pCapRgn && pCapRgn->GetStrongPointFlag()>0 ) // 邀请人在开启据点功能的场景
				{
					CServerRegion* pInvRgn = dynamic_cast<CServerRegion*>(pInvitee->GetFather());
					// 判断双方是否在同一个场景
					if(pInvRgn == pCapRgn)
					{
                        //判断双方是否不同家族 by yuanhongjun 2011-4-2
                        if (pCapRgn->GetStrongPointFlag() == 3)
                        {
                            if (pInvitee->GetFactionID() == NULL_GUID || pInvitee->GetFactionID() != pCaptain->GetFactionID())
                            {
                                //对据点战标识为3时是家族战，需要同一家族才能邀请加入
                                pMsg->GetPlayer()->SendNotifyMessage(CStringReading::LoadString(209, 76));
                                return;
                            }
                        }
                        else
                        {
                            ...
                            {
                                pMsg->GetPlayer()->SendNotifyMessage(CStringReading::LoadString(209, 71));
                                return;
                            }
                        }
					}
	...
	case MSG_C2S_TEAM_BeRecruited://应征
		{
			...
			if(0 != strlen(pTeamMastter->GetRecruitState().szPassword))
			{
				...
				{
					char* pPasswordErr = CStringReading::LoadString(209, 60);
			...
			if( pMasterRgn && pMasterRgn->GetStrongPointFlag()>0 ) // 招募人在开启据点功能的场景
			{
				...
				{
                    //判断双方是否不同家族 by yuanhongjun 2011-4-2
                    if (pMasterRgn->GetStrongPointFlag() == 3)
                    {
                        if (pPlayer->GetFactionID() == NULL_GUID || pPlayer->GetFactionID() != pTeamMastter->GetFactionID())
                        {
                            //对据点战标识为3时是家族战，需要同一家族才能邀请加入
                            pMsg->GetPlayer()->SendNotifyMessage(CStringReading::LoadString(209, 76));
                            return;
