...
long GoodsExManage::TestPutWay(CPlayer *pPlayer, vector<tagGoodsOperate> &vecDelGoods, vector<CGoods*> &vPutGoods)
{
	...
	//! 计算删除的东西空出来的格子
	for (long i = 0; i < vecDelGoods.size(); ++i)
	{
		CGoods			*pGoods		= NULL;
...
bool GoodsExManage::DelGoods(CPlayer *pPlayer, vector<tagGoodsOperate> &vDelGoods, DWORD dwLogType, const CGUID &optGuid)
{
	...
	for (long i = 0; i < vDelGoods.size(); ++i)
	{
		//! 找到该物品的容器
		...
		if( NULL != pCGoodsContainer )
		{
			...
			if (NULL != pGoods)
			{
				...
				{
				case 380:
					GetGameLogInterface()->logT380_goods_disassemble_log(pPlayer, pGoods, lAddType, optGuid, FALSE);
					break;
				case 390:
					GetGameLogInterface()->logT390_goods_synthesize_log(pPlayer, pGoods, lAddType, optGuid, FALSE);
				    break;
				case 400:
					GetGameLogInterface()->logT400_goods_upgrade_log(pPlayer, optGuid, pGoods, 0, 0);
					break;
				default:
				    break;
				}
...
bool GoodsExSetup::Unserialize(BYTE* pStream, long& lOffset)
{
	...
	for (long i = 0; i < lUpgradeSetNum; ++i)
	{
		...
		for (long j = 0; j < lUpgradeSetNum; ++j)
		{
			...
			UpgradeSet.lOdds = _GetLongFromByteArray(pStream, lOffset);			//! 成功几率
			UpgradeSet.lFailedOperate = _GetLongFromByteArray(pStream, lOffset);//! 失败时的处理办法（枚举值）
			UpgradeSet.lOperateNum = _GetLongFromByteArray(pStream, lOffset);	//! 失败时的操作数
			UpgradeSet.ArmLevel = _GetLongFromByteArray(pStream, lOffset);		//! 适用的武器等级	

			//! 生机材料的物品索引
			long lStuffNum = _GetLongFromByteArray(pStream, lOffset);
			for (long k = 0; k < lStuffNum; ++k)
				UpgradeSet.listStuffIndex.push_back(_GetLongFromByteArray(pStream, lOffset));
	...
	for (long i = 0; i < lExtraStuffNum; ++i)
	{
		...
		ExtraStuff.lMaxUseCount = _GetLongFromByteArray(pStream, lOffset);		//! 每次升级装备最大使用数量
		ExtraStuff.lMinGoodsLevel = _GetLongFromByteArray(pStream, lOffset);	//! 使用该宝石的冲等物品的最小冲等等级
		ExtraStuff.lAddOdds = _GetLongFromByteArray(pStream, lOffset);			//! 使用后增加的成功几率百分数
		ExtraStuff.lFailedMinishLevel = _GetLongFromByteArray(pStream, lOffset);//! 使用后，冲等失败时，物品降低的冲等级数	

		long lStuffNum = _GetLongFromByteArray(pStream, lOffset);
		for (long k = 0; k < lStuffNum; ++k)
			ExtraStuff.listStuffIndex.push_back(_GetLongFromByteArray(pStream, lOffset));
	...
	//! 魔化配置
	{
		...
		//! 强化对应的附魔等级
		long lmapUpgradeToElementLeveSize = _GetLongFromByteArray(pStream, lOffset);
		for(long i=0;i<lmapUpgradeToElementLeveSize;++i)
		{
			long lUpgrade = _GetLongFromByteArray(pStream, lOffset);
			long lElement = _GetLongFromByteArray(pStream, lOffset);
			mapUpgradeToElementLevel[lUpgrade] = lElement;
		}
	}
