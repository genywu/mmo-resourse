...
BOOL CTrader::OnPlugInserted()
{
	...
	if( pPlayer )
	{
		...
		pPlayer->m_cWallet.AddListener( &m_cGoldCoins );		
	}
	...
	case CTrader::TS_TRADE_READY:
		{
			...
			{
				CMessage msg( MSG_S2C_PLAYER_WAITFORTRADE );
				msg.Add( lPlugID );
			...
			if( IsReady() )
			{
				...
				{
					CMessage msgRet(MSG_S2C_PLAYER_TRADERESULT);					
					msgRet.Add((BYTE)2);					
					if(pBuyer)
	...
	case CTrader::TS_TRADE_LOCK:
		{
			if(GetOwnerType()==TYPE_PLAYER)
			{
				CMessage msg( MSG_S2C_PLAYER_TRADELOCK );
				//msg.Add( lPlugID );
				//msg.Add( static_cast<BYTE>(TRUE) );
				msg.SendToPlayer( GetOwnerID() );
			}
...
BOOL CTrader::CheckTradeCondition(long &lTotalAmount)
{
	...
	if( pSession )
	{
		...
		if(dwGoodsNullNum==2)
		{
			char* strRes=CStringReading::LoadString(23,21);
			if(strRes)
				pSession->SendNotification(strRes,NOTIFY_COLOR_RED);
			return FALSE;
		}
		...
		for(; it != pSession->GetPlugList().end(); ++it ) 
		{
			...
			if( pTraderPlug )
			{
 				...
 				if( pTrader )
				{
					...
					{
						for(size_t idx=0;idx<vGoods.size();idx++)
						{
							CGoods* pGoods=vGoods[idx];
							if(!pGoods)
								return FALSE;
							if(pGoods->HasAddonProperty(GAP_UNIQUE))
							{
								DWORD dwGoodsId=pGoods->GetBasePropertiesIndex();
								if(!pTrader->IsGoodsUnique(CGoodsFactory::QueryGoodsOriginalName(dwGoodsId)))
								{
									char* strMsg=CStringReading::LoadString(20,55);
									if(strMsg)
									{
										pSession->SendNotification(strMsg);
									}
									return FALSE;
								}
							}	
						...
						{
							char* strRes=CStringReading::LoadString(23,17);
					...
					{
						char* strRes=CStringReading::LoadString(23,18);
					...
					else 
					{
						...
						if( dwCurrentGoldCoins + pTraderPlug->GetContraryValumeOfTrade() > 
							CGoodsFactory::QueryGoodsMaxStackNumber( CGoodsFactory::GetGoldCoinIndex() ) )
						{
							char* strRes=CStringReading::LoadString(23,19);
							_snprintf( szMsg, 256, strRes, pTrader->GetName() );
							pSession->SendNotification( szMsg );
							bResult = FALSE;
						}
										...
										if(pVolumeContainer)
										{
											if(vecResult[i].eOperType == CS2CContainerObjectMove::OT_NEW_OBJECT)
											{
												...
												if(pVolumeContainer->Add(vecResult[i].dwPos, vGoods[i], NULL, &comMsg) )
												{
													...
													GetGameLogInterface()->logT010_goods_log_trade(pTraderPlayer,pPlayer,dwGoodsId,goodsGuid,strGoodsName,0,0,dwGoodsNum,1);
					...
					if( pPlayer && pContraryPlayer)
					{
						...
						for( ; itValumeOfTrade != mGoldCoins.end(); itValumeOfTrade ++ )
						{
							...
							if( itValumeOfTrade->first == *it )
							{
								...
								GetGameLogInterface()->logT010_goods_log_trade(pPlayer,pContraryPlayer,CGlobeSetup::GetSetup()->dwGoldGoodsId,
									CGUID::GUID_INVALID,CGoodsFactory::QueryGoodsName(CGlobeSetup::GetSetup()->dwGoldGoodsId),0,pPlayer->GetMoney(),dwMoneyVal,1);
							}
							else
							{
								nCurrentMoney +=dwMoneyVal;
...
BOOL CTrader::RemoveTradeGoods(vector<CGoods*>& vGoods)
{
	...
	if( lGoodsList.m_bIsAllGoodsExist )
	{
		...
		for( ; i < lGoodsList.m_vGoodsID.size(); i ++ )
		{
			...
			{
				if(pPlayer)
					GetGameLogInterface()->logT010_goods_log_trade(pPlayer,pPlayer,pGoods->GetBasePropertiesIndex(),pGoods->GetExID(),
					CGoodsFactory::QueryGoodsName(pGoods->GetBasePropertiesIndex()),0,lGoodsList.m_vGoodsID.size(),pGoods->GetAmount(),100);
