...
CPersonalShopSeller::CPersonalShopSeller():m_strShopName(CStringReading::LoadString(21,18))
{
...
BOOL CPersonalShopSeller::GetGoodsSellType(const CGUID& guid, LONG lGoodsId, WORD& wSellType, DWORD& dwGroupNum, DWORD& dwOneGroupCnt )
{
	...
	if(dwMaxStack>1)
	{
		...
		for(;it!=m_mGoodsPrice.end();it++)
		{
			if(it->second.lGoodsId==lGoodsId)
			{
				wSellType = it->second.dwSellType;
...
BOOL CPersonalShopSeller::CheckCondition(LONG lPlugID, DWORD &dwTotalMoney, DWORD& dwTotalWeimian )
{
	...
	{
		char* strRes=CStringReading::LoadString(21,7);
		if(strRes)
		{
	...
	if ( pBuyPlayer->GetGoodsAmountByOriginName( 
		...
		CGlobeSetup::GetSetup()->dwWeimianGoodsId ) )  < dwTotalWeimian )
	{
		char* strRes=CStringReading::LoadString(21,14);//(位面不足，无法购买。)
		if(strRes)
		{
			pBuyPlayer->SendNotifyMessage(strRes);
		}
	...
	//SELLER的钱是否超过上限
	if(pSeller->GetMoney()+dwTotalMoney>CGlobeSetup::GetSetup()->dwGoldCoinLimit)
	{
		char* strRes=CStringReading::LoadString(21,10);
		if(strRes)
		{
			pBuyPlayer->SendNotifyMessage(strRes);
		}
...
bool CPersonalShopSeller::PurchaseEx(LONG lPlugID, DWORD dwTotalMoney, long lTotalYuanbao, long lTotalWeimian )
{
	...
	{
		const char* strName=CGoodsFactory::QueryGoodsOriginalName(tagBuyGoodsIter->lId);
		if(!strName)
			return false;
	...
	if ( lTotalWeimian > 0 )
	{
		...
		DelTest._dwGoodsID = CGlobeSetup::GetSetup()->dwWeimianGoodsId;
	...
	for(int i=0;i<vecGoodsToBuyer.size();i++)
	{
		MP_DELETE(vecGoodsToBuyer[i]);
	}
	vecGoodsToBuyer.clear();
	if(!bSucced)
	{
		//购买者背包空间不够
		char* strRes=CStringReading::LoadString(21,8);
		if(strRes)
		{
			pBuyPlayer->SendNotifyMessage(strRes);
		}
	...
	{
		DWORD dwMaxStack = CGoodsFactory::QueryGoodsMaxStackNumber(tagIter->lId);
		if( dwMaxStack > 1 )
		{
			if ( !GetGoodsSellType( tagIter->guid, tagIter->lId, wSellType, dwGroupNum, dwOneGroupCnt ) )
			{
	...
	{	
		...
		CGoodsFactory::CreateGoods( CGlobeSetup::GetSetup()->dwWeimianGoodsId,
		...
		for ( int i = 0; i < vecWeimianGoodsToSeller.size(); ++i )
		{
		...
		{
			//卖家空间不足以添加交易来的位面
			char* strRes=CStringReading::LoadString(21,17);//(摆摊玩家空间不足，购买失败。)
			if( strRes )
			{
				pBuyPlayer->SendNotifyMessage(strRes);
			}
		...
		if(pVolumeContainer)
		{
			if( vecDelResult[i].eOperType == CS2CContainerObjectMove::OT_DELETE_OBJECT)
			{
				...
				if ( pGoods )
				{
					...
					GetGameLogInterface()->logT504_weimian_log(pBuyPlayer,vecDelResult[i].dwNum,1,1);
				}
				else
				{
					return false;
			...
			else if ( vecDelResult[i].eOperType == CS2CContainerObjectMove::OT_CHANGE_AMOUNT )
			{
				...
				GetGameLogInterface()->logT504_weimian_log(pBuyPlayer,vecDelResult[i].dwNum,1,2);
			}
		}
	}
			...
			if( dwSellNum >= dwBuyNum )				// 足够剩余数量的物品
			{
				...
				while( lSellerRemainNum > 0 )
				{
					...
					for( ; shadowIter!=m_cSample.GetGoodsShadowMap()->end(); shadowIter++)
					{
						...
						{	
							DWORD dwCalNum=0;
							if( shadowIter->second.dwGoodsAmount > lSellerRemainNum )
							{
								// 锁定的大于剩余扣除
				...
				for( int i=0; i<goodsVec.size(); i++ )
				{
					...
					if(bSucced)
					{
						...
						if(pVolumeContainer)
						{
							...
							if(vecResult[0].eOperType == CS2CContainerObjectMove::OT_NEW_OBJECT)
							{
								...
								msgToBuyer.AddEx(&vecGoods[0],vecGoods.size());
							...
							GetGameLogInterface()->logT010_goods_log_trade(pSeller,pBuyPlayer,dwGoodsId,goodsGuid,strGoodsName,1,
					...
					else
					{
						...
						char* strRes=CStringReading::LoadString(20,1);
						pBuyPlayer->SendNotifyMessage(strRes);
						return false;
					}
				}
	...
	if ( lTotalWeimian > 0 )
	{
		...
		for ( int i = 0; i < vecResultForSeller.size(); ++i )
		{
			CVolumeLimitGoodsContainer* pVolumeContainer =
			...
			if(pVolumeContainer)
			{
				...
				if(vecResultForSeller[i].eOperType == CS2CContainerObjectMove::OT_NEW_OBJECT)
				{
					...
					msgToSeller.AddEx(&vecGoods[0],vecGoods.size());
		...
		GetGameLogInterface()->logT504_weimian_log(pSeller,lTotalWeimian,0,1);
	}
	...
	for( ; tagIter != pVec->end(); ++tagIter )
	{
		...
		if ( dwPriceType == PT_GOLD_SELL )//金币
		{
			if ( wSellType == ST_SINGLE_SELL )
			{
				...
				_snprintf( strTemp, 256, CStringReading::LoadString(21,19), 
					pBuyPlayer->GetName(),
					dwTatalMoney, tagIter->dwNum, 
					CGoodsFactory::QueryGoodsName( tagIter->lId ),
					strTempTime);
			...
			{
				//  ××花费了××金币（位面精华）购买了×个（组）××（时间××）
				_snprintf( strTemp, 256, CStringReading::LoadString(21,21), 
					pBuyPlayer->GetName(),
					dwTatalMoney, tagIter->dwNum, 
					CGoodsFactory::QueryGoodsName( tagIter->lId ),
					strTempTime);
		...
		else if ( dwPriceType == PT_WEIMIAN_SELL )//位面精华
		{
			...
			{
				//  ××花费了××金币（位面精华）购买了×个（组）××（时间××）
				_snprintf( strTemp, 256, CStringReading::LoadString(21,20), 
					pBuyPlayer->GetName(),
					dwTatalMoney, tagIter->dwNum, 
					CGoodsFactory::QueryGoodsName( tagIter->lId ),
					strTempTime);
			...
			{
				//  ××花费了××金币（位面精华）购买了×个（组）××（时间××）
				_snprintf( strTemp, 256, CStringReading::LoadString(21,22), 
					pBuyPlayer->GetName(),
					dwTatalMoney, tagIter->dwNum, 
					CGoodsFactory::QueryGoodsName( tagIter->lId ),
					strTempTime);
...
BOOL CPersonalShopSeller::GetGoodsList(vector<BYTE>& vOut)
{
	...
	if( m_bShopOpened )
	{
		...
		{								
			DWORD dwMaxStack=CGoodsFactory::QueryGoodsMaxStackNumber(shadowIt->second.dwGoodsBasePropertiesIndex);
			if(dwMaxStack>1)
			{	
				DWORD dwSellNum=0;
