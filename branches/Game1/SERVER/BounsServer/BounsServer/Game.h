#pragma once
#include "..\..\..\nets\BounsNet\message.h"
#include "..\..\..\nets\Servers.h"
#include "..\..\..\nets\MemClient.h"
#include "..\..\..\LogClient\bdb\LogClient.h"
#include "..\..\..\LogClient\bdb\LogicLogInterface.h"
#include "..\..\..\dbaccess\BounsDB\UploadGoods.h"
...
public:
	struct tagSetup
	{
        ...
        DWORD	dwMinDbOptThreadNum;	// 最小上传DB操作线程保持数
        DWORD	dwMaxDbOptThreadNum;	// 最大上传DB操作线程数
        DWORD	dwThreadNumAdjustSpace;	// 自动调整上传线程数的间隔时间


        string	strUploadSqlConType;		// 上传数据库连接类型
        string	strUploadSqlServerIP;		// 上传数据库IP
        string  strUploadSqlUserName;		// 上传数据库用户名
        string  strUploadSqlPassWord;		//  password
        string  strUploadDBName;			//  database name
        ...
        DWORD   dwLoadNoticeTime;       // 加载公告信息间隔时间
        DWORD   dwCycleNoticeTime;      // 循环发送公告间隔时间
...
private:
    ...
    // 公告    
    CRITICAL_SECTION    m_csNotice;
    MapNotice    m_mapNotice;    
    CRsNotice    *m_pRsNotice;
...
public:
    ...
    // 公告
    bool    bLoadNoticeThreadExit;
    bool    bDealNoticeThreadExit;
    HANDLE  hLoadNoticeThread;
    HANDLE  hDealNoticeThread;
    void    StartLoadNoticeThread(bool bFirst = true);
    void    DealNoticeThread();
    void    AddNotice(tagNotice& stNotice);
    void    GetAllNotice(MapNotice& tmpNotice);
    CRsNotice* GetRsNotice() { return m_pRsNotice; }
    MapNotice GetNoticeMap()    { return m_mapNotice; }
...
public:
    ...
    AsynchronousExecute& GetAsynchronousExecute()   { return m_AsynchronousExecute; }
...
unsigned __stdcall LoadNoticeFunc(void* pArguments);        // 公告
