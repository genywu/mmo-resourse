...
map<DWORD,tagMailParam*> CMailList::m_mapMail;
// 载入系统邮件文件
bool CMailList::LoadSysMailList(const char *pFileName)
{
...
// 载入系统邮件参数
bool CMailList::LoadSysMailList( TiXmlElement *pElem )
{
...
//载入系统邮件参数
void CMailList::LoadSysMailList( TiXmlElement *pElem,tagMailParam *ptgMailParam)	
{
	for (TiXmlElement *pChildElem=pElem->FirstChildElement(); pChildElem!=NULL; 
		pChildElem=pChildElem->NextSiblingElement())
	{
...
//载入事件产生的时间区
void CMailList::LoadSysMailEventTime(TiXmlElement *pElem,tagMailEvent *ptgMailEvent)
{
	for (TiXmlElement *pChildElem=pElem->FirstChildElement(); pChildElem!=NULL; 
		pChildElem=pChildElem->NextSiblingElement())
	{
...
//载入时间配置	
void CMailList::LoadSysMailTimeConfig(TiXmlElement *pElem,struct tm *timeinfo)
{
...
// 添加到CByteArray
bool CMailList::AddToByteArray(vector<BYTE>* pByteArray)
{
	long lNum = m_mapMail.size();
	_AddToByteArray(pByteArray,lNum);
	for (map<DWORD,tagMailParam*>::iterator it=m_mapMail.begin(); it!=m_mapMail.end(); ++it)
	{
		...
		//定时器
		long lTNum = (*it).second->ltgTimer.size();
		_AddToByteArray(pByteArray,lTNum);
		for (list<tagTimer*>::iterator tit=(*it).second->ltgTimer.begin(); tit!=(*it).second->ltgTimer.end(); ++tit)
		{
			_AddToByteArray(pByteArray,(*tit),sizeof(tagTimer));
		}	
		//事件
		long lENum = (*it).second->ltgMailEvent.size();
		_AddToByteArray(pByteArray,lENum);
		for (list<tagMailEvent*>::iterator eit=(*it).second->ltgMailEvent.begin(); eit!=(*it).second->ltgMailEvent.end(); ++eit)
		{
		...
		//收信人
		long lRNum = (*it).second->strReceiverList.size();
		_AddToByteArray(pByteArray,lRNum);
		for (list<string>::iterator rit=(*it).second->strReceiverList.begin(); rit!=(*it).second->strReceiverList.end(); ++rit)
		{
			_AddToByteArray(pByteArray,(*rit).c_str());
		}
		//附加物品
		long lGNum = (*it).second->lSGoods.size();
		...
		//发信条件
		long lCNum = (*it).second->lMailCondition.size();
...
// 解码
bool CMailList::DecordFromByteArray(BYTE* pByte, long& pos)
{
	Release();
	char strTemp[102400];
...
// 通过ID获取系统邮件
tagMailParam*	CMailList::GetSysMail(DWORD dwID)
{
	map<DWORD,tagMailParam*>::iterator it = m_mapMail.find(dwID);
	if (it!=m_mapMail.end())
	{
...
//获取双字
DWORD CMailList::GetDwordAttribute(TiXmlElement *pElem,string strName)
{
	double dValue;
...
//获取位型
BYTE CMailList::GetIntAttribute(TiXmlElement *pElem,string strName)
{
	int iValue;
...
//获取字符串
string CMailList::GetStringAttribute(TiXmlElement *pElem,string strName)
{
	string newStr;
...
//获取文本
string CMailList::GetTextAttribute(TiXmlElement *pElem)
{
	string strValue;
...
//获取系统邮件
map<DWORD,tagMailParam*>	CMailList::GetSysMail()																
{
	return m_mapMail;
}

//释放
void CMailList::Release()
{
