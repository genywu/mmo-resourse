...
public:
	...
	static DWORD	GetPlayerExp(BYTE lLevel)	{return (m_vectorPlayerExp.size()>lLevel && lLevel>=0)?m_vectorPlayerExp[lLevel].dwExp:0;}
	static DWORD	GetPlayerExpMultiple(BYTE lLevel)	{return (m_vectorPlayerExp.size()>lLevel && lLevel>=0)?m_vectorPlayerExp[lLevel].dwExpMultiple:0;}
	static LONG64	GetPlayerTotalExp(BYTE lLevel) 
	{ 
		LONG64 llRet = 0;
	...
	//获得满职业等级经验
	static DWORD    GetOccuLevelMaxExp(eOccupation byOccu)
	{
		DWORD dwRet = 0;
		if (byOccu >= 0 && byOccu < OCC_Max)
		{
			if (m_vcOccuLvlExp[byOccu].size()>0)
				dwRet = m_vcOccuLvlExp[byOccu][m_vcOccuLvlExp[byOccu].size()-1].dwExp;
		}
		return dwRet;
	}

	static DWORD	GetPresentExpPerMinute(BYTE lLevel) {return (m_vectorPlayerExp.size()>lLevel && lLevel>=0)?m_vectorPlayerExp[lLevel].lPresentExpPerMinute:0;}
	static DWORD	GetMaxPresentExp(BYTE lLevel) {return (m_vectorPlayerExp.size()>lLevel && lLevel>=0)?m_vectorPlayerExp[lLevel].lMaxPresentExp:0;}
	static float	GetFacBoonExpScale(LONG lLevel){return (m_vectorPlayerExp.size()>=lLevel && lLevel>0)?m_vectorPlayerExp[lLevel - 1].fFacBoonExpScale:0.0f;}
	static float	GetBaseCP(LONG lLevel){return (m_vectorPlayerExp.size()>=lLevel && lLevel>0)?m_vectorPlayerExp[lLevel - 1].lBaseCP:0;}
...
public:
	...
	static BOOL SaveOccuModifyProperty(const char* filename);
	//
	static bool LoadPlayerExpList(const char* filename);		// 从文件读取exp列表
	static bool SavePlayerExpList(const char* filename);		// 保存到文件exp列表
