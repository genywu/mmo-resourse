...
void OnLogMessage(CMessage* pMsg)
{
	...
	case MSG_C2L_LOG_SEND_PT_TOKEN:
		{
			...
			if(GetGame()->OutInfo())
			{
				AddLogText("MSG_C2L_LOG_SEND_PT_TOKEN");
			}
#endif
	...
	case MSG_C2L_LOG_QUESTCDKEY://客户CDKEY验证
		{
			...
			if(1 != dwRe)	
			{
				if(0 != dwRe)
				{
					...
					char szTmp[128] = {0};
					sprintf(szTmp, "登陆请求时IP被限制：IP：%u.%u.%u.%u，账号：%s", (ULONG)arrIp[0], (ULONG)arrIp[1], (ULONG)arrIp[2], (ULONG)arrIp[3], AccountInfo_ToAS._szCdkey);
	...
	case MSG_C2L_LOG_QUEST_PLAYERBASELIST://客户请求自己的基本信息
		{
			...
			if(GetGame()->OutInfo())
			{
				AddLogText("MSG_C2L_LOG_QUEST_PLAYERBASELIST %s", pCDkey);
			}
#endif
	...
	case MSG_W2L_LOG_ANSWER_PLAYERBASELIST://worldserver返回一个账号的玩家基本信息列表
		{
			...
			if(0 != strlen(szCdkey))
			{
				...
				{
					AddLogText("WS返回取得玩家%s 基本信息失败,flag %d", szCdkey, cLogin);
				}

			}
			else
				AddLogText("WS返回玩家%s基本信息时,LS找不到该CDKEY", szCdkey);
	...
	case MSG_C2L_LOG_QUEST_PLAYERDATA://客户请求自己的详细信息 -> 进入游戏
		{
			...
			if(GetGame()->OutInfo())
			{
				AddLogText("MSG_C2L_LOG_QUEST_PLAYERDATA %s", szCdkey);
			}
#endif
	...
	case MSG_C2L_LOG_QUESTCREATE://客户请求创建一个角色
		{
			...
			if(1 != dwRe)	
			{
				if(0 != dwRe)
				{
					...
					char szTmp[128] = {0};
					sprintf(szTmp, "创建角色请求时IP被限制：IP：%u.%u.%u.%u，账号：%s", (ULONG)arrIp[0], (ULONG)arrIp[1], (ULONG)arrIp[2], (ULONG)arrIp[3], szCdkey);
					PutStringToFile("LimitIpLog", szTmp);
				}
	...
	case MSG_C2L_LOG_Validate_Business:
		{
			//char szCdkey[256];
			//pMsg->GetStr(szCdkey, 256);
			//string szCdkey = GetGame()->GetCSdoaValidate().GetSndaIDByIp(pMsg->GetIP());
			string szCdkey = GetGame()->GetCSdoaValidate().GetCDKeyBySocketID(pMsg->GetSocketID());
			//PutoutLog("SDOA_FCM_INFO", LT_DEFAULT, "二次获取资料信息：%s",szCdkey.c_str());
			GetGame()->GetCSdoaValidate().OnAsyncGetUserInfo(szCdkey,false);
		}
		break;
	}
}