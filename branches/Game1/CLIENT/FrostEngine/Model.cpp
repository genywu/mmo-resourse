...
BOOL model::Mesh::Create(const char *pszFileName)
{
	...
	if (!file.Open(pszFileName))
	{
		...
		MessageBox(NULL,pszFileName,"open faild!",MB_OK);
		return FALSE;
	}
	...
	if (dwFlag != MAKEFOURCC('M','E','S','H'))
	{
		...
		MessageBox(NULL,pszFileName,"type doesn't match!",MB_OK);
		return FALSE;
	}
	...
	if (dwVersion != MAKEFOURCC('2','.','0','0'))
	{
		...
		MessageBox(NULL,pszFileName,"version doesn't match!",MB_OK);
		return FALSE;
	}
...
BOOL model::Mesh::Create(CRFile* pFile)
{
	...
	if (dwFlag != MAKEFOURCC('M','E','S','H'))
	{
		...
		MessageBox(NULL,"","mesh type doesn't match!",MB_OK);
		return FALSE;
	}
	...
	if (dwVersion != MAKEFOURCC('2','.','0','0'))
	{
		...
		MessageBox(NULL,"","Mesh version doesn't match!",MB_OK);
		return FALSE;
	}
	...
	for(DWORD dwTriangle = 0; dwTriangle < m_dwTriangleCount; dwTriangle++)
	{
		...
		for (int i = 0; i < 3;i++)
		{
			...
			{
				MessageBox(NULL,"index overflow","Error",MB_OK);
			}
...
BOOL model::AnimJoint::Create(const char *pszFileName)
{
	...
	if (!file.Open(pszFileName))
	{
		...
		MessageBox(NULL,pszFileName,"Create ANIM file faild!",MB_OK); 
		ASSERT(0);
		return TRUE; 
	}
...
BOOL model::AnimVisibility::Create(const char *pszFileName)
{
	...
	if (!file.Open(pszFileName))
	{
		...
		MessageBox(NULL,pszFileName,"Create AnimVisibility file faild!",MB_OK);
		ASSERT(0);
		return TRUE;
	}
...
BOOL model::AnimColor::Create(const char *pszFileName)
{
	...
	if (!file.Open(pszFileName))
	{
		...
		MessageBox(NULL,pszFileName,"Create AnimColor file faild!",MB_OK);
		return FALSE;
	}
...
BOOL model::AnimUV::Create(const char *pszFileName)
{
	...
	if (!file.Open(pszFileName))
	{
		...
		MessageBox(NULL,pszFileName,"Create AnimUV file faild!",MB_OK);
		return TRUE;
	}
...
BOOL model::BindPose::Create(const char *pszFileName)
{
	...
	if (file.Open(pszFileName))
	{
		...
		{
			OutputError("type doesn't match! \"%s\"\n",file.GetPathName());
			MessageBox(NULL,file.GetPathName(),"type doesn't match!",MB_OK);
			return FALSE;
		}
		...
		{
			OutputError("version doesn't match! \"%s\"\n",file.GetPathName());
			MessageBox(NULL,file.GetPathName(),"version doesn't match!",MB_OK);
			return FALSE;
		}
...
BOOL model::BindPose::Create(CRFile* pFile)
{
	...
	{
		OutputError("type doesn't match! \"%s\"\n",pFile->GetFileName());
		MessageBox(NULL,pFile->GetFileName(),"type doesn't match!",MB_OK);
		return FALSE;
	}
	...
	{
		OutputError("version doesn't match! \"%s\"\n",pFile->GetFileName());
		MessageBox(NULL,pFile->GetFileName(),"version doesn't match!",MB_OK);
		return FALSE;
	}
