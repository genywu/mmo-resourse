...
public:
	...
	BOOL Host(UINT dwPort, const char* strIP,long lFlag,DWORD nSocketType=SOCK_STREAM);
	...
	bool AssociateSocketWithCP(SOCKET socket,DWORD dwCompletionKey);	//用于邦定一个SOCKET到完成端口
...
protected:
	...
	struct tagDataStat
	{
		DWORD dwSendStartTime;			//统计开始时间
		long lTotalSendSize;			//发送的总数量
		DWORD dwRecvStartTime;			//统计开始时间
	...
	long				m_lMaxBlockConnetNum;		//最大的阻塞监听数量
	long				m_lSendInterTime;			//建立连接以后到接受到数据的最大间隔事件

	DWORD				m_dwWorkThreadTick;			//完成端口工作线程心跳计数
	DWORD				m_dwNetThreadTick;			//网络线程心跳计数
	DWORD				m_dwAcceptThreadTick;		//监听接受线程心跳计数
...
public:
	...
	DWORD	GetWorkThreadTick()	{return m_dwWorkThreadTick;}
	DWORD	GetNetThreadTick()	{return m_dwNetThreadTick;}
	DWORD	GetAcceptThreadTick() {return m_dwAcceptThreadTick;}
	...
	//输出消息统计信息到net.log文件
	void	OutputMsgStatInfo(long lIntelTime =1800000);
...
public:	
	//主动连接服务器
	long Connect(LPCTSTR lpszHostAddress, UINT nHostPort,long lFlag,DWORD dwTimeOut=0);
	//阻塞连接
	long Connect(CServerClient* pConClient,LPCTSTR lpszHostAddress, UINT nHostPort,DWORD dwTimeOut=0);
	//主动关闭连接
///////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////
