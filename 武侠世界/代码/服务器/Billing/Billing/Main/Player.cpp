...
__ENTER_FUNCTION
	...
	_MY_TRY 
	{
		...
		{
			Log::SaveLog( "./Log/Error.log", "T=[%.4f] m_pSocketInputStream->Fill ret:%d %s", 
...
__ENTER_FUNCTION
	...
	_MY_TRY
	{
		...
		for( ;; )
		{
			...
			memcpy( &packetTick, &header[sizeof(PacketID_t)], sizeof(WORD) ) ;
			memcpy( &packetuint, &header[sizeof(PacketID_t)+sizeof(PacketID_t)], sizeof(uint) );
...
//		m_LastSendTime = m_CurrentTime ;
		...
		if( (int)ret <= SOCKET_ERROR )
		{
			Log::SaveLog( "./Log/Error.log", "[%d] m_pSocketOutputStream->Flush ret:%d %s", 
...
__ENTER_FUNCTION
	...
	if( m_pSocketOutputStream!=NULL )
	{
		...
		Log::SaveLog( "./Log/Billing.log", "SendPacket! ID=%d", pPacket->GetPacketID() ) ;
