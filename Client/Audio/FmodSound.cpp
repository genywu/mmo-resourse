...
CFmodSound::CFmodSound(const char *filename,FSOUND_SAMPLE *pSample,DWORD dwMode)
:Audio(filename)
{
	m_dwMode = dwMode;
	//m_b3D=b3D;
...
//返回说明
//	无
void CFmodSound::Release(bool bDelay)
{
	Stop();
	m_bAlive=false;
}
...
int CFmodSound::Play(int loops)
{
	...
	//设置失败
	if( FSOUND_SetLoopMode(m_nChannel,m_nLoops<1?
...
//返回说明
//	如果成功播放,返回所使用的channel;失败则返回-1
int CFmodSound::FadeIn(unsigned int time,int loops)
{
	SetVolume(0);
...
//返回说明
//	如果成功播放,返回所使用的channel;失败则返回-1
int CFmodSound::FadeOut(unsigned int time,int loops)
{
...
//返回说明
//	长度的毫秒数,-1表示无效
int CFmodSound::GetAudioLength()
{
	//if(!IsAlive())return -1;
	//return -1;//return FSOUND_Sample_GetLength(m_pSample);
...
void CFmodSound::SetPosition(float *pos)
{
	if(pos==0)return;
...
//返回说明
//	无
void CFmodSound::SetPosition(float x,float y,float z)
{
...
//返回说明
//	无
void CFmodSound::GetPosition(float &x,float &y,float &z)
{
...
//注意事项
//	pos必须是有效指针且长度至少为3
void CFmodSound::GetPosition(float *pos)
{
...
void CFmodSound::Run()
{
	...
	bool bVolumeChange = Audio::m_bSoundVolumeChange;
	if(bVolumeChange)
	{
		SetVolume(Audio::s_fSoundVolume);
