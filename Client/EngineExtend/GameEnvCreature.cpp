...
GameBaseEnvCreature * GameBaseEnvCreature::CreateEnvCreature(const char * szFileName)
{
	...
	case 0:
		...
		{
			MessageBox(NULL, szPath, "环境生物[Bufferfly]建立失败", MB_OK);
			SAFEDELETE(p);
		}
		break;
	case 1:
		...
		if (!p->Create(pRoot->GetChildNode("parameter"), szPath))
		{
			MessageBox(NULL, szPath, "环境生物[Crow]建立失败", MB_OK);
			SAFEDELETE(p);
		}
		break;
	case 2:
		...
		if (!p->Create(pRoot->GetChildNode("parameter"), szPath))
		{
			MessageBox(NULL, szPath, "环境生物[Beetle]建立失败", MB_OK);
			SAFEDELETE(p);
		}
		break;
	case 3:
		...
		if (!p->Create(pRoot->GetChildNode("parameter"), szPath))
		{
			MessageBox(NULL, szPath, "环境生物[Rabbit]建立失败", MB_OK);
			SAFEDELETE(p);
		}
		break;
	case 4:		//流星
		...
		if (!p->Create(pRoot->GetChildNode("parameter"), szPath))
		{
			MessageBox(NULL, szPath, "环境生物[Meteor]建立失败", MB_OK);
			SAFEDELETE(p);
		}
		break;
	}
...
void GameEnvCreature_Butterfly::Render(const GameMap *pMap,render::Camera *pCamera)
{
	...
	//D3DXMATRIX m;
	//D3DXMatrixTranslation(&m, m_vBirthPos.x, m_vBirthPos.y, m_vBirthPos.z);
	//m_RegionBox._DEBUG_Render(&m, 0xffffffff);
}
...
void GameEnvCreature_Crow::Update(const D3DXVECTOR3 * pPosition, render::Frustum * pFrustum)
{
	...
	for(DWORD n = 0; n < m_dwCrowCount;n++)
	{
		...
		}else
		{
			...
			//pCrow->dwLifeTime = (pCrow->dwLifeTime < dwTimeElapse) ? 0 : pCrow->dwLifeTime - dwTimeElapse;
...
void GameEnvCreature_Crow::Render(const GameMap *pMap,render::Camera *pCamera)
{
	...
	for(DWORD n = 0; n < m_dwCrowCount;n++)
	{
		...
		//		m_pModel->SetGroupLevel(pCrow->dwModelLevel);
		m_pModel->SetDirLightEnable(false);
		...
		if (m_bLighted)
		{
			//m_pModel->SetModelColor(dwSunLightAmbient);
			//m_pModel->ProcessDirectionalLight(pCamera->GetPosition(),pLightDirectiion,dwSunLightDiffuse);
			//if (!((int)pCrow->vPosition.x <0 || (int)pCrow->vPosition.z < 0 || (int)pCrow->vPosition.x >= (int)pMap->GetWidth() ||
			//	(int)pCrow->vPosition.z >= (int)pMap->GetDepth()))
			//{
			//	DWORD dwColor = pMap->GetGridColor((int)pCrow->vPosition.x,(int)pCrow->vPosition.z);
			//	//颜色增加1.25倍
...
BOOL GameEnvCreature_Beetle::Create(utility::Text::Node * pNode, char * szResourcePath)
{
	...
	for(DWORD n = 0; n < m_dwBeetleCount; ++n)
	{
		...
		//m_pBeetleArray[n].dwModelLevel = 0;
	}
...
void GameEnvCreature_Beetle::Update(const D3DXVECTOR3 *pPosition, render::Frustum * pFrustum)
{
	...
	for(DWORD n = 0; n < m_dwBeetleCount;n++)
	{
		...
		}else
		{
			...
			//pBeetle->dwLifeTime = (pBeetle->dwLifeTime < dwTimeElapse) ? 0 : pBeetle->dwLifeTime - dwTimeElapse;
...
void GameEnvCreature_Beetle::Render(const GameMap *pMap,render::Camera *pCamera)
{
	...
	for(DWORD n = 0; n < m_dwBeetleCount;n++)
	{
		...
		//		m_pModel->SetGroupLevel(pBeetle->dwModelLevel);
...
void GameEnvCreature_Rabbit::Update(const D3DXVECTOR3 *pPosition, render::Frustum * pFrustum)
{
	...
	for(DWORD n = 0; n < m_dwRabbitCount;n++)
	{
		...
		else
		{
			...
			//pRabbit->dwLifeTime = (pRabbit->dwLifeTime < dwTimeElapse) ? 0 : pRabbit->dwLifeTime - dwTimeElapse;
...
void GameEnvCreature_Rabbit::Render(const GameMap *pMap,render::Camera *pCamera)
{
	...
	for(DWORD n = 0; n < m_dwRabbitCount;n++)
	{
		...
		{
			//			pGrid = pMap->GetGrid(pRabbit->vPosition.x,pRabbit->vPosition.z);
			//			if (pGrid && (pGrid->byFlag & 0x08))
			//			{
			D3DXMatrixScaling(&matScale,m_fScale,m_fScale,m_fScale);
...
void GameEnvCreature_Meteor::Render(const GameMap * pMap, render::Camera * pCamera)
{
	...
	for(DWORD dw = 0; dw < m_dwMeteorCount; ++dw)
	{
		...
		if (m_bLighted)
		{
			//m_pModel->SetModelColor(dwSunLightAmbient);
			//m_pModel->ProcessDirectionalLight(pCamera->GetPosition(), pLightDirectiion, dwSunLightDiffuse);
			//if (!((int)pMeteor->vPosition.x < 0 || (int)pMeteor->vPosition.z < 0 || (int)pMeteor->vPosition.x >= (int)pMap->GetWidth() ||
			//	(int)pMeteor->vPosition.z >= (int)pMap->GetDepth()))
			//{
			//	DWORD dwColor = pMap->GetGridColor((int)pMeteor->vPosition.x,(int)pMeteor->vPosition.z);
			//	//颜色增加1.25倍
			//	DWORD r = ( (dwColor & 0x00FF0000) >> 16) * 5 / 4;
			//	if (r > 255 )
			//		r = 255;
