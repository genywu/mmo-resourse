...
BEGIN_EVENT_TABLE( PropertiesPanel, wxPanel )
    ...
    EVT_GRID_LABEL_LEFT_DCLICK(PropertiesPanel::SortGridByName )
END_EVENT_TABLE()
...
//-----------------------------------------------------------------------
void PropertiesPanel::addRegion( const wxString& name, const wxRect& dim, int rowindex ,const wxPoint &OffsetPoint)
{
    if ( rowindex == -1 )
    {
    ...
    wxSnprintf ( value,
        ...
        wxT( "%d" ),
        OffsetPoint.x );
    m_propertiesGrid->SetCellValue( rowindex, 5, value );
    wxSnprintf ( value,
        sizeof( value ) / sizeof( wxChar ),
        wxT( "%d" ),
        OffsetPoint.y );

    m_propertiesGrid->SetCellValue( rowindex, 6, value );
...
wxPoint PropertiesPanel::getRegionOffset( const wxString & name) const
{
    wxPoint point;
    int regionIdx = getInternalRegionIndex( name );
    ...
    if ( regionIdx != -1 )
    {
        point.x = CEGUIHelper::WXStringToInt( m_propertiesGrid->GetCellValue( regionIdx, 5 ) );
        point.y = CEGUIHelper::WXStringToInt( m_propertiesGrid->GetCellValue( regionIdx, 6 ) );
    }
    return point;
}
//-----------------------------------------------------------------------
...
void PropertiesPanel::createPropertiesGrid( wxWindow* parent )
{
    ...
    m_propertiesGrid->CreateGrid( 0, 7, wxGrid::wxGridSelectCells );
    ...
    m_propertiesGrid->SetColLabelValue( 5, wxT( "XOffset" ) );
    m_propertiesGrid->SetColSize( 5, -1 );

    m_propertiesGrid->SetColLabelValue( 6, wxT( "YOffset" ) );
    m_propertiesGrid->SetColSize( 6, -1 );
...
void PropertiesPanel::onGridAddRow( wxCommandEvent& WXUNUSED( event ) )
{
    ...
    if ( isDocumentValid() )
    {
        ...
        // add the new region
        wxPoint point;
        getDocument()->addRegion( getUniqueImageName(), dim, point);
    }
}
...
void PropertiesPanel::onGridCellChange( wxGridEvent& e )
{
    ...
    if ( isDocumentValid() )
    {
        ...
        wxPoint point = getRegionOffset( name );
        if ( e.GetCol() == 0 )
        {
        ...
        document->setRegionArea( name, dim ,point);
    }
}
void PropertiesPanel::SortGridByName(wxGridEvent& e)
{
    if (isDocumentValid())
    {
        wxString ColName = m_propertiesGrid->GetColLabelValue(e.GetCol());
        const wxString SortColName = L"Name";   
        std::list<imageInfo> imagesList;
        int rows = m_propertiesGrid->GetRows();
        int cols = m_propertiesGrid->GetCols();
        for (int i = 0;i<rows;i++)
        {
            imageInfo ima;
            ima.name = m_propertiesGrid->GetCellValue(i,0);
            ima.x = m_propertiesGrid->GetCellValue(i,1);
            ima.y = m_propertiesGrid->GetCellValue(i,2);
            ima.width = m_propertiesGrid->GetCellValue(i,3);
            ima.height = m_propertiesGrid->GetCellValue(i,4);
            ima.XOffset = m_propertiesGrid->GetCellValue(i,5);
            ima.YOffset = m_propertiesGrid->GetCellValue(i,6);
            imagesList.push_back(ima);
        }
        if(!ColName.Cmp(L"Name"))
            imagesList.sort(imageInfo::Nameless);
        else if(!ColName.Cmp(L"x"))
            imagesList.sort(imageInfo::Xless);
        else if(!ColName.Cmp(L"y"))
            imagesList.sort(imageInfo::Yless);
        else if(!ColName.Cmp(L"w"))
            imagesList.sort(imageInfo::Widthless);
        else if(!ColName.Cmp(L"h"))
            imagesList.sort(imageInfo::Heightless);
        deleteAllRegions();
        std::list<imageInfo>::iterator listItor = imagesList.begin();
        for (int i = 0;listItor != imagesList.end();++listItor,i++)
        {
            imageInfo &ima = *listItor;
            m_propertiesGrid->InsertRows( i, 1 );
            m_propertiesGrid->SetCellAlignment( i, 0, wxALIGN_LEFT, wxALIGN_CENTRE );
            m_propertiesGrid->SetCellValue( i, 0, ima.name );
            m_propertiesGrid->SetCellValue( i, 1, ima.x );
            m_propertiesGrid->SetCellValue( i, 2, ima.y );
            m_propertiesGrid->SetCellValue( i, 3, ima.width );
            m_propertiesGrid->SetCellValue( i, 4, ima.height );
            m_propertiesGrid->SetCellValue( i, 5, ima.XOffset );
            m_propertiesGrid->SetCellValue( i, 6, ima.YOffset );
            m_propertiesGrid->AutoSizeColumn( 0, false );
            m_propertiesGrid->ForceRefresh();
        }
    }
