...
//*//					D3DXMatrixRotationY(&matRot, pMapGrid->GetBuild()->fRotationY);
						...
						if (pBuild)
						{
							...
							if (pModel)
							{
								...
								pModel->dwTrianglCount=pModel->pGameModel->GetTriangleCount();
								...
								if (pGameModel&&pGameModel->GetLoadState() == Load_Did)
								{
									...
									pAnimInfo->SetBillboard(&(matScaling * *pBillBoardMatrix));
									pAnimInfo->SetBillboardY(&(matScaling * *pBillBoardMatrixY));
...
void EditMap::Save(FILE * pFile)
{
	...
	for(n = 0; n < (DWORD)(m_iWidth * m_iDepth); n++)
	{
		...
		fwrite(&m_pGridArray[n].GetBuild()->vRotation,sizeof(D3DXVECTOR3),1,pFile);
		fwrite(&m_pGridArray[n].GetBuild()->vScaling,sizeof(D3DXVECTOR3),1,pFile);
		fwrite(&m_pGridArray[n].GetBuild()->vPosOffset,sizeof(D3DXVECTOR3),1,pFile);
...
void EditMap::Load(FILE * pFile, DWORD dwVersion)
{	 
	m_dwTotalPalacedBuildCount = 0;
	...
	if (dwVersion == MAP_VERSION_1_5 ||
		dwVersion == MAP_VERSION_1_6)
	{
		for(n = 0; n < (DWORD)(m_iWidth * m_iDepth); n++)
		{
			// 此乃1.10版本读取
			fread(&m_pGridArray[n].m_tBuild.wID,sizeof(WORD),1,pFile);
			fread(&m_pGridArray[n].m_tBuild.vRotation.y,sizeof(float),1,pFile);
			fread(&m_pGridArray[n].m_tBuild.vPosOffset.y,sizeof(float),1,pFile);
	...
	}else if (dwVersion == MAP_VERSION_1_7)
	{
		for(n = 0; n < (DWORD)(m_iWidth * m_iDepth); n++)
		{
			// 此乃1.7版本读取
			fread(&m_pGridArray[n].m_tBuild.wID,sizeof(WORD),1,pFile);
			fread(&m_pGridArray[n].m_tBuild.vRotation,sizeof(D3DXVECTOR3),1,pFile);
			fread(&m_pGridArray[n].m_tBuild.vScaling,sizeof(D3DXVECTOR3),1,pFile);
			fread(&m_pGridArray[n].m_tBuild.vPosOffset,sizeof(D3DXVECTOR3),1,pFile);
				...
				if (pLightSource)
				{
					...
					for(int i = -iRadius; i <= iRadius;i ++)
					{
						for(int j = -iRadius; j <= iRadius;j ++)
						{
							...
							D3DXMatrixRotationYawPitchRoll(&matrot, pMapGrid->GetBuild()->vRotation.y, pMapGrid->GetBuild()->vRotation.x, pMapGrid->GetBuild()->vRotation.z);
//*//						D3DXMatrixRotationY(&matrot,pMapGrid->GetBuild()->fRotationY);
...

		{
			...
			if (!pToolBuildEdit->FindSelectIndex(n))
			{
				if (wCurBuildID != 0xFFFF)
				{
					...
					if (pGameModel)
					{
						D3DXMATRIX matWorld,matView,matPro,matRot,matScaling;
						D3DXMatrixTranslation(&matWorld,vCenter.x + pGridBuild->vPosOffset.x, vCenter.y + pGridBuild->vPosOffset.y, vCenter.z + pGridBuild->vPosOffset.z);
						D3DXMatrixRotationYawPitchRoll(&matRot, pGridBuild->vRotation.y, pGridBuild->vRotation.x, pGridBuild->vRotation.z);
						D3DXMatrixScaling(&matScaling, pGridBuild->vScaling.x, pGridBuild->vScaling.y, pGridBuild->vScaling.z);
						matWorld = matScaling * matRot * matWorld;
...
//						D3DXMatrixMultiply(&matWorld,&matRot,&matWorld);
						...
						//及时渲染
						pMapGrid->m_cBuildModelAnimInfo.SetBillboard(&(matScaling * *pBillboardMatrix));
						pMapGrid->m_cBuildModelAnimInfo.SetBillboardY(&(matScaling * *pBillboardMatrixY));	
						...
						if (pModel->bLighted)
						{
							...
							for(DWORD nLight= 0; nLight < (DWORD)vectorLightList.size(); nLight++)
							{
								...
								//计算距离
								D3DXVECTOR3 vOffset = vLightCenter - pGridBuild->vPosOffset;/*vLightCenter - vCenter;*/
