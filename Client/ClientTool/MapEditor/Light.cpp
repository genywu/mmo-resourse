...
namespace light
{
	...
	void DirectionalLight::HandleMessage(const D3DXMATRIX *pViewMatrix)
	{
	...
	void DirectionalLight::ScreenToVector( int  iScreenPtX, int iScreenPtY ,int iScreenWidth,int iScreenHeight,float fRadius,D3DXVECTOR3 *pVector)
	{
		// Scale to screen
	...
	void DirectionalLight::Render()
	{
		...
		m_pAnimInfo->SetViewMatrix(pEditWnd->GetCamera()->GetViewMatrix());
		m_pAnimInfo->SetProjectedMatrix(pEditWnd->GetCamera()->GetProjectionMatrix());
	...
	D3DXQUATERNION DirectionalLight::QuatFromBallPoints(const D3DXVECTOR3 &vFrom, const D3DXVECTOR3 &vTo)
	{
		D3DXVECTOR3 vPart;
	...
	void DirectionalLight::SetLightDirection(const D3DXVECTOR3* vSunLightDirection)
	{
		m_vDefDirection =*vSunLightDirection;
		m_vCurDirection = m_vDefDirection;
	}
