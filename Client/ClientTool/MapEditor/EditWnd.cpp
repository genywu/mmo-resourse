...
static const char * pszMapVersion = "1.7";		//地图版本号
...
void EditWnd::_ShowInformation(void)
{
	...
	pLayer2D->OutPutText(iStartX,iStartY,szTemp,lstrlen(szTemp),0xfff0ff90);
...
void EditWnd::_ShowStatus(void)
{
	...
	sprintf(szTemp,"Mode: %s Cam:%.2d %.2d Vis: %ld VD: %2.2f",pszModeName[m_eEditStatus],(int)pPosition->y,iCameraAngle,dwVisibleTriangle ,Setting::GetInstance()->GetViewDistance());
	int iLen = lstrlen(szTemp);
	...
	if (dwVisibleTriangle > 3000)
	{
		...
		{
			sprintf(szTemp,"#FFFFAA6CMode: #FFC0F285%s #FFFFAA6CCam:#FFC0F285%.2d %.2d #FFFFAA6CVis: #FFFF623E%u VD: %2.2f",pszModeName[m_eEditStatus],(int)pPosition->y,iCameraAngle,dwVisibleTriangle,Setting::GetInstance()->GetViewDistance());
		}
		else
			sprintf(szTemp,"#FFFFAA6CMode: #FFC0F285%s #FFFFAA6CCam:#FFC0F285%.2d %.2d #FFFFAA6CVis: #FFFFEE61%ld VD: %2.2f",pszModeName[m_eEditStatus],(int)pPosition->y,iCameraAngle,dwVisibleTriangle,Setting::GetInstance()->GetViewDistance());
	}
	else
		sprintf(szTemp,"#FFFFAA6CMode: #FFC0F285%s #FFFFAA6CCam:#FFC0F285%.2d %.2d #FFFFAA6CVis: #FFD1FF9A%ld VD: %2.2f",pszModeName[m_eEditStatus],(int)pPosition->y,iCameraAngle,dwVisibleTriangle,Setting::GetInstance()->GetViewDistance());
...
void EditWnd::_ShowAxis(void)
{
	...
	ui::Wnd::SetUIRendState();
	pLayer2D->OutPutText((int)v[1].x,668 + (int)v[1].y - 16,"X",1,0xFFFF0000);

	v[0] = v0; v[1] = vy;
	pLayer3D->_DEBUG_DrawLine(v,D3DCOLOR_ARGB(255,0,255,0));
	D3DXVec3TransformCoord(&v[1],&v[1],&matFinal);
	ui::Wnd::SetUIRendState();
	pLayer2D->OutPutText((int)v[1].x,668 + (int)v[1].y - 16,"Y",1,0xFF00FF00);

	v[0] = v0; v[1] = vz;
	pLayer3D->_DEBUG_DrawLine(v,D3DCOLOR_ARGB(255,0,0,255));
	D3DXVec3TransformCoord(&v[1],&v[1],&matFinal);
	ui::Wnd::SetUIRendState();
	pLayer2D->OutPutText((int)v[1].x,668 + (int)v[1].y - 16,"Z",1,0xFF0000FF);
}
...
{  
	...
	if (dwVersion !=  MAP_VERSION_CURRENT)
	{
		if (dwVersion == MAP_VERSION_1_5)
		{
		}else if (dwVersion == MAP_VERSION_1_6)		//1.5版
			bnew = true;
		else if (dwVersion == MAP_VERSION_1_7)		//1.7版
		{
		}else
		{
	...
	m_pToolBuildEdit->Destroy();

	//读取地图
	pMap->Create(dwWidth,dwDepth);
	pMap->Load(fp, dwVersion);
	CommDB::GetInstance()->SetMap(pMap);
	_CenterCamera(dwWidth,dwDepth);
	//设置方向灯模型的朝向
	m_pDirectionalLight->SetLightDirection(pMap->GetSunLightDirection());
	...
	if (dwVersion == MAP_VERSION_1_7)
	{
		...
		for (DWORD dw = 0; dw < dwNum; ++dw)
		{
			...
			m_pToolBuildEdit->GetNonActivityGridIndex().push_back(iIndex);
		}
	}
...
void EditWnd::SaveMap(const char * pszFileName)
{
	...
	if (fp)
	{
		...
		//版本号
		DWORD dwVersion = MAP_VERSION_CURRENT;
		...
		{
			int iIndex;
			DWORD dwNum = (DWORD)m_pToolBuildEdit->GetNonActivityGridIndex().size();
			fwrite(&dwNum, sizeof(DWORD), 1, fp);
			for (DWORD dw = 0; dw < dwNum; ++dw)
			{
				iIndex = m_pToolBuildEdit->GetNonActivityGridIndex()[dw];
...
void EditWnd::CreatePopupWnd(void)
{
	...
	m_pAboutWnd->Create("地图编辑器",&rc,0,this);
...
bool EditWnd::FileIsExist(LPCSTR sourceFile,LPCSTR lpFileName)
{
	bool bIsExist=false;
	WIN32_FIND_DATA findData;
	HANDLE hFindFile;
	hFindFile=FindFirstFile(sourceFile,&findData);
	if (hFindFile!=INVALID_HANDLE_VALUE)
	{                
		if (!strcmp(findData.cFileName,lpFileName))
		{
			bIsExist=true;
...
void EditWnd::ExportMap(BOOL bSurface, BOOL bHeightmap, BOOL bColormap, BOOL bGrid, BOOL bNode,BOOL bSound)
{
	...
	//检查所选择的文件夹下面是否存在这些文件
	char cfileExistName[7][MAX_PATH]={"config","surface",
		"heightmap","colormap","grid","node","bSound"};
	BOOL bfileExistName[7]={true};
	//将参数翻新到定义的数据里，参与操作运算
	bfileExistName[1]=bSurface;
	bfileExistName[2]=bHeightmap;
	bfileExistName[3]=bColormap;
	bfileExistName[4]=bGrid;
	bfileExistName[5]=bNode;
	bfileExistName[6]=bSound;
	bool bIsReplace=false;
	for (int i=0;i<7;i++)
	{
		char pathNew[MAX_PATH];
		sprintf(pathNew,"%s\\%s",path,cfileExistName[i]);
		//如果对应的复选框被选说明要保存此文件
		if(bfileExistName[i])
		{
			//查找文件是否存在存在
			bIsReplace=FileIsExist(pathNew,cfileExistName[i]);
		}
		if (bIsReplace)
		{
			//如果存在则提示是否覆盖
			char pathInfo[MAX_PATH];
			sprintf(pathInfo,"%s已经存在。要替换它吗？",pathNew);
			int dIsReplace=MessageBox(NULL,pathInfo,"提示",MB_OKCANCEL);
			//当dIsReplace=1时则为是，2时则为否
			if (dIsReplace==2)
			{
				bfileExistName[i]=false;
			}
		}
		bIsReplace=false;
	}
	//下面将上面操作信息翻新到参数数据里
	bSurface=bfileExistName[1];
	bHeightmap=bfileExistName[2];
	bColormap=bfileExistName[3];
	bGrid=bfileExistName[4];
	bNode=bfileExistName[5];
	bSound=bfileExistName[6];
	...
	if (bNode)
	{
		...
		for(n = 0 ;  n <(DWORD)(pMap->GetWidth() * pMap->GetDepth()); n++)
		{
			...
			if (wBuildID != 0xFFFF)
			{
				...
				fwrite(&pMapGrid->GetBuild()->vPosOffset,sizeof(D3DXVECTOR3),1,file);
				fwrite(&pMapGrid->GetBuild()->vRotation,sizeof(D3DXVECTOR3),1,file);
				fwrite(&pMapGrid->GetBuild()->vScaling,sizeof(D3DXVECTOR3),1,file);
				dwCount++;
			}
