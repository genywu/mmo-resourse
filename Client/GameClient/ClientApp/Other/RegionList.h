...
public:
	...
	CMartialPos(void);
	virtual  ~CMartialPos(void);

private:
	// 解析军事据点配置文件
	BOOL LoadMartialPos();
	// 释放军事据点配置文件
	void ReleaseMartialPos();
	// 初始化各个地图下的邮箱坐标
	BOOL InitMartialPos(tagBeachHead& stBeachHead,TiXmlElement* pElem);

	static vector<tagBeachHead>	m_listMartialPos;
};