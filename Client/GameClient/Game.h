...
class CSkillXml;
class ConcreteSkillComp;
class ConcreteDecSkillXml;
class CMemClient;
class UIManager;
...
//class CQuestStep;
/***********************************************************************/
/* zhaohang add 2010-9-6*/
class SkillDocIni;
class SkillShowIni;
class SkillPoseIni;
class SkillAttrIniSetup;
/***********************************************************************/
class GameDespParser;    //物品描述规则
...
enum PLAYER_EXTEND_ID
{
    ...
    PEI_PACK4=10004,
    PEI_PACK5=10005,
    PEI_BUSINESPACKFIELD = 20000,		// 放跑墒背包的栏位ID
    PEI_BUSINESSPACK = 20001			// 跑墒背包容器ID
};
// 判断容器是否属于背包类
...
private:	
    ...
    //AudioEngine *m_pAudioEng;					// 音频引擎
    CAudioList	*m_pAudioList;					// 音效列表，管理所有shape的声音

    CDataBlockAllocator	*m_pDBAllocator;		// 数据块
    CClient* m_pNetClient;						// 客户端网络
    //===========================================================================================
    //CInterface* m_pInterface;					// 游戏界面

    COccupSkillList *m_pOccupSkillList;			// 职业技能列表
	/***********************************************************************/
	/* zhaohang add 2010-9-6*/
	SkillDocIni *m_pSkillDocIni;			//技能文案配置
	SkillShowIni   *m_pSkillShowIni;			//技能显示配置
	SkillPoseIni	*m_pSkillPoseIni;			//技能姿态配置
	//SkillAttrIniSetup* m_pSkillAttrIniSetup;	//技能客户端属性配置
	/***********************************************************************/
    ...
    CBitmapX*	m_pKingIcon;					// 国王图标
    CBitmapX*	m_pBitmapFemaleIcon;			// 女性图标
    ...
    CMainPlayerHand * m_MainPlayerHand;         // 玩家手上容器。
...
private:
    ...
    //CShadowVolume m_pVol[MAX_VOL];						// 阴影数据
...
public:
    ...
    //阴影纹理
    //CShadowMapping m_theShadowMapping;
	...
	/***********************************************************************/
	/* zhaohang add */
	SkillDocIni* GetSkillDocConfig()			{ return m_pSkillDocIni; }
	SkillShowIni*   GetSkilShowIni()			{ return m_pSkillShowIni;}
	SkillPoseIni*   GetSkillPoseIni()			{ return m_pSkillPoseIni;}
	//SkillAttrIniSetup* GetSkillAttrIniSetup()	{ return m_pSkillAttrIniSetup;}
	/***********************************************************************/
    //================================================================
    ...
    CBitmapX* GetKingIcon()						{ return m_pKingIcon; }
    CBitmapX* GetFemaleIcon()					{return m_pBitmapFemaleIcon;}
    ...
    //连接ChatServer服务器
    bool ConnectChatServer(const char*, int);
    ...
    //获取玩家手上容器
    CMainPlayerHand * GetMainPlayerHand()   {return m_MainPlayerHand;}
    ////---------------------------------------------------------------------------------
    //// 体积阴影

    //// 初始化阴影渲染所需状态
    //void InitRenderStage();

    //// 计算体积阴影
    //void CalculateShadowVolumes(CShape *pShape,D3DXVECTOR3 &vLighDirectional);
    //void ResetShadowVolumeMeshCache() { m_dwNumOfVol = 0;}
    //void AddShadowVolumeMesh(CShape *pShape,D3DXVECTOR3 &vLighDirectional);
    ...
    //// 渲染sm阴影
    //void SetShadowMappingLight(D3DXVECTOR3 vL);		// 设置阴影贴图的阳光位置
    //void BeginShadowMapping();						// 开始阴影贴图函数
    //void RenderShadowMapping(CShape *pShape);		// 渲染阴影贴图函数
    //void EndShadowMapping();						// 结束阴影贴图函数
    //void SetShadowMappingBlend();		// 在地面渲染时设置纹理通道进行混合渲染

    //// 取得模型中的组件数
    //DWORD GetMeshCount(GameModel *pModel);

    //// 渲染平面阴影
    //void RenderFlatShadow(CShape* pShape);
    ...
    */
    ...
    void C2L_AddRole_Send(const char *strName,char nOccupation, char nSex, BYTE lHead, BYTE lFace,BYTE lCountry,BYTE lConstellation,BYTE bRandCountry);//向loginserver请求添加一个角色
...
public:
    ...
    void SetQueuePos(long lNum);//		{m_lQueuePos = lNum;}
...
public:
    ...
    //DWORD GetReGameCode(){return m_lReGameCode;};       //获得验证小退的参数
private:
...
public:
    ...
    AutoSupply* GetAutoSupply()  {return m_AutoSupply;}
    //////////////////////////////////////////////////////////////////////////
    // 自动登陆
    //////////////////////////////////////////////////////////////////////////
public:
...
public:
    ...
    void OutPutText(int iX, int iY, const char * pszString, size_t iLen, D3DCOLOR color, int iStyleIndex = 0,
    ...
    long GetFontWidth(const char * Text, int iStyleIndex = 0, float x_scale = 1.0f);
    long GetFontHeight(int iStyleIndex = 0 , float y_scale = 1.0f);
...
//    };
//
//    void LoadNeedDisplayHairList(const char* filename);			// 读取带头盔要显示头发的列表
//    bool IsNeedDisplayHair(BYTE lOccupation, BYTE lEquipID);	// 查询是否需要显示头发
//
//protected:
//    list<tagHelmHair>		m_listHelmHair;		// 带头盔要显示头发的列表

//    //////////////////////////////////////////////////////////////////////////
//    //	需要显示刀光的列表
//    //////////////////////////////////////////////////////////////////////////
//public:
//    struct tagRenderTrail
//    {
//        BYTE	lOccupation;		//	职业
//        WORD	wAction;			//	动作
//        WORD	wWeaponType;		//	武器类型
//        DWORD	dwTrailName;		//	刀光类型
//        DWORD	dwTrailColorIndex;	//	刀光颜色编号
//    };
//
//    void LoadRenderTrailList(const char* filename);						// 读取刀光列表
//    tagRenderTrail* GetRenderTrail(BYTE lOccupation, WORD wAction, WORD wWeaponType);		// 获取某职业的某个动作是否显示刀光
//
//protected:
//    list<tagRenderTrail>		m_listRenderTrail;	// 显示刀光列表
...
public:
    ...
    bool PopWinMessage();//(UINT &message, WPARAM &wParam, LPARAM &lParam);
    //======================================================================================
...
public:
    ...
    void ReLoadGame();
    void OpenUrlEx(const char* UrlId);                            //打开消费反馈界面以外的其它网站
...
private:
    ...
    //FreeType
    //FreeTypeFont		*	m_pFreeType;
public:
    void RenderTTF_Text(int ix, int iy, const WCHAR* strText, long fontType, long lFontSize,DWORD color = D3DCOLOR_ARGB(255,255,255,255),long spacebetween = 0);

private:
    NpcTalkBox *    m_NpcTalkBox;
    GameDespParser * m_gdParer;
public:
    NpcTalkBox * GetNpcTalkBox() {return m_NpcTalkBox;}
    GameDespParser *GetGameDespParser()   {return m_gdParer;}
