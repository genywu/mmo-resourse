...
BOOL  FM_IntersectedPlane(const D3DXVECTOR3 vPoly[], const D3DXVECTOR3 vLine[], D3DXVECTOR3 &vNormal, float &originDistance)
{
	...
	originDistance = FM_PlaneDistance(vNormal, vPoly[0]);
...
BOOL  FM_InsidePolygon(D3DXVECTOR3 &vIntersection, const D3DXVECTOR3 Poly[])
{
	...
	{	
		...
		Angle += FM_AngleBetweenVectors(vA, vB);	// Find the angle between the 2 vectors and add them all up as we go along
	}
...
{							
	...
	float vectorsMagnitude = FM_Magnitude(Vector1) * FM_Magnitude(Vector2) ;
...
BOOL FM_LineSegmentIntesectTriangle(const D3DXVECTOR3 vLineSegment[2],const D3DXVECTOR3 vTriangle[3],D3DXVECTOR3 &vIntersection)
{
	...
	// Reference   // Reference
	if(!FM_IntersectedPlane(vTriangle, vLineSegment,   vNormal,   originDistance))
		return FALSE;

	// Now that we have our normal and distance passed back from IntersectedPlane(), 
	// we can use it to calculate the intersection point.  

	FM_IntersectionPoint(vNormal, vLineSegment, originDistance,vIntersection);

	// Now that we have the intersection point, we need to test if it's inside the polygon.
	if(FM_InsidePolygon(vIntersection, vTriangle))
...
BOOL FM_RadialIntersectedPlane(const D3DXVECTOR3 &vOrigin,const D3DXVECTOR3 &vDirection,D3DXVECTOR3 vTriangle[3],D3DXVECTOR3 &vIntersection)
{
	...
	// First, make sure our line intersects the plane
	// Reference   // Reference
	FM_IntersectedPlane(vTriangle, vLineSegment,   vNormal,   originDistance);
...
bool FM_SpherePolygonCollision(D3DXVECTOR3 vPolygon[],D3DXVECTOR3 &vCenter,float radius,D3DXVECTOR3 &vIntersection)
{
	...
	if (FM_ClassifySphere(vCenter, vNormal, vPolygon[0], radius, distance)) 
	{
		...
		if(FM_InsidePolygon(vIntersection, vPolygon) ||
			FM_EdgeSphereCollision(vCenter, vPolygon, 3, radius))
		{
...
{
	float d = FM_PlaneDistance(vNormal, vPoint);
...
bool FM_EdgeSphereCollision(D3DXVECTOR3 &vCenter, 
						 D3DXVECTOR3 vPolygon[], int vertexCount, float radius)
{
	...
	for(int i = 0; i < vertexCount; i++)
	{
		...
		FM_ClosestPointOnLine(vPolygon[i], vPolygon[(i + 1) % vertexCount], vCenter,vPoint);

		// Now, we want to calculate the distance between the closest point and the center
		float distance = FM_Distance(vPoint, vCenter);
...
D3DXVECTOR3 *FM_Vec3TransformCoordArray(D3DXVECTOR3 *pVectArrayOutput,const D3DXVECTOR3 *pVectArrayInput,const D3DXMATRIX *pMatrix,DWORD dwCount)
{
	...
	{
		x = pVectArrayInput[n].x * pMatrix->_11 + pVectArrayInput[n].y * pMatrix->_21 + pVectArrayInput[n].z * pMatrix->_31 + pMatrix->_41;
		y = pVectArrayInput[n].x * pMatrix->_12 + pVectArrayInput[n].y * pMatrix->_22 + pVectArrayInput[n].z * pMatrix->_32 + pMatrix->_42;
		z = pVectArrayInput[n].x * pMatrix->_13 + pVectArrayInput[n].y * pMatrix->_23 + pVectArrayInput[n].z * pMatrix->_33 + pMatrix->_43;
		//w = pVectArrayInput[i].x * pMatrix->_14 + pVectArrayInput[i].y * pMatrix->_24 + pVectArrayInput[i].z * pMatrix->_34 + pMatrix->_44;
		pVectArrayOutput[n].x=x;
		pVectArrayOutput[n].y=y;
		pVectArrayOutput[n].z=z;
		//pVectArrayOutput[i].w=w;
	}
	return pVectArrayOutput;
}
