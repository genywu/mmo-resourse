...
void Water::CManager::AddWater(CWater * pWater)
{
	...
	pWater->m_lID = m_listAllWaters.size();
}
...
bool Water::CWater::Create(CRFile * pFile)
{
	...
	if (pFile == NULL)
	{
		_snprintf(szTemp, _MAX_PATH, "错误!水面文件%s不存在!", m_szWaterFileName);
		PutDebugString(szTemp);
		return false;
	}
	...
	if (pNode == NULL)
	{
		_snprintf(szTemp, _MAX_PATH, "错误!水面文件%s配置脚本没有\"baseproperty\"节点", m_szWaterFileName);
		PutDebugString(szTemp);
		Script.Destroy();
		return false;
	}
	...
	if (pNode == NULL)
	{
		_snprintf(szTemp, _MAX_PATH, "错误!水面文件%s配置脚本没有\"normallayer\"节点", m_szWaterFileName);
		PutDebugString(szTemp);
		Script.Destroy();
		return false;
	}
	...
	if (pNode == NULL)
	{
		_snprintf(szTemp, _MAX_PATH, "错误!水面文件%s配置脚本没有\"environmentlayer\"节点", m_szWaterFileName);
		PutDebugString(szTemp);
		Script.Destroy();
		return false;
	}
...
bool Water::CWater::CreateShaderGroup(CRFile * pFile, DWORD dwTextureProcision)
{
	...
	if (pFile == NULL)
	{
		_snprintf(szTemp, _MAX_PATH, "错误!水面文件%s配置脚本Shader文件不存在!", m_szWaterFileName);
		PutDebugString(szTemp);
		return false;
	}
