...
public:
	...
	struct tagBuild
	{
		...
		{
			wID = tBuild.wID;
			fRotationY = tBuild.fRotationY;
			fHeight  = tBuild.fHeight;
			m_vHideFlagGridIndexArray.clear();
			std::vector<DWORD>::const_iterator it;
			for(it = tBuild.m_vHideFlagGridIndexArray.begin(); it != tBuild.m_vHideFlagGridIndexArray.end(); it++)
			{	
...
public:
	...
	void  SetBuildRotationY(float fBuildRationY) { m_tBuild.fRotationY = fBuildRationY;}
	void  SetBuildHeight(float fBuildHeight)     { m_tBuild.fHeight  = fBuildHeight;}
