...
void WndNewMap::Render()
{
	...
	if (dwStyle & FWS_VISIBLE)
	{
		...
		pLayer2D->TextOut(iTextStartX,iTextStartY,m_tWindowInfo.szName,iLen,0xffffffff);

		pLayer2D->TextOut(iX + 26, iY + 40,"宽度",4,0xffffffff);
		pLayer2D->TextOut(iX + 26, iY + 80,"深度",4,0xffffffff);
