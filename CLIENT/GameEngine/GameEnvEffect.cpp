...
GameBaseEnvEffect *GameBaseEnvEffect::CreateEnvEffect(const char *szFileName)
{
	...
	case 0:
		...
		{
			MessageBox(NULL,szPath,"Entironment effect [Flow] create fail",MB_OK);
			SAFEDELETE(p);
		}
		break;
	case 1:
		...
		if (!p->Create(pRoot->GetChildNode("parameter"),szPath))
		{
			MessageBox(NULL,szPath,"Entironment effect [Particle] create fail",MB_OK);
			SAFEDELETE(p);
		}
		break;
	}
...
BOOL GameEnvEffect_Flow::Create(utility::Text::Node *pNode,char *szResourcePath)
{
	...
	for(DWORD n = 0; n < m_dwLayerCount;n++)
	{
		...
		{
			MessageBox(NULL,szName,"Entironment effect Flow's texture create fail!",MB_OK);
			bResult = FALSE;
		}
		//流动方向，和速度
