...
GameModel *CGameModelManager::LoadModel(DWORD dwModelID,const string& strModelPath,const string& strFileName,bool bAsyn)
{
	...
	_snprintf(szTempt,_MAX_PATH,"%s",strFileName.c_str());

	PutDebugString(szTempt);
	if(bAsyn)
	{
		...
		else
		{
			...
			if(pFile)
			{
				...
				if(pFile)
				{
					...
					{
						_snprintf(szError,1024,
							"CGameModelManager::LoadParticleEmitter():Load ShaderInfo (%s) failed.\n",
							szTempt);
						PutDebugString(szError);
					}
				}
	...
	for(;it != tprojs.end();it++)
	{
		...
		else
		{
			...
			if(pFile)
			{
				...
				rfClose(pFile);
				//装载Shader信息
				for (int i = 0;i < pcteproj->GetMyTpSize();i++)
				{
					if( !LoadShaderInfo(pcteproj->GetShaderGroupHandle(),i),bAsyn)
					{
						_snprintf(szError,1024,
							"CGameModelManager::OnTextureProjectiveShaderGroupReadCompleted():Load ShaderInfo (%s) failed.\n",
							pFile->GetFileName());
						PutDebugString(szError);
					}
				}
...
BOOL CGameModelManager::LoadWater(GameModel * pGameModel, const string & strModelPath, bool bAsyn)
{
	...
	}else
	{
		...
		{
			_snprintf(szError, 1024, "CGameModelManager::LoadWater():Load ShaderInfo (%s) failed.\n", pFile->GetFileName());
			PutDebugString(szError);
		}
...
{	
	...
	if(itSD != m_GameModelSToD.end())
	{
		...
		{
			_snprintf(szTempt,_MAX_PATH,"错误，从GameModels列表中得到%s模型为空.",strFileName.c_str());
			PutDebugString(szTempt);
		}
	}
	else
	{		
		...
		{
			PutDebugString("GameModels array overflow.\n");
	...
	if(pGameModelInfo)
	{
		...
		{
			PutDebugString("DisplayModels array overflow.\n");
			return pDisplayModel;
		}
								 ...
								 CRFile* pFile,bool bAsyn)
{
	...
	{
		_snprintf(szError,1024,
			"CGameModelManager::LoadMesh():Load Mesh File (%s\\%s) failed,.\n",
			m_GameModels[dwModelID]->strFileName,pRenderMesh->pszMeshFileName);
		PutDebugString(szError);
		return FALSE;
	}
	...
	{
		_snprintf(szError,1024,
			"CGameModelManager::LoadMesh():Create Mesh File (%s\\%s) failed,.\n",
			m_GameModels[dwModelID]->strFileName,pRenderMesh->pszMeshFileName);
		PutDebugString(szError);
	...
	{
		_snprintf(szError,1024,
			"CGameModelManager::LoadMesh():Load ShaderInfo (%s) failed.\n",
			pFile->GetFileName());
		PutDebugString(szError);
	}
...
BOOL CGameModelManager::LoadTexture(render::Interface::tagTextureInfo* pTextureInfo,DWORD dwTextureID,bool bAsyn)
{
	...
	else
	{
		...
		{
			_snprintf(szError,1024,"CGameModelManager::LoadTexture():Load texture (%s) failed.\n",szTempt);
			PutDebugString(szError);
									...
									DWORD dwPart,DWORD & dwLevel,bool bAsyn)
{
	...
	if(NULL == pGroup)
	{
		_snprintf(szError,1024,"Load Group info Error!(Model:%s,Group:%s)",
			pGameModelInfo->strFileName,Dw2Str(dwGroup));
		PutDebugString(szError);
		return pLevel;
	}
	//如果Part是0,选择默认Part
	...
	if(NULL == pPart)
	{
		_snprintf(szError,1024,"Load Part info Error!(Model:%s,Group:%s,Part:%d)",
			pGameModelInfo->strFileName,Dw2Str(dwGroup),dwPart);
		PutDebugString(szError);
		return pLevel;
	}
	...
	if(pPart->GetLoadState() == Load_Did)
	{
		...
		{
			_snprintf(szError,1024,"Load level info error!(Model:%s,Group:%s,Part:%d,Lvl:%d)",
				pGameModelInfo->strFileName,Dw2Str(dwGroup),dwPart,dwLevel);
			PutDebugString(szError);
		}
	}
	return pLevel;
												...
												tagAsynRFParam* pParam,CRFile* prfFile)
{
	...
	if(pLocator && prfFile)
	{
		...
		{
			//错误
			_snprintf(szError,1024,"CGameModelManager::OnLoacatorReadCompleted():Create Loacator (%s) failed.\n",
				prfFile->GetFileName());
			PutDebugString(szError);
		}
		pLocator->SetLoadState(Load_Did);
												...
												tagAsynRFParam* pParam,CRFile* prfFile)
{
	...
	if(ppickmesh && prfFile)
	{
		...
		{
			//错误
			_snprintf(szError,1024,"CGameModelManager::OnPickMeshReadCompleted():Create PickMesh (%s) failed.\n",
				prfFile->GetFileName());
			PutDebugString(szError);
		}
											  ...
											  CRFile* prfFile)
{
	...
	{
		_snprintf(szError,1024,"CGameModelManager::OnLoacatorReadCompleted():Create Loacator (Model:%s) failed.\n",
			pGameModelInfo->strFileName);
		PutDebugString(szError);
	}
}
											...
											CRFile* prfFile)
{
	...
	if(NULL == prfFile)
	{
		_snprintf(szError,1024,"Load resource of part error!(Model:%s,Group:%s,Part:%s)",
			pModelInfo->strFileName,Dw2Str(pParam->dwGroup),Dw2Str(pParam->dwPart));
		PutDebugString(szError);
		return;
	}
	//创建Part的模型数据
	...
	for(;it!=DisplayModels.end();it++)
	{
			...
			pDisplayModelInfo->pDisplayModel->IsGroupPartShow(pParam->dwGroup,pParam->dwPart))
		{
			...
			{
				_snprintf(szError,1024,"取模型等级信息出错!(Model:%s,Group:%s,Part:%d,Lvl:%d)",
					pModelInfo->strFileName,Dw2Str(pParam->dwGroup),pParam->dwPart,dwLevel);
				PutDebugString(szError);
			}
...
void CGameModelManager::OnTextureReadCompleted(tagAsynRFParam* pParam,CRFile* prfFile)
{
	...
	{
		_snprintf(szError,1024,"CGameModelManager::OnTextureReadCompleted():Load texture (%s) failed.\n",pTextureInfo->pszFileName);
		PutDebugString(szError);
	}
	pTextureInfo->SetLoadState(Load_Did);
													...
													tagAsynRFParam* pParam,CRFile* prfFile)
{
	...
	{
		_snprintf(szError,1024,"CGameModelManager::OnParticleMeshReadCompleted():Load particle mesh(modelname:%s) failed.\n",pGameModelInfo->strFileName);
		PutDebugString(szError);
	}
}
														   ...
														   tagAsynRFParam* pParam,CRFile* prfFile)
{
	...
	{
		_snprintf(szError,1024,
			"CGameModelManager::OnParticleShaderGroupReadCompleted():Load ShaderInfo (%s) failed.\n",
			prfFile->GetFileName());
		PutDebugString(szError);
	}
}
																	...
																	tagAsynRFParam* pParam,CRFile* prfFile)
{
	...
	//装载Shader信息
	for (int i = 0;i < pctexproj->GetMyTpSize();i++)
	{
 ...
 *		增加了同步方式重载模型资源
 */
void CGameModelManager::ReloadAllModel(bool bAsyn)
{
	...
	for(;GameModelID<GModelArraySize;GameModelID++)
	{
		if(m_GameModels[GameModelID])
		{
			...
			//释放所有资源
			set<DWORD> & DisplayModelsOld = pGameModelInfo->DisplayModels;
			set<DWORD>::iterator it = DisplayModelsOld.begin();
			for(;it!=DisplayModelsOld.end();it++)
			{
				tagDisplayModelInfo* pDisplayModelInfo = m_DisplayModels[(*it)];
				if(pDisplayModelInfo)
				{
			...
			{
				CRFile * prfFile;
				FILE* pf = fopen(szTempt,"rb");
				if(pf)
				{
					prfFile = new CRFile(szTempt,pf);
					fclose(pf);
				}
				...
				//装载BindPos
				LoadBindPos(pGameModelInfo->pGameModel,pGameModelInfo->strFileName, false);
				//装载Locator
				LoadLoactor(pGameModelInfo->pGameModel,pGameModelInfo->strFileName, false);
				//装载ParticleEmitter
				LoadParticleEmitter(pGameModelInfo->pGameModel,pGameModelInfo->strFileName,false);
				//装载TraiEmitter
				LoadTrailEmitter(pGameModelInfo->pGameModel,pGameModelInfo->strFileName, false);
				//装载投影纹理
				LoadTextureProjective(pGameModelInfo->pGameModel,pGameModelInfo->strFileName, false);
				//装载PickMesh
				LoadPickMesh(pGameModelInfo->pGameModel,pGameModelInfo->strFileName, false);
				//装载水面效果
				LoadWater(pGameModelInfo->pGameModel, pGameModelInfo->strFileName, false);

				set<DWORD> &DisplayModelsNew = pGameModelInfo->DisplayModels;
				it = DisplayModelsNew.begin();
				for(;it!=DisplayModelsNew.end();it++)
				{
					tagDisplayModelInfo* pDisplayModelInfo = m_DisplayModels[(*it)];
					if(pDisplayModelInfo)
					{
						...
						CDisplayModel::mapDisplayGroups& DisplayGroups = pDisplayModelInfo->pDisplayModel->GetGroups();
						CDisplayModel::itDisplayGroup itDG = DisplayGroups.begin();
						for(;itDG != DisplayGroups.end();itDG++)
						{
							CDisplayModel::CDisplayGroup* pDisplayGroup = (*itDG).second;
							...
							if(pGroup)
							{
								...
								if(pPart)
								{
									...
									if(pPart->GetLoadState() == Load_Not)
										LoadPart(pPart,pGameModelInfo->dwID,dwGroup,dwPart,	pGameModelInfo->strFileName);
								}
							}
						}
					}
