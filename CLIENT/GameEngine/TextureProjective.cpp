...
//#include "..\GameClient\AppClient\ClientRegion.h"
...
void CTextureProjective::CreateFromFile(CRFile* pfile)
{
	...
	for(int i = 0;i < shadercount;i++)
	{
		...
		{
			MessageBox(NULL,"not define texture BUFFER size!","Error",MB_OK);
			return;
		}
...
HRESULT CTextureProjective::CreateShadowTexture()
{
	...
	if (!m_simpleshadow.pShadowTexture)
	{
		...
		if (!m_simpleshadow.pShadowTexture)
		{
			MessageBox(NULL,"read simple shadow texture faild!","Error",MB_OK);
		}
		rfClose(rfile);
	}
	if (m_bsimpleshadow)
...
void CTextureProjective::BeginRendShadow(D3DXVECTOR3 pos,DWORD size)
{
	...
	if (!m_bsimpleshadow)
	{
		...
		D3DXMatrixOrthoLH(&m_mattempprojective,size,size,0,1000.0f);
		//	pd3dDevice->EndScene();
...
CTextureProjective::SHADOWTEXTURE* CTextureProjective::GetCanUseShadowTexture()
{
	...
	for (int i = 1;i < m_ushadowusednum;i++)
	{
		it++;
	}
	return &(*it);
...
void CTextureProjective::RendShadow(SHADOWTEXTURE* p,D3DXVECTOR3 ldir,D3DXMATRIX matworld,D3DXMATRIX matview,D3DXMATRIX matproj)
{
	...
	if (!m_bsimpleshadow)
	{
		D3DXMatrixOrthoLH(&m_matshadowproj,p->size,p->size,1.0f,200.0f);
	}
	else
	{
...
int CTextureProjective::CreateRTTTexture()
{
	...
	if (!m_pRttAlphaMainPlayer)
	{
		...
		if (!m_pRttAlphaMainPlayer)
		{
			MessageBox(NULL,"read MainPlayer's HeadPIC Alpha texture faild!","Error",MB_OK);
		}
		rfClose(rfile);
	}
	if (!m_pRttAlphaCircle)
	{
		...
		if (!m_pRttAlphaCircle)
		{
			MessageBox(NULL,"read team's HeadPIC Alpha texture faild!","Error",MB_OK);
		}
		rfClose(rfile);
	}
	// 创建RTT纹理
