...
BOOL GameModel::Group::Part::Create(utility::Text::Node *pPartNode,const char *szModelPath)
{
	...
	*((DWORD*)szTemp) = 0;
	...
	if(NULL == pTempVar)
	{
		_snprintf(szErrorInfo,1024,"model file(%s) Part(%s) config error。",szModelPath,pPartNode->GetName());
		PutDebugString(szErrorInfo);
		return FALSE;
	}
...
BOOL GameModel::Group::Part::Create(CRFile * pFile)
{
	...
	if (pMeshArrayNode)
	{
		...
		for(DWORD n = 0; n < m_dwRenderMeshCount; n ++)
		{
			...
			if (!pTempVar)
			{
				MessageBox(NULL,m_strFileName,"not define Mesh file!",MB_OK);
			}
			if (pTempVar)
			{
	...
	if (pLevelArrayNode)
	{
		...
		for(DWORD n = 0; n < m_dwLevelCount; n ++)
		{
			...
			{
				//错误
				MessageBox(NULL,"read LevelNode faild!","Error",MB_OK);
			}
		}
	}
	script.Destroy();
...
BOOL GameModel::Group::Create(utility::Text::Node *pGroupNode,const char *szModelPath)
{
	...
	if (dwPartCount)
	{
		for(DWORD n = 0; n < dwPartCount; n++)
		{
			...
			if(!bResult)
			{
				_snprintf(szErrorInfo,1024,"model(%s) Group(%s) config error",szModelPath,pGroupNode->GetName());
				PutDebugString(szErrorInfo);
			}
			ASSERT(bResult);
...
BOOL GameModel::Create(const char *pszFileName,DWORD dwTextureProcision)
{
	...
	if (!file.Open(pszFileName))
	{
		...
		MessageBox(NULL,pszFileName,"Create GameModel faild!",MB_OK);
		ASSERT(0);
		return FALSE;
	}
	utility::Text script;
	...
	if (!fileShader.Open(szShaderFileName))
	{
		...
		MessageBox(NULL,szShaderFileName,"Create GameModel faild!",MB_OK);
		return FALSE;
	}
	...
	if (pGroupListNode)
	{
		...
		{
			MessageBox(NULL,"model group num is bigger than GAMEMODEL_MAXGROUP!",NULL,MB_OK);
		...
		for(DWORD n = 0;n < dwGroupCount;n++)
		{
			...
			if (g_bDebug)
			{
				...
				{
					char str[256];
					sprintf(str, "%s 的 group: %04d redefined", pszFileName, n-1);
					MessageBox(NULL,str,"error",MB_OK);
				}
			}
	...
	else
	{
		if (m_mapActionArray.size()!=0)
		{
			...
			MessageBox(NULL,pszFileName,"Error: bindpose file is not defined.\n",MB_OK);
		}
	}
	...
	if (pParticleEmitterNode)
	{
		...
		if (dwCount > GAMEMODEL_MAXPARTICLEEMITTER)
		{
			...
			MessageBox(NULL,pszFileName,"particle_emitter num reachs the GAMEMODEL_MAXPARTICLEEMITTER, discard the overflow parts",MB_OK);
		}
	...
	if (pTrailEmitterNode )
	{
		...
		if (dwCount > GAMEMODEL_MAXTRAILEMITTER)
		{
			...
			MessageBox(NULL,pszFileName,"trail_emitter'num reachs the GAMEMODEL_MAXTRAILEMITTER,discard the overflow parts",MB_OK);
		}
		for(DWORD n =0 ; n < dwCount; n++)
		{
...
BOOL GameModel::Create(CRFile* pFile)
{
	...
	if (pGroupListNode)
	{
		...
		if (dwGroupCount > GAMEMODEL_MAXGROUP)
		{
			MessageBox(NULL,"Mode Group' num reachs the GAMEMODEL_MAXGROUP",NULL,MB_OK);
			dwGroupCount = GAMEMODEL_MAXGROUP - 1;
		}
		for(DWORD n = 0;n < dwGroupCount;n++)
		{
			...
			if (g_bDebug)
			{
				...
				{
					char str[256];
					sprintf(str, "group: %04d redefined",n-1);
					MessageBox(NULL,str,"error",MB_OK);
				}
			}
			m_mapGroupArray.insert(GROUP_PAIR(pGroup->GetName(),pGroup));
	...
	else
	{
		if (m_mapActionArray.size()!=0)
		{
			OutputError("model not define a bindpose.\n");
			MessageBox(NULL,"","Error: bindpose file is not defined.\n",MB_OK);
		}
	}
	...
	if (pParticleEmitterNode)
	{
		...
		{
			dwCount = GAMEMODEL_MAXPARTICLEEMITTER;
			MessageBox(NULL,"","particle_emitter num reachs the GAMEMODEL_MAXPARTICLEEMITTER, discard the overflow part",MB_OK);
		}
	...
	if (pTrailEmitterNode )
	{
		...
		{
			dwCount = GAMEMODEL_MAXTRAILEMITTER;
			MessageBox(NULL,"","trail_emitter'num reachs the GAMEMODEL_MAXTRAILEMITTER,discard the overflow parts",MB_OK);
		}
		for(DWORD n =0 ; n < dwCount; n++)
		{
			TrailEmitter *pTrailEmitter = new TrailEmitter;
...
BOOL GameModel::Group::Part::Level::Create(utility::Text::Node *pLevelNode,const char *szResourcePath)
{
	...
	if (pTempVar)
	{
		...
		if (!m_pAnimColor->Create(szTemp))
		{
			...
			ASSERT(0);
			MessageBox(NULL,szTemp,"Create animcolor faild",MB_OK);			
			return FALSE;
		}
	}
	//读取animUV文件
	...
	if (pTempVar)
	{
		...
		{
			OutputConsole("error: Create\"%s\" failed!\n",szTemp);
			MessageBox(NULL,szTemp,"Create animuv faild",MB_OK);			
			ASSERT(0);
			return FALSE;
		}
	}
	...
	if (pTempVar)
	{
		...
		{
			OutputConsole("error: Create\"%s\" failed!\n",szTemp);
			MessageBox(NULL,szTemp,"Create animvisibility faild",MB_OK);			
			ASSERT(0);
			return FALSE;
		}
	}
	...
	if (pTempVar)
	{
		...
		{
			OutputConsole("error: Create\"%s\" failed!\n",szTemp);
			MessageBox(NULL,szTemp,"Create animtexture faild",MB_OK);			
			ASSERT(0);
			return FALSE;
		}
	}
	...
	if (particlenode)
	{
		for (int i = 0;i < particlenode->GetVarCount();i++)
		{
