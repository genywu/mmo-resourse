...
public:
	//!						
	bool						Init(CMessage *pMsg, LogClientInfo &logClientInfo);

	//!							更新控制条件
	bool						UpdateCondition(CMessage *pMsg);

	//!							设置是否获得LOGSERVER确认
	void						PassAffirm(bool bAffirm);

	void						SetLogSocket(long lLogSocket);

	long						GetLogSocket(void);
...
private:
	...
	//!							从数组中获得表结构
	void						ByteArrayToTable(BYTE *pData, long pos);

	//!							把日志参数压入BYTE数组
	bool						GetLogByteArray(va_list& args, LogTable &tableInfo, vector<BYTE> &vData);
	//!							把输入的参数值按类型压入BYTE数组
	//!							参数args必须是va_start之后的
	//!							参数lType类型为eDataTypeCode
	bool						ParamToByteArray(va_list& args, const LogItem& logItem, vector<BYTE> &vData);
	//!							同上，但要进行附加条件检测
	//!							RE: -1，执行失败； 0，无须检测或检测失败；1，检测通过
	long						ParamToByteArray_AndTestCondition(va_list& args, const LogItem& logItem, vector<BYTE> &vData);
