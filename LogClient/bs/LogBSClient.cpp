...
bool LogBSClient::Init(CMessage *pMsg, LogClientInfo &logClientInfo)
{
...
bool LogBSClient::UpdateCondition(CMessage *pMsg)
{
...
void LogBSClient::PassAffirm(bool bAffirm)
{
	m_BeAffirmed = bAffirm;
}

void LogBSClient::SetLogSocket(long lLogSocket)
{
	m_LogSocket = lLogSocket;
}

long LogBSClient::GetLogSocket(void)
{
	return m_LogSocket;
}

void LogBSClient::Release(void)
{
	//! 结束线程
	if (0 != m_hWirteThread)
	{
...
//!	发送SQL语句日志
bool LogBSClient::SendStrSqlLogToServer(const char *pSql)
{
	if(NULL == pSql)
		return false;
...
//! 时间和服务器编号是默认的输入的
bool LogBSClient::SendLogToServer(long LogType, ...)
{
...
//! 在本地记录服务器格式日志（时间自动添加）
bool LogBSClient::LogToHere(long LogType, ...)
{
	TableMap_ite ite = m_TableInfo.find(LogType);
	if (ite == m_TableInfo.end())
		return false;
...
//! 在本地记录文本格式日志（时间自动添加）
bool LogBSClient::LogTxtToHere(const char *pTxt)
{
	SYSTEMTIME stTime;
	GetLocalTime(&stTime);
	char szTime[1024] = {0};
	sprintf(szTime, "[%02d-%02d %02d:%02d:%02d] ", stTime.wMonth, stTime.wDay, stTime.wHour, stTime.wMinute, stTime.wSecond);
...
//! 设置日志的保存模式
bool LogBSClient::SetSaveMode(long lLogType, eSendControl eSC)
{
...
//! 从数组中获得表结构
void LogBSClient::ByteArrayToTable(BYTE *pData, long pos)
{
	m_TableInfo.clear();
...
//! 把日志参数压入BYTE数组
bool LogBSClient::GetLogByteArray(va_list& args, LogTable &tableInfo, vector<BYTE> &vData)
{
...
bool LogBSClient::ParamToByteArray(va_list& args, const LogItem& logItem, vector<BYTE> &vData)
{
...
long LogBSClient::ParamToByteArray_AndTestCondition(va_list& args, const LogItem& logItem, vector<BYTE> &vData)
{
	long return_value = 0;
...
long LogBSClient::TestCondition(const ItemCondition &condition, void *pCurrValue)
{
...
unsigned __stdcall	LogBSClient::FileWirteThread(void* pThis)
{
	LogBSClient *pLogClient = (LogBSClient*)pThis;
...
//////////////////////////////////////////////////////////////////////////
void LogBSClient::LogFile_ForThread::WirteToBuf(string &strTxt)
{
...
long LogBSClient::LogFile_ForThread::BufToFile(void)
{
	if (0 == _str_buf.size())
	{
		return 1;
...
bool LogBSClient::LogFile_ForThread::OpenStream(void)
{
	if (_ofstream.is_open())
	{
		return true;
	...
	_ofstream.open(_file_name.c_str(), ios::app);
