...
//#ifdef _CLENTTOTEST_
//map<string, DWORD> CClientResource::s_mapNotFoundFiles;			//没有找到的文件<路径, ID>
//#endif
...
void	CClientResource::Initial(eResourceType eType,string& strWorkFolder,string& strFileInfoName,HWND hwWnd)
{
	m_strCurFolder=strWorkFolder;
	m_strFileInfoName=strFileInfoName;
	m_hWnd=hwWnd;
...
DWORD   CClientResource::GetFileContentCRC(string& strFileName)
{
	DWORD dwReCRC32 = 0;
...
CPackage* CClientResource::GetPackage(DWORD dwPackID)
{
...
CPackage* CClientResource::GetPackageForUpdate(DWORD dwPackID)
{
	string strPackPath = m_strCurFolder+"\\Data";
	itPackage it = m_pPackages.find(dwPackID);
	if(it != m_pPackages.end())
...
bool	CClientResource::ReMakePackage(tagFileInfo* pTagFileInfo,CPackage* pNewPackage)
{
	...
	else
	{
		...
		if(pTagFileInfo->bFolder==false && !( pTagFileInfo->dwPackageType&0x1) )
		{
			...
			if(pPackage)
			{
				DWORD dwSize = pPackage->GetFileSize(
...
bool	CClientResource::AddFile(string& strPos,string& strFileName)
{
	...
	/*if(ptagFileInfo && !(ptagFileInfo->dwPackageType&0x1))
	{
		...
		if(pf )
		{
			...
			{
				DWORD dwOrginSize = 0;
				DWORD dwSize = pPackage->AddFile(pf,strNewTemptFile.c_str(),dwCrc32,
...
//从缓冲区一个文件
bool	CClientResource::AddFile(string& strFileName,BYTE* pBuf,long lSize)
{
	FILE *pnewf = fopen(strFileName.c_str(),"w+b");
	if(pnewf)
	{
		fwrite(pBuf, lSize, 1, pnewf);
		fclose(pnewf);
	}
	else
	{
		char szOutPutInfo[256];
		sprintf(szOutPutInfo,"Please check the out path：%s is exist or no %d",strFileName.c_str(),errno);
		PutDebugString(szOutPutInfo);
...
//添加一个没文件内容的文件,下载是用
bool	CClientResource::AddEmptyFile(const char* strFileName,int nSize,DWORD dwCrc32,
									  DWORD dwPackage,DWORD dwCompress,int dwOrginSize)
{
	...
	else
	{
		...
		{
			char str[MAX_PATH]="";
			_snprintf(str, MAX_PATH, "创建文件 %s 出错。",strFileName);
			PutDebugString(str);
			return false;
		}
		fclose(pf);
...
bool	CClientResource::AddFileData(const char* strFileName,BYTE* pBuf,
									 int nSize)
{
	if(nSize <= 0)
		return true;
	...
	else if( (ptagFileInfo->dwPackageType&0x1)==1 )
	{
		...
		if(pf)
		{
			...
			nRet = fwrite(pBuf,nSize,1,pf);
			if(nRet != 1)
			{
				char str[MAX_PATH]="";
				_snprintf(str, MAX_PATH, "写文件 %s 出错。",strFileName);
				PutDebugString(str);
				return false;
			}
			nRet = fflush(pf);
...
bool	CClientResource::IsFileBodyExist(DWORD dwPackage,const char* strFileName)
{
	string strName(strFileName);
	if(strName.find_first_of("\\") != 0 )
	{
...
bool CClientResource::AdjustFile(const char* strFileName,
									DWORD dwTemptPackage,DWORD dwTemptCompress)
{
	string strTemptFileName(strFileName);

	DWORD dwSize=0,dwOrginSize=0,dwValidSzie=0,dwCrc32=2;
	DWORD dwPackageType=1,dwCompressType=1;bool bFolder = false;
...
//提取一个文件夹或文件
bool	CClientResource::ExtrFileFolderFromPatch(tagFileInfo* ptagFileInfo,string strFolder, CPackage* package,
								CFilesInfo* pFilesInfo)
{
	...
	if(pTemptFilesInfo->HaveChild(ptagFileInfo))
	{
		...
		for(itFileInfo it = ptagFileInfo->ChildFilesInfo.begin();
			it != ptagFileInfo->ChildFilesInfo.end();it++)
		{
			ExtrFileFolderFromPatch((*it).second, strDirectory, package, pFilesInfo);	
		}
	}
	else
	{
		if(ptagFileInfo->bFolder==false)
		{
			...
			{
				if(package)
					package->ExtractToFile(strFileName.c_str(),strNewFileName);
			}
			else
			{
	...
	else
	{
		if(ptagFileInfo->bFolder==false)
		{
			...
			else if(ptagFileInfo->dwOrginSize > 0)
			{
				...
				if(fp)
				{
					BYTE *lpszIn = new BYTE[ptagFileInfo->dwOrginSize];
					BYTE *lpszOut = new BYTE[ptagFileInfo->dwOrginSize*2];
					fread(lpszIn, ptagFileInfo->dwOrginSize, 1, fp);
					DWORD nOutSize = ptagFileInfo->dwOrginSize*2;
		...
		//		{
		//			char str[100]="";
		//			sprintf(str,"打开文件 %s 出错。",strFileName);
		//			PutDebugString(str);
		//			return false;
		//		}
		//		assert(fp);
		...
		//		{
		//			char str[100]="";
		//			sprintf(str,"fseek文件 %s 出错。",strFileName);
		//			PutDebugString(str);
		//			return false;
		//		}
		...
		//		{
		//			char str[100]="";
		//			sprintf(str,"ftell文件 %s 出错。",strFileName);
		//			PutDebugString(str);
		//			return false;
		//		}
		...
		//		if(dwSize > 0)
		//		{
		//			BYTE *lpszIn = new BYTE[dwSize];
		//			BYTE *lpszOut = new BYTE[dwSize*2];
...
bool CClientResource::OnExtrFileFolderFromPatch(string& strPos,string& strFolderFileName, CPackage* package)
{
	if(ahThread)
	{
		if(::WaitForSingleObject( ahThread,0 )==WAIT_TIMEOUT)
	...
	ExtrFileFolderFromPatch(ptagFileInfo,strFolderFileName, package);
	return true;
}
	...
	do
	{
		if ( hFind != INVALID_HANDLE_VALUE )
		{
			...
			if ( SetCurrentDirectory(strTempt) )
			{
				...
				{
					DWORD dwPackage = GetFirstEnableBit(pParenttagFileInfo->dwPackageType);
					DWORD dwCompress= GetFirstEnableBit(pParenttagFileInfo->dwCompressType);
					//DWORD dwPackage = 1;
					//DWORD dwCompress= 1;
					pTemptParentInfo =
...
unsigned __stdcall CClientResource::LoadClientResourceProc(void* pArguments)
{
	...
	DWORD dwPackage=0,dwCompress=0;
...
void	CClientResource::SearchOneFile(tagFileInfo* pParenttagInfo,
									   CHAR* strPath, string& strFileName,DWORD dwSize)
{
	////文件中不能有空格
...
//添加一个文件夹
void	CClientResource::AddFolder(string& strFolder,DWORD dwPackage,DWORD dwCompress)
{
	if( dwPackage&0x1 )
	{
...
void	CClientResource::AddFile(string& strSouFolder,string& strFileName,
								 int nSize,DWORD dwCrc32,DWORD dwPackage,DWORD dwCompress)
{
	...
	if( !ENABLEBIT(dwPackage,NOPACK) )
	{
		...
		if(pf)
		{
			...
			{
				DWORD dwOrginSize = 0;
				DWORD dwSize = pPackage->AddFile(pf,strFileName.c_str(),dwCrc32,
...
void CClientResource::ExitAsynReadFileThread()
{
	...
	PutDebugString("Release CSCompletedReadFile OK!");
	//释放已经读取的资源列表
	...
	for(;it != m_CompletedReadFiles.end();it++)
	{
		...
		if(pCompletedFile)
		{
			...
			if(pCompletedFile->prFile && pCompletedFile->prFile->GetFileName())
				_snprintf(strInfo, 256, "Release %s Start",pCompletedFile->prFile->GetFileName());
...
CRFile* CClientResource::SyncReadFile(const char* strFileName)
{
	...
	if (!m_pSyncReadFile->prFile)
	{
		...
		//{
		//	s_mapNotFoundFiles[sFileName] = (DWORD)(s_mapNotFoundFiles.size() + 1);

			char strInfo[512] = "";
			sprintf_s(strInfo, 512, "SyncReadFile:%s NotFind", m_pSyncReadFile->strFileName);
			PutStringToFile("FileNotFind", strInfo);
		//}
#endif
	}
		...
		while(true)
		{
			...
			if (!pAsynReadFile->prFile)
			{
				...
				//	s_mapNotFoundFiles[sFileName] = (DWORD)(s_mapNotFoundFiles.size() + 1);
					...
					char strInfo[512] = "";
					sprintf_s(strInfo, 512, "AsynReadFile:%s NotFind", pAsynReadFile->strFileName);
					PutStringToFile("FileNotFind", strInfo);
				//}
#endif
			}
